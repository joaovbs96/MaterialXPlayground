...
// js/mtlx-engine.js, js/doc-ui.jsx, and js/node-preview.jsx, so this
// script tag must load AFTER all four. Reads window.__MTLX_EMBED (set
// synchronously by index.html's early <head> script) for embed-mode
// behavior.
        ...
        function App({ active = true } = {}) {
            ...
            return (
                <div className="space-y-4 sm:space-y-6 md:h-full md:flex md:flex-col md:min-h-0">
                    ...
                    {/* Help popup: click-outside or Esc closes. Rendered
                        through a portal directly under <body> — a fixed
                        overlay inside the app tree can end up anchored to an
                        ancestor instead of the viewport (any transformed /
                        filtered / backdrop-filtered ancestor becomes the
                        containing block for position:fixed), which offsets
                        the panel and dims only part of the screen. */}
                    {!EMBED && showHelp && ReactDOM.createPortal(
                        <div
