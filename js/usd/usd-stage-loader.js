 ...
 */
export function loadUsdStage({ files, rootPath, onProgress, signal, purposePolicy = "defaultRender", subdivisionLevel = 1, fastWeld = true }) {
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      worker.postMessage({ id, type: "load", files: requestFiles, rootPath, purposePolicy, subdivisionLevel, fastWeld });
    } catch (error) {
