...
| `geometryUrl` | URL to a `.obj`, `.glb`, or `.gltf` model | *(none; uses the `geometry` param above)* | Custom preview model. Fetched by the iframe itself, under the same CORS requirement as `src`. A `.gltf` may reference a sibling `.bin`, fetched relative to the model URL under that same CORS requirement; texture references in the file are always ignored, since previews use geometry only (any KTX2/Basis-compressed texture is stripped right along with the rest, before it's ever decoded). `KHR_draco_mesh_compression` meshes are decoded locally via the vendored Draco WASM decoder. On success the preview switches to the custom model, and the `geometry` HUD control (when shown) gains a "Custom Model" entry so a visitor can switch back to a built-in geometry. A fetch or parse failure keeps the configured geometry and is reported through `mtlx-error`. If you're embedding under a strict host-page CSP, Draco decoding needs `worker-src blob:` allowed. |
