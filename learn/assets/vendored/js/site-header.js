...
(function () {
    ...
    var IS_LEARN = !!window.__MTLX_LEARN__;
    var APP_ROOT = window.__MTLX_APP_ROOT__ || '../';
    var LEARN_URL = 'learn/';
    var IS_SHELL = !IS_LEARN && (/(^|\/)(index\.html)?$/i.test(location.pathname) || !!window.__MTLX_VSCODE__);
    ...
    var NAV = [
        ...
        { id: 'learn', label: 'Learn', shellHref: '#!home' },   // real page; hrefFor() special-cases it
    ];
    ...
    var activeId = IS_LEARN ? 'learn' : shellActiveId(window.location.hash || '');

    // Resolves a NAV item's actual href for the current context: on a
    // learn page (IS_LEARN), every shell view lives back across
    // APP_ROOT (the SPA root) and the Learn tab points at this same
    // page's own root; inside the shell, the Learn tab points at the
    // real /learn/ page and every other item keeps its shellHref as-is.
    function hrefFor(item) {
        if (IS_LEARN) return item.id === 'learn' ? (APP_ROOT + LEARN_URL) : (APP_ROOT + item.shellHref);
        return item.id === 'learn' ? LEARN_URL : item.shellHref;
    }
    var brandHref = IS_LEARN ? (APP_ROOT + '#!home') : (IS_SHELL ? '#!home' : 'index.html');
