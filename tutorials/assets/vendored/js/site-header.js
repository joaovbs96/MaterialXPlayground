...
(function () {
    ...
    var IS_SHELL = !IS_TUTORIALS && (/(^|\/)(index\.html)?$/i.test(location.pathname) || !!window.__MTLX_VSCODE__);
    ...
    var NAV = [
        ...
        { id: 'tutorials', label: 'Tutorials', shellHref: '#!home' },   // real page; hrefFor() special-cases it
    ];
    ...
    // tutorials page (IS_TUTORIALS), every shell view lives back across
    // APP_ROOT (the SPA root) and the Tutorials tab points at this same
    // page's own root; inside the shell, the Tutorials tab points at the
    // real /tutorials/ page and every other item keeps its shellHref as-is.
    function hrefFor(item) {
        if (IS_TUTORIALS) return item.id === 'tutorials' ? (APP_ROOT + TUTORIALS_URL) : (APP_ROOT + item.shellHref);
        return item.id === 'tutorials' ? TUTORIALS_URL : item.shellHref;
    }
