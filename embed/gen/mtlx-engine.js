...
'uniform int u_shadowSlotFace['+SHADOW_LIGHT_SLOTS_MAX+'];','uniform int u_shadowSlotFaceCount['+SHADOW_LIGHT_SLOTS_MAX+'];','float mx_shadow_vsm(vec2 moments, float receiverDepth) {','    float p = (receiverDepth <= moments.x) ? 1.0 : 0.0;','    float variance = max(moments.y - moments.x * moments.x, 2e-7);','    float d = receiverDepth - moments.x;','    float lit = max(p, variance / (variance + d * d));','    return smoothstep(0.3, 1.0, lit);','}','float mx_shadow_atlas(int caster, vec3 P, vec3 Ng) {','    if (caster < 0) return 1.0;','    vec4 depthPlane = u_shadowDepthPlanes[caster];','    vec2 depthRange = u_shadowDepthRanges[caster];','    float nearDepth = depthRange.x;','    float depthSpan = max(depthRange.y, 1e-9);','    vec2 projectionScale = u_shadowSourceRadii[caster].zw;','    bool perspective = projectionScale.x > 0.0 || projectionScale.y > 0.0;',// Normal-offset receiver bias: move the test point about 1.5 caster
...
'    vec2 sourceRadius = u_shadowSourceRadii[caster].xy;',// Offset an orthographic (directional) caster, or a perspective AREA
...
'    bool offsetEligible = !perspective || sourceRadius.x > 0.0 || sourceRadius.y > 0.0;','    float texelWorld = u_shadowTexelWorldSize[caster];','    vec3 offsetP = P;','    if (texelWorld > 0.0 && offsetEligible) {','        float rawDepth = dot(vec4(P, 1.0), depthPlane);','        float rawZ = max(nearDepth + depthSpan * rawDepth, 0.0);','        float scale = perspective ? rawZ : 1.0;','        offsetP = P + Ng * (texelWorld * scale * 0.25);','    }','    vec4 c4 = u_shadowMatrices[caster] * vec4(offsetP, 1.0);','    if (c4.w <= 0.0) return 1.0;','    vec3 sc = c4.xyz / c4.w;','    sc = sc * 0.5 + 0.5;','    if (any(lessThan(sc, vec3(0.0))) || any(greaterThan(sc, vec3(1.0)))) return 1.0;','    vec4 tile = u_shadowTiles[caster];','    vec2 atlasTexel = 1.0 / vec2(textureSize(u_shadowAtlas, 0));','    vec2 tileTexel = atlasTexel / max(tile.zw, vec2(1e-6));',// Clamp the local tile coordinate by half a texel before mapping into
...
'    vec2 localUv = clamp(sc.xy, tileTexel * 0.5, vec2(1.0) - tileTexel * 0.5);','    vec2 centerUv = tile.xy + localUv * tile.zw;','    vec2 moments = texture(u_shadowAtlas, centerUv).xy;',// Projected XY still comes from the real light camera, but moments
// use a linear light-view depth plane supplied by the renderer.
'    float receiverDepth = dot(vec4(offsetP, 1.0), depthPlane);','    float receiverZ = nearDepth + depthSpan * receiverDepth;',// A source with zero extent is a point/directional caster: retain the
