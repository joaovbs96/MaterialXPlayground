...
const getMxEnv = version => {
  ...
  if (!mxEnvPromises.has(ver)) {
    ...
    const factoryPromise = import(factoryUrl).then(mod => typeof mod.default === 'function' ? mod.default : loadMxFactoryViaScript(ver));
    mxEnvPromises.set(ver, factoryPromise.then(factory => factory({
      ...
      const configureGenContext = ctx => {
        ...
        try {
          ctx.getOptions().hwSrgbEncodeOutput = false;
  ...
  if (!renderables.length && synthesizeDefinitions) {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (!copyDef) continue;
        mxSafe(() => {
          copyDef.copyContentFrom(entry.def);
          return true;
        }, false);
        mxSafe(() => {
          copyDef.setName(defCopyName);
          return true;
        }, false);
        for (const g of entry.graphs) {
          ...
          mxSafe(() => {
            copyGraph.copyContentFrom(g);
            return true;
          }, false);
          mxSafe(() => {
            copyGraph.setName(graphCopyName);
            return true;
          }, false);
          mxSafe(() => {
            copyGraph.setNodeDefString(defCopyName);
            return true;
          }, false);
        }
        ...
        if (!inst) continue;
        mxSafe(() => {
          inst.setNodeDefString(defCopyName);
          return true;
        }, false);
        renderables.push({
          name: nodeString + ' (definition)',
          node: inst,
          definition: true
        });
      }
...
const readMtlxXml = async (mx, doc, xml) => {
  let opts = null;
  try {
    if (typeof mx.XmlReadOptions === 'function') {
      opts = new mx.XmlReadOptions();
...
const xmlTokenize = text => {
  ...
  while (m = re.exec(text)) {
    ...
    tokens.push({
      key: m[0][0] === '<' ? xmlTokenKey(m[0]) : m[0].trim(),
      raw: m[0],
      gap: text.slice(gapStart, m.index)
    });
    gapStart = m.index + m[0].length;
  }
  return {
    tokens,
    tail: text.slice(gapStart)
  };
};
...
const xmlPatchTag = (srcRaw, outRaw) => {
  const s = xmlTagParts(srcRaw),
    o = xmlTagParts(outRaw);
  if (!s || !o || s.tag !== o.tag || s.selfClosing !== o.selfClosing) return null;
  const sName = s.attrs.get('name'),
    oName = o.attrs.get('name');
  ...
  s.attrs.forEach((sa, n) => {
    ...
    if (!oa) edits.push({
      ...
      text: ''
    });else if (oa.value !== sa.value) edits.push({
      start: sa.valueStart,
      end: sa.valueEnd,
      text: esc(oa.value, sa.quote)
    });
  });
  let added = '';
  o.attrs.forEach((oa, n) => {
    if (!s.attrs.has(n)) added += ' ' + n + '="' + esc(oa.value, '"') + '"';
  });
  if (added) edits.push({
    start: s.insertAt,
    end: s.insertAt,
...
const preserveSourceFormatting = (sourceText, writtenXml, {
  maxD = 3000,
  ...
  for (let j = 0; j < out.tokens.length; j++) {
    ...
    for (let jj = j + 1; jj < out.tokens.length; jj++) if (match[jj] >= 0) {
      nextSrc = match[jj];
      break;
    }
    let patched = null;
