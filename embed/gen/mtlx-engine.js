...
const countFragmentSamplers=fs=>{const re=/uniform\s+(?:(?:low|medium|high)p\s+)?(sampler2D|sampler3D|samplerCube)\s+(\w+)\s*(?:\[\s*(\d+)\s*\])?\s*;/g;const names=[];let count=0;let m;while((m=re.exec(fs))!==null){const size=m[3]?parseInt(m[3],10):1;count+=size;names.push(m[3]?m[2]+'['+m[3]+']':m[2]);}return{count,names};};// three.js RawShaderMaterial + glslVersion:GLSL3 prepends its own
...
const SHADOW_FACE_SLOTS=24;const SHADOW_LIGHT_SLOTS_MAX=32;// One bias policy for every caster kind: a normal offset off the surface
...
const patchRgbtPayload=fs=>{const original=fs;let supported=true;let out=fs;const transFnIdx=out.indexOf('vec3 mx_surface_transmission');if(transFnIdx!==-1){const bodyIdx=out.indexOf('{',transFnIdx);if(bodyIdx===-1)supported=false;else if(out.indexOf('u_peelRgbt',transFnIdx)===-1){out=out.slice(0,bodyIdx+1)+'\n    if (u_peelRgbt != 0) return tint;\n'+out.slice(bodyIdx+1);}}// Restrict the replacement to the generated viewing-transmission section
...
const firstFn=out.search(/^(?:void|vec[234]|float|int|bool|mat[234])\s+\w+\s*\(/m);const decl='uniform int u_peelRgbt;\nuniform int u_peelRgbtPass;\nuniform int u_peelRgbtLayer;\n/* MX_RGBT_PAYLOAD_SUPPORTED */\n';if(firstFn===-1)return original;if(out.indexOf('uniform int u_peelRgbt;')===-1){out=out.slice(0,firstFn)+decl+out.slice(firstFn);}// patchTransmissionAlpha runs first to preserve the complete legacy
...
const skipSkyVis=!!(sceneFeatureOptions&&sceneFeatureOptions.skipSkyVis);const dropThicknessMap=!!(sceneFeatureOptions&&sceneFeatureOptions.dropThicknessMap);// OFFICIAL PARITY: per-material generation options on SHARED
...
fs=patchTransmissionAlpha(fs);let payloadSupported=false;if(sceneRgbt){fs=patchRgbtPayload(fs);payloadSupported=fs.indexOf('/* MX_RGBT_PAYLOAD_SUPPORTED */')!==-1;}let lightTransportSupported=false;if(lightTransport){fs=patchLightTransportPayload(fs,notices,lightTransport);lightTransportSupported=fs.indexOf('MX_LIGHT_TRANSPORT_TERMINAL_RETURN')!==-1;}fs=patchShadowBounds(fs);fs=patchShadowLightScope(fs);fs=patchLightSourceKindStruct(fs);fs=patchAreaLightSourceCosine(fs);fs=patchAmbientOcclusion(fs,{skipSkyVis});fs=patchTransmissionThickness(fs,{dropThicknessMap});// Depth-peel machinery: baked into every fragment shader
...
const generatePreviewSources=(...args)=>mxExclusive(()=>generatePreviewSourcesUnlocked(...args));// ANGLE D3D11 reports 16 texture image units; a Scene material bakes nine
...
const SAMPLER_BUDGET_DROP_ORDER=[{key:'skipSkyVis',label:'sky visibility (u_skyVisMap)'},{key:'dropThicknessMap',label:'thickness map (u_thicknessMap)'}];// Scene-view material compiler. This deliberately exposes the preview shader
...
const compileMtlxSceneMaterial=async({mx,gen,genContext,renderable,label='material',isMounted=()=>true,document:documentArg=null,sceneRgbt=false,lightTransport=false,samplerBudget=null})=>{if(!renderable)throw new Error('MaterialX scene material is missing its renderable surface.');// Test-only override wins over the caller's live GL limit, so a headless
...
if(has('u_ssaoMap'))uniforms.u_ssaoMap={value:ssaoMap||getDummyTexWhite()};if(has('u_ssaoTexel'))uniforms.u_ssaoTexel={value:ssaoTexel?ssaoTexel.clone():new THREE.Vector2()};if(has('u_ssaoStrength'))uniforms.u_ssaoStrength={value:ssaoMap?ssaoStrength:0};// Zero scale means zero path length, which is clear glass: the safe
...
return{render,supported:halfOk,dispose:free,debug:()=>({opaque:resources?resources.opaque:null})};};// createPeelPipeline(renderer, { getDisplayTransform, getDisplayExposure }): reusable depth-
