...
const MTLX_DEFAULT_VERSION='1.39.5';// MaterialX light shader ids. The id bound with bindLightShader IS the
...
const LIGHT_TYPE_DIRECTIONAL=1;const LIGHT_TYPE_POINT=2;const LIGHT_TYPE_SPOT=3;// Per-light source shape, kept separate from MaterialX's light type because
...
const LIGHT_SOURCE_KIND_AREA=1;// Slots reserved for lights imported from a USD stage. This lands in the
...
const STAGE_LIGHT_SLOTS=16;// Rig lights parsed from environment_map.mtlx, recorded here so the
// light-limit tiers can size one generation without the getMxEnv closure.
let mxRigLightCount=0;// Light-limit kill switch, default ON. '0' disables it, read per
// generation (one localStorage hit) so toggling it needs no reload.
const LIGHT_LIMIT_KEY='mtlx_light_limit';const readLightLimit=()=>{try{return localStorage.getItem(LIGHT_LIMIT_KEY)!=='0';}catch(e){return true;}};// Stage-light slot tiers. MAX_LIGHT_SOURCES is baked into the generated
...
const STAGE_LIGHT_TIERS=[0,4,8,STAGE_LIGHT_SLOTS];// Viewer, Compare, docs previews, Graph previews and embeds bind the rig
...
const PREVIEW_STAGE_LIGHT_COUNT=0;// Feature-gated shaders kill switch, default ON. '0' restores the old
// always-generate behaviour (shadow sampling and the occlusion block in
// every material of every tool). Read per generation.
const FEATURE_GATED_KEY='mtlx_feature_gated_shaders';const readFeatureGated=()=>{try{return localStorage.getItem(FEATURE_GATED_KEY)!=='0';}catch(e){return true;}};// Features no preview tool can ever turn on: the Viewer, Compare, docs and
...
const PREVIEW_FEATURE_OPTIONS={skipShadowMap:true,skipOcclusion:true};// Smallest tier that still covers `count`; anything unknown or over the
// ceiling falls back to the full reservation.
const chooseStageLightTier=count=>{const n=Number(count);if(!Number.isFinite(n)||n<0)return STAGE_LIGHT_SLOTS;const tier=STAGE_LIGHT_TIERS.find(t=>t>=n);return tier===undefined?STAGE_LIGHT_SLOTS:tier;};const mxEnvPromises=new Map();// Classic-<script> fallback for UMD builds (e.g. 1.39.4) that have no
...
let mxQueueTail=Promise.resolve();// Lock-discipline diagnostics (see mxWarnIfLocked). mxLockDepth counts
...
let mxLockDepth=0;let mxExclusiveHeldSync=false;function mxExclusive(fn){mxLockDepth++;const run=()=>Promise.resolve().then(()=>{mxExclusiveHeldSync=true;try{return fn();}finally{mxExclusiveHeldSync=false;}});const p=mxQueueTail.then(run,run);// The tail must never carry a rejection forward (it would look like
// every later caller failed), settle it to undefined either way.
mxQueueTail=p.then(()=>undefined,()=>undefined);// Lock depth follows the OUTER promise (fn plus anything it awaits),
// not just the synchronous run() above, settles whether fn resolved
// or rejected.
p.then(()=>{mxLockDepth--;},()=>{mxLockDepth--;});return p;}// Tripwire for synchronous wasm helpers called lock-free from the JSX
...
const mxWarnIfLocked=name=>{if(mxLockDepth>0&&!mxExclusiveHeldSync){console.warn('[mtlx] '+name+' called while an exclusive wasm operation is in flight, possible heap-detach hazard; route this call through mxExclusive.');}};// Logs generated GLSL + discovered uniforms, fastest way to diagnose a
...
try{if(!window.MTLX_PERF_LOG&&localStorage.getItem('mtlxPerfLog')){window.MTLX_PERF_LOG=true;}}catch(e){/* ignore */}// Gated console.warn for expected/recoverable conditions (e.g. a missing
...
const mtlxWarn=(...args)=>{if(DEBUG_SHADERS)console.warn(...args);};// "Force Transparency" (Settings dialog, default off). Off = official-
...
let FORCE_TRANSPARENCY=(()=>{try{return localStorage.getItem('mtlxForceTransparency')==='1';}catch(e){return false;}})();const getForceTransparency=()=>FORCE_TRANSPARENCY;const setForceTransparency=(v,{persist=true}={})=>{FORCE_TRANSPARENCY=!!v;if(persist){try{localStorage.setItem('mtlxForceTransparency',FORCE_TRANSPARENCY?'1':'0');}catch(e){/* best-effort */}}// Settings-dialog/Scene-card callers persist (default); embed-boot.js's
...
LIVE_VIEWS.forEach(view=>{try{view.refreshRenderMode&&view.refreshRenderMode();}catch(e){/* view mid-teardown */}});try{window.dispatchEvent(new CustomEvent('mtlx-settings-changed',{detail:{key:'forceTransparency',value:FORCE_TRANSPARENCY}}));}catch(e){/* best-effort */}};// NOTE: no separate "depth peeling" setting exists, Force Transparency
...
let DISPLACEMENT_ENABLED=(()=>{try{const qs=new URLSearchParams(window.location.search);if(qs.has('displacement')){const parsed=parseBoolFlag(qs.get('displacement'));if(parsed!==null)return parsed;}return localStorage.getItem('mtlxDisplacement')!=='0';}catch(e){return true;}})();const getDisplacementEnabled=()=>DISPLACEMENT_ENABLED;const setDisplacementEnabled=(v,{persist=true}={})=>{DISPLACEMENT_ENABLED=!!v;if(persist&&window.self===window.top){try{localStorage.setItem('mtlxDisplacement',DISPLACEMENT_ENABLED?'1':'0');}catch(e){/* best-effort */}}LIVE_VIEWS.forEach(view=>{try{view.refreshDisplacement&&view.refreshDisplacement();}catch(e){/* view mid-teardown */}});try{window.dispatchEvent(new CustomEvent('mtlx-settings-changed',{detail:{key:'displacement',value:DISPLACEMENT_ENABLED}}));}catch(e){/* best-effort */}};// Displacement shading-normal mode (Settings/test hook): 'analytic' derives
...
let DISPLACEMENT_NORMALS_MODE=(()=>{try{const qs=new URLSearchParams(window.location.search);if(qs.has('displacementnormals')){const v=qs.get('displacementnormals');if(v==='analytic'||v==='mesh')return v;}const raw=localStorage.getItem('mtlxDisplacementNormals');if(raw==='analytic'||raw==='mesh')return raw;// Default to 'mesh' until the analytic path is verified free of the
...
return'mesh';}catch(e){return'mesh';}})();const getDisplacementNormalsMode=()=>DISPLACEMENT_NORMALS_MODE;const setDisplacementNormalsMode=(v,{persist=true}={})=>{if(v!=='analytic'&&v!=='mesh')return;DISPLACEMENT_NORMALS_MODE=v;if(persist&&window.self===window.top){try{localStorage.setItem('mtlxDisplacementNormals',DISPLACEMENT_NORMALS_MODE);}catch(e){/* best-effort */}}LIVE_VIEWS.forEach(view=>{try{view.refreshDisplacement&&view.refreshDisplacement();}catch(e){/* view mid-teardown */}});try{window.dispatchEvent(new CustomEvent('mtlx-settings-changed',{detail:{key:'displacementNormals',value:DISPLACEMENT_NORMALS_MODE}}));}catch(e){/* best-effort */}};// Preview subdivision level (Settings dialog, default 2, 0..3). Feeds
...
let PREVIEW_SUBDIVISION_LEVEL=(()=>{try{const qs=new URLSearchParams(window.location.search);if(qs.has('previewsubdivision')){const n=Number(qs.get('previewsubdivision'));if(Number.isInteger(n)&&n>=0&&n<=3)return n;}const raw=localStorage.getItem('mtlxPreviewSubdivision');if(raw!==null){const stored=Number(raw);if(Number.isInteger(stored)&&stored>=0&&stored<=3)return stored;}return 2;}catch(e){return 2;}})();const getPreviewSubdivisionLevel=()=>PREVIEW_SUBDIVISION_LEVEL;const setPreviewSubdivisionLevel=(level,{persist=true}={})=>{const n=Number(level);if(!Number.isFinite(n))return;// non-numeric input is ignored
const clamped=Math.min(3,Math.max(0,Math.round(n)));PREVIEW_SUBDIVISION_LEVEL=clamped;if(persist&&window.self===window.top){try{localStorage.setItem('mtlxPreviewSubdivision',String(PREVIEW_SUBDIVISION_LEVEL));}catch(e){/* best-effort */}}LIVE_VIEWS.forEach(view=>{try{view.refreshDisplacement&&view.refreshDisplacement();}catch(e){/* view mid-teardown */}});try{window.dispatchEvent(new CustomEvent('mtlx-settings-changed',{detail:{key:'previewSubdivision',value:PREVIEW_SUBDIVISION_LEVEL}}));}catch(e){/* best-effort */}};// Highest triangle count a preview mesh may reach after subdivision,
...
let HEIGHT_TO_NORMAL_TEXEL=(()=>{try{const qs=new URLSearchParams(window.location.search);if(qs.has('heightToNormalTexel'))return qs.get('heightToNormalTexel')==='1';return localStorage.getItem('mtlxHeightToNormalTexel')==='1';}catch(e){return false;}})();// Specular environment method. 'prefilter' is MaterialXView's path: the
...
let DIFFUSE_ENV_METHOD=(()=>{try{if(window.MTLX_DIFFUSE_ENV==='sh'||window.MTLX_DIFFUSE_ENV==='convolve')return window.MTLX_DIFFUSE_ENV;const qs=new URLSearchParams(window.location.search);if(qs.has('diffuseEnv'))return qs.get('diffuseEnv')==='sh'?'sh':'convolve';return localStorage.getItem('mtlx_diffuse_env')==='sh'?'sh':'convolve';}catch(e){return'convolve';}})();const getDiffuseEnvMethod=()=>DIFFUSE_ENV_METHOD;const setDiffuseEnvMethod=(v,{persist=true}={})=>{DIFFUSE_ENV_METHOD=v==='sh'?'sh':'convolve';if(persist){try{localStorage.setItem('mtlx_diffuse_env',DIFFUSE_ENV_METHOD);}catch(e){/* best-effort */}}// Not generation-affecting: same lookup, same uniform, just a rebind,
...
const PEEL_LAYERS=8;// Shared 1x1 opaque-black dummy texture: the DEFAULT binding for the
...
let MTLX_DUMMY_TEX=null;const getDummyTex=()=>{if(!MTLX_DUMMY_TEX){MTLX_DUMMY_TEX=new THREE.DataTexture(new Uint8Array([0,0,0,255]),1,1,THREE.RGBAFormat);MTLX_DUMMY_TEX.needsUpdate=true;}return MTLX_DUMMY_TEX;};// White counterpart, depth==1.0 (far plane): the fail-safe default for
...
let MTLX_DUMMY_TEX_WHITE=null;// Shadow matrix meaning "no shadow": maps every world position to the origin,
...
const parseUniforms=src=>{const out=[];const re=/uniform\s+(\w+)\s+(u_\w+)\s*(?:\[\s*\w+\s*\])?\s*;/g;let m;while((m=re.exec(src))!==null)out.push({type:m[1],name:m[2]});return out;};// Counts sampler declarations that cost a texture image unit, including
...
const countFragmentSamplers=fs=>{const re=/uniform\s+(?:(?:low|medium|high)p\s+)?(sampler2D|sampler3D|samplerCube)\s+(\w+)\s*(?:\[\s*(\d+)\s*\])?\s*;/g;const names=[];let count=0;// Scene/engine samplers are all u_-prefixed; a material's own texture
// reads are named after their image node (<node>_file).
let scene=0;let m;while((m=re.exec(fs))!==null){const size=m[3]?parseInt(m[3],10):1;count+=size;if(/^u_/.test(m[2]))scene+=size;names.push(m[3]?m[2]+'['+m[3]+']':m[2]);}return{count,names,scene,material:count-scene};};// Estimates fragment uniform vector cost against MAX_FRAGMENT_UNIFORM_VECTORS.
...
const estimateFragmentUniformVectors=fs=>{const defines=new Map();const defineRe=/#define\s+(\w+)\s+(\d+)\b/g;let dm;while((dm=defineRe.exec(fs))!==null)defines.set(dm[1],parseInt(dm[2],10));const resolveSize=token=>/^\d+$/.test(token)?parseInt(token,10):defines.has(token)?defines.get(token):1;const structRows=new Map();const baseTypeRows=type=>{if(structRows.has(type))return structRows.get(type);if(type==='mat2'||type==='mat2x2')return 2;if(type==='mat3'||type==='mat3x3')return 3;if(type==='mat4'||type==='mat4x4')return 4;return 1;};const structRe=/struct\s+(\w+)\s*\{([^}]*)\}/g;const memberRe=/(?:(?:low|medium|high)p\s+)?(\w+)\s+\w+\s*(?:\[\s*(\w+)\s*\])?\s*;/g;let sm;while((sm=structRe.exec(fs))!==null){let rows=0;let mm;memberRe.lastIndex=0;while((mm=memberRe.exec(sm[2]))!==null)rows+=baseTypeRows(mm[1])*(mm[2]?resolveSize(mm[2]):1);structRows.set(sm[1],rows);}const isSampler=type=>/^sampler(2D|3D|Cube)$/.test(type);const uniformRe=/uniform\s+(?:(?:low|medium|high)p\s+)?(\w+)\s+(\w+)\s*(?:\[\s*(\w+)\s*\])?\s*;/g;const entries=[];let um;while((um=uniformRe.exec(fs))!==null){const[,type,name,sizeToken]=um;const size=sizeToken?resolveSize(sizeToken):1;const rows=isSampler(type)?0:baseTypeRows(type)*size;entries.push({name,type,rows});}const estimate=entries.reduce((sum,e)=>sum+e.rows,0);const largest=entries.slice().sort((a,b)=>b.rows-a.rows).slice(0,5);return{estimate,largest};};// three.js RawShaderMaterial + glslVersion:GLSL3 prepends its own
...
const parseVertexInputs=vs=>{const out=[];const re=/^\s*in\s+(\w+)\s+(i_\w+)\s*;/gm;let m;while((m=re.exec(vs))!==null)out.push({type:m[1],name:m[2]});return out;};// Integer geomprop varyings need GLSL ES 3.00's "flat" qualifier, and every
...
const INT_GEOMPROP_TYPES={int:'float',ivec2:'vec2',ivec3:'vec3',ivec4:'vec4',uint:'float',uvec2:'vec2',uvec3:'vec3',uvec4:'vec4'};const isIntGeompropType=type=>Object.prototype.hasOwnProperty.call(INT_GEOMPROP_TYPES,type);// ESSL's geompropvalue node names both the vertex input and the vertex-data
...
const patchGeompropVaryings=(vs,fs)=>{const outRe=/^\s*out\s+(\w+)\s+i_geomprop_(\w+)\s*;/gm;const hasOut=outRe.test(vs);const inRe=/^\s*in\s+\w+\s+i_geomprop_\w+\s*;/m;if(!hasOut&&inRe.test(vs)){mtlxWarn('mtlx-engine: found a vertex "in i_geomprop_*" without a matching "out", patchGeompropVaryings skipped.');}// Connector type per geomprop name, gathered before any rewrite.
const connectorTypes=new Map();outRe.lastIndex=0;let om;while((om=outRe.exec(vs))!==null)connectorTypes.set(om[2],om[1]);const vsLines=vs.split('\n').map(line=>{const declMatch=line.match(/^(\s*)in\s+(\w+)\s+i_geomprop_(\w+)\s*;/);if(declMatch){const[,indent,type,name]=declMatch;const floatType=INT_GEOMPROP_TYPES[type];return floatType?`${indent}in ${floatType} i_geomprop_${name};`:line;}const outMatch=line.match(/^(\s*)out\s+(\w+)\s+i_geomprop_(\w+)\s*;/);if(outMatch){const[,indent,type,name]=outMatch;return`${indent}${isIntGeompropType(type)?'flat ':''}out ${type} vd_geomprop_${name};`;}const connMatch=line.match(/^(\s*)i_geomprop_(\w+)\s*=\s*i_geomprop_\2\s*;/);if(connMatch){const[,indent,name]=connMatch;const type=connectorTypes.get(name);return isIntGeompropType(type)?`${indent}vd_geomprop_${name} = ${type}(round(i_geomprop_${name}));`:`${indent}vd_geomprop_${name} = i_geomprop_${name};`;}// Any other in-stage use of an integer geomprop (e.g. displacement
// reading it directly) rounds the float attribute back to its int type.
let out=line;for(const[name,type]of connectorTypes){if(!isIntGeompropType(type))continue;out=out.replace(new RegExp('\\bi_geomprop_'+name+'\\b','g'),`${type}(round(i_geomprop_${name}))`);}return out;});const patchedVs=vsLines.join('\n');const patchedFs=fs.replace(/\bi_geomprop_(\w+)\b/g,'vd_geomprop_$1').replace(/^(\s*)in\s+(\w+)\s+vd_geomprop_(\w+)\s*;/gm,(full,indent,type,name)=>`${indent}${isIntGeompropType(type)?'flat ':''}in ${type} vd_geomprop_${name};`);return{vs:patchedVs,fs:patchedFs};};// Finds CALL sites of fnName in GLSL source text (not its definition:
...
const findGlslCalls=(text,fnName)=>{const calls=[];const idRe=new RegExp('\\b'+fnName+'\\s*\\(','g');let m;while((m=idRe.exec(text))!==null){const before=text.slice(Math.max(0,m.index-8),m.index);if(/\bvoid\s*$/.test(before))continue;// "void fnName(" is the definition
const openParenIdx=m.index+m[0].length-1;let depth=1,i=openParenIdx+1;while(i<text.length&&depth>0){if(text[i]==='(')depth++;else if(text[i]===')')depth--;i++;}if(depth!==0)continue;// unbalanced, bail out defensively
const closeParenIdx=i-1;let j=i;while(j<text.length&&/\s/.test(text[j]))j++;if(text[j]!==';')continue;// followed by "{" => a definition, skip
calls.push({argsText:text.slice(openParenIdx+1,closeParenIdx),start:m.index,end:j+1});}return calls;};// Splits a GLSL call's argument list on top-level commas only (args can
// themselves be calls, e.g. "vec2(0.000000, 0.000000)").
const splitGlslArgs=argsText=>{const out=[];let depth=0,start=0;for(let i=0;i<argsText.length;i++){const c=argsText[i];if(c==='(')depth++;else if(c===')')depth--;else if(c===','&&depth===0){out.push(argsText.slice(start,i).trim());start=i+1;}}out.push(argsText.slice(start).trim());return out;};// Generic version of findGlslCalls: finds every CALL statement (any
...
const findAllCallStatements=text=>{const calls=[];const idRe=/\b(\w+)\s*\(/g;let m;while((m=idRe.exec(text))!==null){const before=text.slice(Math.max(0,m.index-8),m.index);if(/\bvoid\s*$/.test(before))continue;// definition, not a call
const openParenIdx=m.index+m[0].length-1;let depth=1,i=openParenIdx+1;while(i<text.length&&depth>0){if(text[i]==='(')depth++;else if(text[i]===')')depth--;i++;}if(depth!==0)continue;const closeParenIdx=i-1;let j=i;while(j<text.length&&/\s/.test(text[j]))j++;if(text[j]!==';')continue;calls.push({fnName:m[1],argsText:text.slice(openParenIdx+1,closeParenIdx),start:m.index,end:j+1});}return calls;};// Finds every "void NAME(params) { ... }" definition in GLSL source text
...
const findFunctionDefs=text=>{const defs=[];const re=/\bvoid\s+(\w+)\s*\(/g;let m;while((m=re.exec(text))!==null){const openParenIdx=m.index+m[0].length-1;let depth=1,i=openParenIdx+1;while(i<text.length&&depth>0){if(text[i]==='(')depth++;else if(text[i]===')')depth--;i++;}if(depth!==0)continue;const paramsText=text.slice(openParenIdx+1,i-1);let j=i;while(j<text.length&&/\s/.test(text[j]))j++;if(text[j]!=='{')continue;// not immediately followed by a body: not a definition we can scope
let bodyDepth=1,k=j+1;while(k<text.length&&bodyDepth>0){if(text[k]==='{')bodyDepth++;else if(text[k]==='}')bodyDepth--;k++;}if(bodyDepth!==0)continue;const params=splitGlslArgs(paramsText).filter(p=>p.length).map(p=>{const parts=p.trim().split(/\s+/);return{name:parts[parts.length-1],type:parts.slice(0,-1).join(' ')};});defs.push({name:m[1],params,bodyStart:j+1,bodyEnd:k-1});}return defs;};const findEnclosingFunction=(pos,funcDefs)=>funcDefs.find(f=>pos>=f.bodyStart&&pos<=f.bodyEnd);// uv_scale/uv_offset arrive as UNIFORM NAMES in generated GLSL (MaterialX
...
const isIdentityVec2Arg=(expr,introspected,x,y)=>{const lit=expr.match(/^vec2\(\s*([-\d.eE]+)\s*,\s*([-\d.eE]+)\s*\)$/);if(lit)return Math.abs(parseFloat(lit[1])-x)<1e-4&&Math.abs(parseFloat(lit[2])-y)<1e-4;const u=introspected&&introspected.find(e=>e.name===expr&&e.type==='vector2');if(!u||!Array.isArray(u.data)||u.data.length<2)return false;return Math.abs(u.data[0]-x)<1e-4&&Math.abs(u.data[1]-y)<1e-4;};const HEIGHTTONORMAL_TEXEL_FN=`
...
const CHANNEL_EXTRACT_FN_RE=/^(NG_separate[234]_|NG_extract_|mx_extract_)/;// Traces `varName` (used as the H argument of a heighttonormal call, or
...
const traceHeightSource=(varName,funcDef,fs,allFuncs,introspected,notices,depth)=>{if(depth>6)return null;// defensive cap, real chains are 2-3 deep
const body=fs.slice(funcDef.bodyStart,funcDef.bodyEnd+1);// 1) Direct sampler-backed result: mx_image_<type>(...) / mx_hextiledimage_<type>(...).
for(const call of findAllCallStatements(body)){const args=splitGlslArgs(call.argsText);const isImage=/^mx_image_(float|color3|color4|vector2|vector3|vector4)$/.test(call.fnName);const isHextiled=/^mx_hextiledimage_(color3|color4)$/.test(call.fnName);if(!isImage&&!isHextiled)continue;const resultVar=args[args.length-1];if(resultVar!==varName)continue;if(isImage){if(args.length<13)continue;const sampler=args[0],texcoord=args[3],uvScale=args[10],uvOffset=args[11];if(!isIdentityVec2Arg(uvScale,introspected,1,1)||!isIdentityVec2Arg(uvOffset,introspected,0,0)){notices.push(`heighttonormal: skipped a call site (non-identity uv_scale/uv_offset on "${resultVar}")`);return null;}return{kind:'image',sampler,texcoord};}return{kind:'hextiled',sampler:args[0]};}// 2) Simple alias / component extract: "[type] H = X;" / "H = X.c;" / "H = X[n];".
const aliasRe=new RegExp('(?:^|[;{}\\s])(?:\\w+\\s+)?'+varName+'\\s*=\\s*([A-Za-z_]\\w*)\\s*(?:\\.\\w+|\\[\\s*\\d+\\s*\\])?\\s*;');const aliasMatch=body.match(aliasRe);if(aliasMatch&&aliasMatch[1]!==varName){return traceHeightSource(aliasMatch[1],funcDef,fs,allFuncs,introspected,notices,depth+1);}// 3) separate3/extract-style calls: varName is one of the output args.
for(const call of findAllCallStatements(body)){if(!CHANNEL_EXTRACT_FN_RE.test(call.fnName))continue;const args=splitGlslArgs(call.argsText);if(args.length<2||!args.slice(1).includes(varName))continue;return traceHeightSource(args[0],funcDef,fs,allFuncs,introspected,notices,depth+1);}// 4) varName is a formal parameter of this function: follow its ONLY
...
const paramIdx=funcDef.params.findIndex(p=>p.name===varName);if(paramIdx===-1)return null;const callSites=findGlslCalls(fs,funcDef.name);if(callSites.length!==1)return null;const callerArgs=splitGlslArgs(callSites[0].argsText);const actualArg=callerArgs[paramIdx]&&callerArgs[paramIdx].trim();if(!actualArg||!/^\w+$/.test(actualArg))return null;const callerFunc=findEnclosingFunction(callSites[0].start,allFuncs);if(!callerFunc)return null;return traceHeightSource(actualArg,callerFunc,fs,allFuncs,introspected,notices,depth+1);};// Experimental opt-in (see HEIGHT_TO_NORMAL_TEXEL above): rewrites
...
const applyHeightToNormalTexel=(fs,notices,introspected)=>{if(!getHeightToNormalTexel())return fs;const allFuncs=findFunctionDefs(fs);const h2nCalls=findGlslCalls(fs,'mx_heighttonormal_vector3');if(!h2nCalls.length)return fs;let rewrittenImage=0,rewrittenHextile=0;// Rebuild right-to-left so earlier offsets stay valid as later (later
// in text order, processed first here) spans are replaced.
for(let k=h2nCalls.length-1;k>=0;k--){const call=h2nCalls[k];const args=splitGlslArgs(call.argsText);if(args.length!==4)continue;const[heightVar,scale,texcoord,outVar]=args;const enclosing=findEnclosingFunction(call.start,allFuncs);if(!enclosing)continue;const src=traceHeightSource(heightVar,enclosing,fs,allFuncs,introspected,notices,0);if(!src)continue;let replacement;if(src.kind==='image'){replacement=`mx_heighttonormal_vector3_texel(${src.sampler}, ${src.texcoord}, ${scale}, ${outVar});`;rewrittenImage++;}else{replacement=`mx_heighttonormal_vector3_hextile_texel(${heightVar}, ${src.sampler}, ${texcoord}, ${scale}, ${outVar});`;rewrittenHextile++;}fs=fs.slice(0,call.start)+replacement+fs.slice(call.end);}const rewritten=rewrittenImage+rewrittenHextile;if(rewritten>0){// Insert after any leading "precision ...;" directives, not at
// position 0: ESSL requires those before the first float/int/
// sampler use, and our injected functions have all three.
let insertAt=0;const precisionRe=/^precision\s+\w+\s+\w+\s*;\s*$/gm;let pm;while((pm=precisionRe.exec(fs))!==null)insertAt=pm.index+pm[0].length;let injected='';if(rewrittenImage)injected+=HEIGHTTONORMAL_TEXEL_FN;if(rewrittenHextile)injected+=HEIGHTTONORMAL_HEXTILE_TEXEL_FN;fs=fs.slice(0,insertAt)+'\n'+injected+fs.slice(insertAt);notices.push(`heighttonormal: ${rewritten} call(s) use texel-space gradients (experimental)`);}return fs;};// Hair helper pbrlib nodes pull in the full BSDF/lighting include chain,
...
const patchUnlitLightingRefs=src=>{const referencedNotDefined=name=>new RegExp('\\b'+name+'\\s*\\(').test(src)&&!new RegExp('vec3\\s+'+name+'\\s*\\(').test(src);const needsIrr=referencedNotDefined('mx_environment_irradiance');const needsRad=referencedNotDefined('mx_environment_radiance');const needsTrans=referencedNotDefined('mx_surface_transmission');if(needsIrr||needsRad||needsTrans){let simple='';// no dependencies, can go at the very top
let fresnel='';// needs the FresnelData struct
if(needsIrr)simple+='vec3 mx_environment_irradiance(vec3 N) { return vec3(0.0); }\n';if(needsRad)fresnel+='vec3 mx_environment_radiance(vec3 N, vec3 V, vec3 X, vec2 alpha, int distribution, FresnelData fd) { return vec3(0.0); }\n';if(needsTrans)fresnel+='vec3 mx_surface_transmission(vec3 N, vec3 V, vec3 X, vec2 alpha, int distribution, FresnelData fd, vec3 tint) { return vec3(0.0); }\n';const header='\n// [mtlx-engine] no-op lighting stubs for an unlit shader (see patchUnlitLightingRefs)\n';if(simple)src=header+simple+src;if(fresnel){const structIdx=src.indexOf('struct FresnelData');const insertAt=structIdx!==-1?src.indexOf('};',structIdx)+2:-1;if(insertAt>1){src=src.slice(0,insertAt)+header+fresnel+src.slice(insertAt);}else{// A silent skip here would leave mx_environment_radiance/
// mx_surface_transmission called but never stubbed, failing
// later with a cryptic GLSL error, throw instead, loudly.
throw new Error('patchUnlitLightingRefs: could not locate the "struct FresnelData" anchor (or its closing "};") in generated fragment shader, MaterialX output format may have changed');}}}// Last prepend so the define stays the very first line of the source.
if(/\bDIRECTIONAL_ALBEDO_METHOD\b/.test(src)&&!/#define\s+DIRECTIONAL_ALBEDO_METHOD\b/.test(src)){src='#define DIRECTIONAL_ALBEDO_METHOD 0\n'+src;}return src;};// The shared MaterialX light library keeps a +1 scene-unit offset in point
...
const patchScenePhysicalLightFalloff=(src,sceneRgbt=false)=>{if(!sceneRgbt)return src;return src.replace(/pow\s*\(\s*distance\s*\+\s*1\.0\s*,\s*light\.decay_rate\s*\+\s*M_FLOAT_EPS\s*\)/g,'pow(max(distance, M_FLOAT_EPS), light.decay_rate)');};// Shared display-transform GLSL body (`inVar`/`outVar`: vec3 in/out).
...
const DISPLAY_TRANSFORM_SWITCH_GLSL=(inVar,outVar,modeVar,exposureVar)=>{const p='        ';return p+'vec3 _c = max('+inVar+', vec3(0.0));\n'+(exposureVar?p+'_c *= '+exposureVar+';\n':'')+p+'if ('+modeVar+' == 1) {\n'+TONE_CURVE_GLSL('aces',p+'    ')+p+'} else if ('+modeVar+' == 3) {\n'+TONE_CURVE_GLSL('neutral',p+'    ')+p+'}\n'+// lin_rec709 (2) is the inspection mode: no tone map, no OETF and no
// clamp, so over-range values survive a float readback.
p+'vec3 '+outVar+' = _c;\n'+p+'if ('+modeVar+' != 2) {\n'+p+'    _c = clamp(_c, vec3(0.0), vec3(1.0)); // saturate()\n'+p+'    vec3 _lo = _c * 12.92;\n'+p+'    vec3 _hi = 1.055 * pow(_c, vec3(1.0 / 2.4)) - 0.055;\n'+p+'    '+outVar+' = mix(_hi, _lo, step(_c, vec3(0.0031308)));\n'+p+'}\n';};const ACES_SRGB_GLSL=(inVar,outVar,mode,exposureVar)=>{// Camera exposure, ahead of the tone curve. It belongs here and nowhere
...
const exposeBody=exposureVar?'        _c *= '+exposureVar+';\n':'';const head='        vec3 _c = max('+inVar+', vec3(0.0));\n'+exposeBody;// lin_rec709 is the inspection mode: no tone map, no OETF and no clamp, so
...
let BASE_TONEMAP_CHUNK=null;const CUSTOM_TONEMAP_STUB='vec3 CustomToneMapping( vec3 color ) { return color; }';const applyThreeToneMappingChunk=mode=>{if(!THREE.ShaderChunk||!THREE.ShaderChunk.tonemapping_pars_fragment)return false;if(BASE_TONEMAP_CHUNK===null)BASE_TONEMAP_CHUNK=THREE.ShaderChunk.tonemapping_pars_fragment;if(BASE_TONEMAP_CHUNK.indexOf(CUSTOM_TONEMAP_STUB)===-1)return false;const body=mode==='lin_rec709'?'\treturn color * toneMappingExposure;\n':'\tvec3 _c = max(color * toneMappingExposure, vec3(0.0));\n'+TONE_CURVE_GLSL(mode,'\t')+'\treturn clamp(_c, vec3(0.0), vec3(1.0));\n';THREE.ShaderChunk.tonemapping_pars_fragment=BASE_TONEMAP_CHUNK.replace(CUSTOM_TONEMAP_STUB,'vec3 CustomToneMapping( vec3 color ) {\n'+body+'}');return true;};// Injects the current display transform (getDisplayTransform(), see
...
const encodeDisplay=src=>{// Both anchors are load-bearing: a silent skip here used to ship
...
const m=src.match(/\bout\s+vec4\s+(\w+)\s*;/);if(!m)throw new Error('encodeDisplay: could not locate the fragment shader\'s "out vec4 <name>;" declaration, MaterialX output format may have changed');const v=m[1];// Both the curve and the exposure ride along as uniforms, so neither one
// regenerates a shader when it changes and each view can hold its own.
let out=src;const decl='uniform float u_displayExposure;\nuniform int u_displayTransform;';if(out.indexOf('uniform int u_displayTransform;')===-1){const mainIdx=out.indexOf('void main');if(mainIdx===-1)throw new Error('encodeDisplay: could not locate "void main" to declare the display uniforms, MaterialX output format may have changed');out=out.slice(0,mainIdx)+decl+'\n'+out.slice(mainIdx);}const idx=out.lastIndexOf('}');if(idx===-1)throw new Error('encodeDisplay: could not locate a closing "}" (expected main()\'s closing brace) in generated fragment shader, MaterialX output format may have changed');const inject='\n    // Injected by previewer: display transform (see encodeDisplay()\'s header comment), then sRGB.\n'+'    if (u_peelLinear == 0) {\n'+DISPLAY_TRANSFORM_SWITCH_GLSL(v+'.rgb','_enc','u_displayTransform','u_displayExposure')+'        '+v+' = vec4(_enc, '+v+'.a);\n'+'    }\n';return out.slice(0,idx)+inject+out.slice(idx);};// Deliberate energy-compromise constant for the peel-mode env-refraction
...
const PEEL_REFRACTION_SCALE=0.5;// Bounds-guards MaterialX's mx_shadow_occlusion. The library samples the
...
// untouched.
const patchShadowBounds=fs=>{const call='mx_variance_shadow_occlusion(shadowMoments, shadowCoord.z)';const anchor='return  '+call;if(fs.indexOf(anchor)===-1)return fs;const guard=['if (shadowCoord4.w <= 0.0) return 1.0;','if (any(lessThan(shadowCoord, vec3(0.0))) || any(greaterThan(shadowCoord, vec3(1.0)))) return 1.0;',// A light inside the scene cannot be covered by one 2D map, so the
...
'vec2 mx_shadowEdge = min(shadowCoord.xy, vec2(1.0) - shadowCoord.xy);','float mx_shadowFade = smoothstep(0.0, 0.12, min(mx_shadowEdge.x, mx_shadowEdge.y));',// Variance shadow maps leak: Chebyshev's bound is only an upper bound,
...
const SHADOW_FACE_SLOTS=32;const SHADOW_LIGHT_SLOTS_MAX=32;// One bias policy for every caster kind: a normal offset off the surface
...
const SHADOW_NORMAL_OFFSET_TEXELS=1.0;const SHADOW_DEPTH_BIAS_TEXELS=1.0;const patchShadowLightScope=(fs,{skipTransmittance=false}={})=>{const call='occlusion = mx_shadow_occlusion(u_shadowMap, u_shadowMatrix, positionWorld);';const site='L = lightShader.direction;';if(fs.indexOf(call)===-1||fs.indexOf(site)===-1)return fs;// The geometric normal, when the surface exposes one. Used to push the
...
+(skipTransmittance?'':'\n                vec3 mx_transmit = vec3(1.0);'+'\n                if (mx_face < 0) {'+'\n                    mx_transmit = vec3(1.0);'+'\n                } else if (mx_shadowTransmit[mx_face].x < -0.5) {'+'\n                    mx_shadowTransmit[mx_face] = mx_shadow_transmittance(mx_face, positionWorld, mx_shadowNormal);'+'\n                    mx_transmit = mx_shadowTransmit[mx_face];'+'\n                } else {'+'\n                    mx_transmit = mx_shadowTransmit[mx_face];'+'\n                }')+'\n                lightShader.intensity *= '+(skipTransmittance?'':'mx_transmit * ')+'u_shadowDiagnosticVisibilityScale;'+'\n            }');if(out.indexOf('uniform sampler2D u_shadowAtlas;')!==-1)return out;const decl=['uniform sampler2D u_shadowAtlas;','uniform mat4 u_shadowMatrices['+SHADOW_FACE_SLOTS+'];',// xy = tile origin in atlas UV, zw = tile size.
'uniform vec4 u_shadowTiles['+SHADOW_FACE_SLOTS+'];',// Normalized positive light-view Z plane for linear moments.
'uniform vec4 u_shadowDepthPlanes['+SHADOW_FACE_SLOTS+'];',// x = near, y = far - near, in the same world units used by the
// authored emitter radius. Kept separate because the normalized
// plane alone cannot recover its near offset.
'uniform vec2 u_shadowDepthRanges['+SHADOW_FACE_SLOTS+'];',// x/y = authored source radius in world units; z/w = explicit
// perspective projection scale. Directional casters use all zeroes.
'uniform vec4 u_shadowSourceRadii['+SHADOW_FACE_SLOTS+'];',// World-space size of one atlas texel at the caster's near plane
// (perspective) or across the whole frustum (orthographic). Feeds
// both the normal-offset and the depth bias below; zero disables
// both for that slot.
'uniform float u_shadowTexelWorldSize['+SHADOW_FACE_SLOTS+'];',// Light position for an omni caster's face, used to pick which of
// its six faces a shaded point falls into. Unused (zero) otherwise.
'uniform vec3 u_shadowFaceOrigin['+SHADOW_FACE_SLOTS+'];',// 1.0 where a face actually holds rendered data, 0.0 where the
// renderer reserved the slot but never allocated a cell for it.
'uniform float u_shadowFaceValid['+SHADOW_FACE_SLOTS+'];',// A cube group's own world +X/+Y/+Z, read only at the group's base
// face index: world axes for omni, the emitter's own frame for area.
'uniform vec3 u_shadowFaceBasisX['+SHADOW_FACE_SLOTS+'];','uniform vec3 u_shadowFaceBasisY['+SHADOW_FACE_SLOTS+'];','uniform vec3 u_shadowFaceBasisZ['+SHADOW_FACE_SLOTS+'];',// Per light slot: base face index (or -1 for none) and how many
// consecutive faces it spans (1 for directional, 6 for a cube group).
'uniform int u_shadowSlotFace['+SHADOW_LIGHT_SLOTS_MAX+'];','uniform int u_shadowSlotFaceCount['+SHADOW_LIGHT_SLOTS_MAX+'];',// Test-only multiplier for direct incoming visibility. Normal
...
'uniform float u_shadowDiagnosticVisibilityScale;'].concat(skipTransmittance?[]:[// Packed into one texture: R1 (nearest transmitter) cell origin/size,
...
'uniform sampler2D u_shadowTransmittance;','uniform vec4 u_shadowRecordCells['+SHADOW_FACE_SLOTS+'];']).concat(['#define SHADOW_NORMAL_OFFSET_TEXELS '+SHADOW_NORMAL_OFFSET_TEXELS.toFixed(4),'#define SHADOW_DEPTH_BIAS_TEXELS '+SHADOW_DEPTH_BIAS_TEXELS.toFixed(4),'float mx_shadow_vsm(vec2 moments, float receiverDepth) {','    float p = (receiverDepth <= moments.x) ? 1.0 : 0.0;','    float variance = max(moments.y - moments.x * moments.x, 2e-7);','    float d = receiverDepth - moments.x;','    float lit = max(p, variance / (variance + d * d));','    return smoothstep(0.3, 1.0, lit);','}','float mx_shadow_atlas(int caster, vec3 P, vec3 Ng) {','    if (caster < 0) return 1.0;','    vec4 depthPlane = u_shadowDepthPlanes[caster];','    vec2 depthRange = u_shadowDepthRanges[caster];','    float nearDepth = depthRange.x;','    float depthSpan = max(depthRange.y, 1e-9);','    vec2 projectionScale = u_shadowSourceRadii[caster].zw;','    bool perspective = projectionScale.x > 0.0 || projectionScale.y > 0.0;','    vec2 sourceRadius = u_shadowSourceRadii[caster].xy;',// Normal offset AND depth bias share one texel-world estimate,
// scaled by distance for a perspective caster (using the raw,
// unbiased point). Small enough now to apply to every caster kind.
'    float texelBase = u_shadowTexelWorldSize[caster];','    float rawDepth = dot(vec4(P, 1.0), depthPlane);','    float rawZ = max(nearDepth + depthSpan * rawDepth, 0.0);','    float texelWorld = texelBase * (perspective ? rawZ : 1.0);','    vec3 offsetP = P + Ng * (texelWorld * SHADOW_NORMAL_OFFSET_TEXELS);','    vec4 c4 = u_shadowMatrices[caster] * vec4(offsetP, 1.0);','    if (c4.w <= 0.0) return 1.0;','    vec3 sc = c4.xyz / c4.w;','    sc = sc * 0.5 + 0.5;','    if (any(lessThan(sc, vec3(0.0))) || any(greaterThan(sc, vec3(1.0)))) return 1.0;','    vec4 tile = u_shadowTiles[caster];','    vec2 atlasTexel = 1.0 / vec2(textureSize(u_shadowAtlas, 0));','    vec2 tileTexel = atlasTexel / max(tile.zw, vec2(1e-6));',// Clamp the local tile coordinate by half a texel before mapping into
...
'    vec2 localUv = clamp(sc.xy, tileTexel * 0.5, vec2(1.0) - tileTexel * 0.5);','    vec2 centerUv = tile.xy + localUv * tile.zw;','    vec2 moments = texture(u_shadowAtlas, centerUv).xy;',// Projected XY comes from the real light camera; moments use a
...
'    float receiverDepth = dot(vec4(offsetP, 1.0), depthPlane);','    receiverDepth -= SHADOW_DEPTH_BIAS_TEXELS * texelWorld / depthSpan;','    float receiverZ = nearDepth + depthSpan * receiverDepth;',// A source with zero extent is a point/directional caster: retain the
...
'    float lit = mx_shadow_vsm(moments, receiverDepth);','    if (sourceRadius.x <= 0.0 && sourceRadius.y <= 0.0) {','        vec2 e0 = min(sc.xy, vec2(1.0) - sc.xy);','        return mix(1.0, lit, smoothstep(0.0, 0.04, min(e0.x, e0.y)));','    }','    vec2 searchRadius = sourceRadius * projectionScale * 0.5 / max(nearDepth, 1e-9)','        * max(receiverZ - nearDepth, 0.0) / max(receiverZ, 1e-6);',// The PCSS bound is a constant in tile space (two texels of a 1024
// tile) so a 512 cell filters the same world footprint as a full tile
// rather than twice as wide.
'    vec2 pcssMaxRadius = vec2(2.0 / 1024.0);','    searchRadius = min(searchRadius, pcssMaxRadius);',// Estimate a blocker over a source-size footprint. The search and
// filter stay inside this caster tile, so atlas slots cannot bleed.
'    float blockerSum = 0.0;','    float blockerCount = 0.0;','    for (int oy = -1; oy <= 1; oy++) {','        for (int ox = -1; ox <= 1; ox++) {','            vec2 local = clamp(sc.xy + vec2(float(ox), float(oy)) * searchRadius, tileTexel * 0.5, vec2(1.0) - tileTexel * 0.5);','            vec2 sm = texture(u_shadowAtlas, tile.xy + local * tile.zw).xy;','            if (sm.x < receiverDepth) { blockerSum += sm.x; blockerCount += 1.0; }','        }','    }','    float blockerDepth = blockerCount > 0.0 ? blockerSum / blockerCount : moments.x;','    float blockerZ = nearDepth + depthSpan * blockerDepth;','    vec2 filterRadius = sourceRadius * projectionScale * 0.5','        * max(receiverZ - blockerZ, 0.0) / max(blockerZ, 1e-6)','        / max(receiverZ, 1e-6);','    filterRadius = min(filterRadius, pcssMaxRadius);','    float filtered = 0.0;','    for (int oy = -1; oy <= 1; oy++) {','        for (int ox = -1; ox <= 1; ox++) {','            vec2 local = clamp(sc.xy + vec2(float(ox), float(oy)) * filterRadius, tileTexel * 0.5, vec2(1.0) - tileTexel * 0.5);','            vec2 sm = texture(u_shadowAtlas, tile.xy + local * tile.zw).xy;',// Apply VSM bleed reduction per tap before averaging; reducing only
// after the average turns a partially visible area source nearly black.
'            filtered += mx_shadow_vsm(sm, receiverDepth);','        }','    }','    lit = filtered / 9.0;',// One 2D map cannot cover a light inside the room, so the frustum ends
...
const firstFn=out.search(/^(?:void|vec[234]|float|int|bool|mat[234])\s+\w+\s*\(/m);const at=firstFn!==-1?firstFn:out.indexOf('void main');if(at===-1)return fs;return out.slice(0,at)+decl+out.slice(at);};// Adds the source shape to MaterialX's generated LightData struct. Area lights
...
const patchLightSourceKindStruct=fs=>{const match=fs.match(/struct\s+LightData\s*\{[\s\S]*?\n\};/);if(!match||/\bsourceKind\b/.test(match[0]))return fs;const struct=match[0].replace(/\n\};$/,'\n    int sourceKind;\n};');return fs.slice(0,match.index)+struct+fs.slice(match.index+match[0].length);};// Applies finite planar emitter geometry to the point/spot quadrature used
...
const patchAreaLightSourceCosine=fs=>{const site='L = lightShader.direction;';if(fs.indexOf(site)===-1||fs.indexOf('u_lightData')===-1||fs.indexOf('sourceKind')===-1)return fs;let out=fs.replace(site,site+'\n            if (u_lightData[activeLightIndex].sourceKind == '+LIGHT_SOURCE_KIND_AREA+') {'+'\n                lightShader.intensity *= max(dot(u_lightData[activeLightIndex].direction, -L), 0.0);'+'\n            }');return out;};// Geometric specular anti-aliasing. standard_surface's generated GLSL
...
const patchSpecularAA=(fs,{specularAA=false}={})=>{if(!specularAA)return fs;if(fs.indexOf('mx_specular_aa_widen')!==-1)return fs;const anchor=/mx_roughness_anisotropy\(coat_affected_roughness_out,\s*specular_anisotropy,\s*main_roughness_out\);/;if(!anchor.test(fs))return fs;let out=fs.replace(anchor,m=>m+'\n    main_roughness_out = mx_specular_aa_widen(main_roughness_out, normal, coat_affected_roughness_out);');const decl=[// sigma2 in variance space: normal-slope variance plus roughness-
...
'const float MX_SPECULAR_AA_NORMAL_SCALE = 0.002;','const float MX_SPECULAR_AA_ROUGHNESS_SCALE = 0.005;','const float MX_SPECULAR_AA_THRESHOLD = 0.01;','vec2 mx_specular_aa_widen(vec2 alpha, vec3 aaNormal, float aaRoughness) {','    vec3 dNx = dFdx(aaNormal);','    vec3 dNy = dFdy(aaNormal);','    float dRx = dFdx(aaRoughness);','    float dRy = dFdy(aaRoughness);','    float normalVariance = dot(dNx, dNx) + dot(dNy, dNy);','    float roughnessVariance = dRx * dRx + dRy * dRy;','    float kernelRoughness = min(MX_SPECULAR_AA_NORMAL_SCALE * normalVariance + MX_SPECULAR_AA_ROUGHNESS_SCALE * roughnessVariance, MX_SPECULAR_AA_THRESHOLD);','    vec2 alpha2 = clamp(alpha * alpha + vec2(kernelRoughness), 0.0, 1.0);','    return sqrt(alpha2);','}',''].join('\n');// Same "before the first function definition" anchor as the other
// fragment patches: the call site lives inside a function that
// precedes main(), so the helper must be declared earlier still.
const firstFn=out.search(/^(?:void|vec[234]|float|int|bool|mat[234])\s+\w+\s*\(/m);const at=firstFn!==-1?firstFn:out.indexOf('void main(');if(at===-1)return fs;return out.slice(0,at)+decl+out.slice(at);};// Feeds a screen-space ambient occlusion factor into the slot MaterialX
...
const firstFn=src.search(/^(?:void|vec[234]|float|int|bool|mat[234])\s+\w+\s*\(/m);const at=firstFn!==-1?firstFn:src.indexOf('void main(');if(at===-1)return src;return src.slice(0,at)+decl+src.slice(at);};const patchAmbientOcclusion=(fs,{skipSkyVis=false,skipAoVolume=false}={})=>{const anchor=/(\/\/ Ambient occlusion\s*\n\s*)occlusion = 1\.0;/;if(!anchor.test(fs))return fs;// Sky visibility needs the world position; without that varying only the
...
const hasWorldPos=!skipSkyVis&&/\bin\s+vec3\s+positionWorld\s*;/.test(fs)&&/\bin\s+vec3\s+normalWorld\s*;/.test(fs);// The baked occlusion VOLUME (js/usd-scene-skyvis.js's AO bake) is a
// separate sampler-budget drop from sky visibility; skipAoVolume falls
// back to the sky-times-ssao combination without it.
const useVolume=hasWorldPos&&!skipAoVolume;// Mirrors the computed occlusion into a file-scope float (currently
...
const assignment=!hasWorldPos?'occlusion = mx_ssao_occlusion(); mx_envOcclusionValue = occlusion;':useVolume?'occlusion = mx_sky_visibility() * min(mx_volume_occlusion(), mx_ssao_occlusion()); mx_envOcclusionValue = occlusion;':'occlusion = mx_ssao_occlusion() * mx_sky_visibility(); mx_envOcclusionValue = occlusion;';let out=fs.replace(anchor,'$1'+assignment);out=ensureEnvOcclusionGlobal(out);const skyDecls=!hasWorldPos?[]:[// Baked coarse visibility of the environment (js/usd-scene-skyvis.js).
...
'uniform highp sampler3D u_skyVisMap;','uniform vec3 u_skyVisMin;','uniform vec3 u_skyVisSize;',// Sampled one cell along the normal, into the free space the surface
// faces, rather than at the surface itself where the cell is half solid.
'uniform float u_skyVisCell;','uniform float u_skyVisStrength;','float mx_sky_visibility() {','    if (u_skyVisStrength <= 0.0) return 1.0;',// MaterialX's generated closures face-forward their shading normal,
...
'    vec3 skyNormal = normalize(normalWorld);','    if (!gl_FrontFacing) skyNormal = -skyNormal;',// A surface on the stage boundary can sit on the near face of its
...
'    vec3 uvw = (positionWorld + skyNormal * (1.5 * u_skyVisCell) - u_skyVisMin) / max(u_skyVisSize, vec3(1e-6));','    if (any(lessThan(uvw, vec3(0.0))) || any(greaterThan(uvw, vec3(1.0)))) return 1.0;',// The bake stores the sky visibility function's first order moments:
...
'    vec4 moments = texture(u_skyVisMap, uvw);','    float visibilityMean = moments.r;','    vec3 visibilityDirection = (moments.gba * 255.0 - vec3(128.0)) / 127.0;','    float vis = clamp(visibilityMean + dot(visibilityDirection, skyNormal), 0.0, 1.0);','    return mix(1.0, vis, clamp(u_skyVisStrength, 0.0, 1.0));','}'];// A copy of mx_sky_visibility over its own baked volume (see
...
const volumeDecls=!useVolume?[]:['uniform highp sampler3D u_aoVolumeMap;','uniform vec3 u_aoVolumeMin;','uniform vec3 u_aoVolumeSize;','uniform float u_aoVolumeCell;','uniform float u_aoVolumeStrength;','float mx_volume_occlusion() {','    if (u_aoVolumeStrength <= 0.0) return 1.0;','    vec3 skyNormal = normalize(normalWorld);','    if (!gl_FrontFacing) skyNormal = -skyNormal;','    vec3 uvw = (positionWorld + skyNormal * (1.5 * u_aoVolumeCell) - u_aoVolumeMin) / max(u_aoVolumeSize, vec3(1e-6));','    if (any(lessThan(uvw, vec3(0.0))) || any(greaterThan(uvw, vec3(1.0)))) return 1.0;','    vec4 moments = texture(u_aoVolumeMap, uvw);','    float visibilityMean = moments.r;','    vec3 visibilityDirection = (moments.gba * 255.0 - vec3(128.0)) / 127.0;','    float vis = clamp(visibilityMean + dot(visibilityDirection, skyNormal), 0.0, 1.0);','    return mix(1.0, vis, clamp(u_aoVolumeStrength, 0.0, 1.0));','}'];const decls=['uniform sampler2D u_ssaoMap;','uniform vec2 u_ssaoTexel;','uniform float u_ssaoStrength;',// .r is the raw AO factor, .g the guard's per-pixel confidence (screen
...
'float mx_ssao_occlusion() {','    vec2 aoSample = texture(u_ssaoMap, gl_FragCoord.xy * u_ssaoTexel).rg;','    return mix(1.0, clamp(aoSample.r, 0.0, 1.0), clamp(u_ssaoStrength, 0.0, 1.0) * clamp(aoSample.g, 0.0, 1.0));','}'].concat(skyDecls).concat(volumeDecls).concat(['']).join('\n');// Must land before the FIRST function definition, not before main():
...
const firstFn=out.search(/^(?:void|vec[234]|float|int|bool|mat[234])\s+\w+\s*\(/m);const at=firstFn!==-1?firstFn:out.indexOf('void main(');if(at===-1)return fs;// nothing recognisable: leave the shader untouched
return out.slice(0,at)+decls+out.slice(at);};// Diffuse bounce v3: a REAL one-bounce irradiance estimate, baked per cell
...
const patchDiffuseBounceAdd=(fs,{skipSkyVis=false,skipBounce=false}={})=>{// No trailing `;` in the capture: the addition must land INSIDE this
...
const anchor=/shader_constructor_out\.color \+= occlusion \* (\w+)\.response;/;const match=fs.match(anchor);if(!match)return fs;const hasWorldPos=!skipSkyVis&&!skipBounce&&/\bin\s+vec3\s+positionWorld\s*;/.test(fs)&&/\bin\s+vec3\s+normalWorld\s*;/.test(fs);if(!hasWorldPos)return fs;// Needs the surface's own diffuse albedo in scope at the injection
...
if(!/\bvec3\s+base_color_nonnegative_out\b/.test(fs))return fs;// base_color_nonnegative_out is a LOCAL variable inside the function
...
const out=fs.replace(anchor,'shader_constructor_out.color += occlusion * $1.response + mx_diffuse_bounce_add(base_color_nonnegative_out);');// patchAmbientOcclusion runs before this function (see the call site),
...
// missing.
const hasVolumeOcclusion=/float\s+mx_volume_occlusion\s*\(\s*\)\s*\{/.test(out);const hasSsaoOcclusion=/float\s+mx_ssao_occlusion\s*\(\s*\)\s*\{/.test(out);const decls=[// Baked one-bounce irradiance volume (js/usd-scene-skyvis.js's
// bakeBounceGeometry/shadeBounce), scalar SH1 encoded exactly like
// mx_sky_visibility's moments.
'uniform highp sampler3D u_skyBounceMap;','uniform vec3 u_skyBounceMin;','uniform vec3 u_skyBounceSize;','uniform float u_skyBounceCell;','uniform float u_skyBounceStrength;',// The bake's own maximum reconstructed value (denormalizes the
...
'uniform float u_bounceScale;','uniform vec3 u_bounceTint;','vec3 mx_diffuse_bounce_add(vec3 albedo) {','    if (u_skyBounceStrength <= 0.0 || u_bounceScale <= 0.0) return vec3(0.0);','    vec3 n = normalize(normalWorld);','    if (!gl_FrontFacing) n = -n;','    vec3 uvw = (positionWorld + n * (1.5 * u_skyBounceCell) - u_skyBounceMin) / max(u_skyBounceSize, vec3(1e-6));','    if (any(lessThan(uvw, vec3(0.0))) || any(greaterThan(uvw, vec3(1.0)))) return vec3(0.0);','    vec4 m = texture(u_skyBounceMap, uvw);','    vec3 d = (m.gba * 255.0 - vec3(128.0)) / 127.0;','    float e = u_bounceScale * clamp(m.r + dot(d, n), 0.0, 1.0);','    float near = '+(hasVolumeOcclusion&&hasSsaoOcclusion?'min(mx_volume_occlusion(), mx_ssao_occlusion())':hasSsaoOcclusion?'mx_ssao_occlusion()':'1.0')+';','    return clamp(u_skyBounceStrength, 0.0, 1.0) * near * e * u_bounceTint * albedo;','}',''].join('\n');// Same "before the first function definition" anchor patchAmbientOcclusion
...
const firstFn=out.search(/^(?:void|vec[234]|float|int|bool|mat[234])\s+(?!mx_ssao_occlusion\b|mx_sky_visibility\b|mx_volume_occlusion\b)\w+\s*\(/m);const at=firstFn!==-1?firstFn:out.indexOf('void main(');if(at===-1)return fs;return out.slice(0,at)+decls+out.slice(at);};// Scene-only correction for the generated OpenPBR thin-wall contract.
...
const patchSceneThinWalledTransmission=(fs,enabled,notices)=>{if(!enabled||fs.indexOf('/* MX_SCENE_THIN_WALL */')!==-1)return fs;let count=0;const out=fs.replace(/void\s+NG_open_pbr_surface_surfaceshader\w*\s*\(([^)]*)\)\s*\{/g,(head,args)=>{if(!/\bbool\s+geometry_thin_walled\b/.test(args)||!/\bfloat\s+transmission_depth\b/.test(args)||!/\bvec3\s+transmission_color\b/.test(args))return head;count++;return head+'\n    /* MX_SCENE_THIN_WALL */\n'+'    if (geometry_thin_walled) transmission_depth = 0.0;\n';});if(!count&&/void\s+NG_open_pbr_surface_surfaceshader/.test(fs)&&notices){notices.push('Scene thin-wall correction unavailable: the generated OpenPBR function contract changed; keeping MaterialX output');}return out;};// Transport-only "transfer" record. Captures B/D/opacity/thin at the
...
const patchLightTransportPayload=(fs,notices,requestedMode)=>{if(fs.indexOf('/* MX_LIGHT_TRANSPORT */')!==-1)return fs;const isTransfer=requestedMode===4||requestedMode==='transfer'||requestedMode===true;if(!isTransfer){if(notices)notices.push('Light transport unavailable: only the transfer record is supported.');return fs;}if(!/\bin\s+vec3\s+positionWorld\s*;/.test(fs)){if(notices)notices.push('Light transport unavailable: generated shader has no world position varying.');return fs;}let seen=0;let terminal='';let out=fs.replace(/void\s+NG_open_pbr_surface_surfaceshader\w*\s*\(([^)]*)\)\s*\{/g,(head,args)=>{const terminalOut=args.match(/\bout\s+(\w+)\s+(\w+)\s*$/);if(!/\bfloat\s+transmission_weight\b/.test(args)||!/\bvec3\s+transmission_color\b/.test(args)||!/\bfloat\s+transmission_depth\b/.test(args)||!/\bfloat\s+geometry_opacity\b/.test(args)||!/\bbool\s+geometry_thin_walled\b/.test(args)||!terminalOut)return head;seen++;terminal=(head.match(/void\s+(\w+)\s*\(/)||[])[1]||'';return head+'\n    /* MX_LIGHT_TRANSPORT */\n'// Thin transfer owns the authored tint. Solid tint belongs only
...
+'    '+terminalOut[2]+' = '+terminalOut[1]+'('+Array(terminalOut[1]==='surfaceshader'?2:1).fill('vec3(0.0)').join(', ')+');\n'+'    /* MX_LIGHT_TRANSPORT_TERMINAL_RETURN */\n'+'    return;\n';});if(seen!==1){if(notices)notices.push('Light transport unavailable: ambiguous generated OpenPBR terminal');return fs;}const first=out.search(/^(?:void|vec[234]|float|int|bool|mat[234])\s+\w+\s*\(/m);const om=out.match(/\bout\s+vec4\s+(\w+)\s*;/);if(first<0||!om){if(notices)notices.push('Light transport unavailable: generated output contract changed');return fs;}const decl='vec3 mx_transportB=vec3(1.0), mx_transportD=vec3(0.0); float mx_transportOpacity=0.0, mx_transportThin=0.0;\n'// Record-only uniforms. u_recordDepthPlane matches the shadow atlas
// convention (dot(vec4(P,1), plane) -> normalized linear depth);
// u_recordUnitScale mirrors u_thicknessScale's scene-unit conversion.
+'uniform vec4 u_recordDepthPlane;\n'+'uniform sampler2D u_recordEntryDepth;\n'+'uniform sampler2D u_recordExitDepth;\n'+'uniform vec2 u_recordCellOrigin;\n'+'uniform int u_recordPass;\n'+'uniform vec2 u_recordTexel;\n'+'uniform float u_recordDepthSpan;\n'+'uniform float u_recordUnitScale;\n';out=out.slice(0,first)+decl+out.slice(first);const main=out.search(/\bvoid\s+main\s*\(\s*\)\s*\{/);const call=main<0||!terminal?null:out.slice(main).match(new RegExp('\\b'+terminal+'\\s*\\([\\s\\S]*?\\);'));if(!call){if(notices)notices.push('Light transport unavailable: generated terminal call contract changed');return fs;}const at=main+call.index+call[0].length;const mainOpen=out.indexOf('{',main);let depth=0,mainEnd=-1;for(let i=mainOpen;i>=0&&i<out.length;i++){if(out[i]==='{')depth++;else if(out[i]==='}'&&--depth===0){mainEnd=i;break;}}if(mainEnd<0){if(notices)notices.push('Light transport unavailable: generated main contract changed');return fs;}// Entry/exit depths come from the consumer's nearest/farthest prepasses
...
return out.slice(0,at)+'\n    /* MX_LIGHT_TRANSPORT_EARLY_RETURN MX_LIGHT_TRANSPORT_TAIL_PRUNED */'+transfer+out.slice(mainEnd);};// Gives MaterialX's volume absorption the path length it is missing.
//
...
const patchTransmissionThickness=(fs,{dropThicknessMap=false}={})=>{const anchor='vdf.throughput = exp(-absorption);';if(fs.indexOf(anchor)===-1)return fs;// Needs the standard HW varyings to locate the fragment along the ray.
const hasVars=dropThicknessMap||/\bin\s+vec3\s+positionWorld\s*;/.test(fs)&&/uniform\s+vec3\s+u_viewPosition\s*;/.test(fs);if(!hasVars)return fs;let out=fs.replace(anchor,'vdf.throughput = exp(-absorption * mx_transmission_path_length());');// Sampler-budget drop: no u_thicknessMap declared at all, always the
...
const decls=dropThicknessMap?['uniform float u_thicknessReferencePath;','float mx_transmission_path_length() {','    return max(u_thicknessReferencePath, 0.0);','}',''].join('\n'):['uniform sampler2D u_thicknessMap;','uniform vec2 u_thicknessTexel;',// The renderer's sceneRoot has converted positions to metres. The
...
const patchLocalEnvironmentRadiance=(fs,{skipLocalEnv=false,notices=null}={})=>{if(fs.indexOf('mx_local_env_mix')!==-1)return fs;// The generator's own prefilter body (mx_environment_prefilter.glsl),
// matched verbatim; FIS (no single `Li` assignment) and any shader
// shape the generator changes underneath us both safe-fail here.
const anchor='vec3 Li = mx_latlong_map_lookup(L, u_envMatrix, mx_latlong_alpha_to_lod(avgAlpha), u_envRadiance);';const anchorIdx=fs.indexOf(anchor);const usable=anchorIdx!==-1&&/\bin\s+vec3\s+positionWorld\s*;/.test(fs)&&/\bvec2\s+mx_latlong_projection\s*\(/.test(fs);if(skipLocalEnv||!usable){if(anchorIdx!==-1&&!skipLocalEnv&&notices){notices.push('local reflections: shader anchor unusable (missing positionWorld or mx_latlong_projection); reflections stay image based');}return fs;}let out=fs.slice(0,anchorIdx)+anchor+'\n    Li = mx_local_env_mix(Li, positionWorld, L, mx_latlong_alpha_to_lod(avgAlpha));'+fs.slice(anchorIdx+anchor.length);const declIfAbsent=line=>out.indexOf(line)===-1?line+'\n':'';const decls=declIfAbsent('uniform sampler2D u_localEnvRadiance;')+declIfAbsent('uniform float u_localEnvMips;')+declIfAbsent('uniform float u_localEnvStrength;')+declIfAbsent('uniform vec3 u_localEnvProbe;')+declIfAbsent('uniform vec3 u_localEnvBoxMin;')+declIfAbsent('uniform vec3 u_localEnvBoxMax;')+declIfAbsent('uniform int u_localEnvParallax;');const helpers=// P is positionWorld, R is the same L the generated body already
...
const patchScreenSpaceReflection=(fs,{skipSsr=false,notices=null}={})=>{if(fs.indexOf('mx_environment_radiance_ibl')!==-1)return fs;if(skipSsr||!/\bin\s+vec3\s+positionWorld\s*;/.test(fs))return fs;const sig='vec3 mx_environment_radiance(vec3 N, vec3 V, vec3 X, vec2 alpha, int distribution, FresnelData fd)';const anchorRe=/vec3 mx_environment_radiance\(vec3 N, vec3 V, vec3 X, vec2 alpha, int distribution, FresnelData fd\)[ \t]*\r?\n[ \t]*\{/;const anchorMatch=anchorRe.exec(fs);if(!anchorMatch){const called=/mx_environment_radiance\(N,\s*V,\s*X,\s*\w+,\s*(?:distribution|\d+),\s*fd\)/.test(fs);const stubPresent=fs.indexOf(sig+' { return vec3(0.0); }')!==-1;if(called&&!stubPresent&&notices){notices.push('screen-space reflection: environment radiance anchor not found; reflections stay image based');}return fs;}const bodyStart=anchorMatch.index+anchorMatch[0].length;// Rename the definition to mx_environment_radiance_ibl (name only; the
// parameter list/body text is untouched).
let out=fs.slice(0,anchorMatch.index)+'vec3 mx_environment_radiance_ibl'+anchorMatch[0].slice('vec3 mx_environment_radiance'.length)+fs.slice(bodyStart);const declIfAbsent=line=>out.indexOf(line)===-1?line+'\n':'';const decls='uniform int u_ssrEnabled;\n'+'uniform float u_ssrStrength;\n'+'uniform float u_ssrMaxRoughness;\n'+'uniform mat4 u_historyViewProjectionMatrix;\n'+'uniform mat4 u_historyViewProjectionInverseMatrix;\n'+'uniform vec3 u_historyViewPosition;\n'+declIfAbsent('uniform sampler2D u_opaqueColor;')+declIfAbsent('uniform float u_opaqueColorLevels;')+declIfAbsent('uniform float u_sceneRadius;')+declIfAbsent('uniform highp sampler2D u_opaqueDepth;')+// File-scope trace result: written by ONE call to mx_ssr_trace per
...
const nvAnchorRe=/vec3 N = normalize\(normalWorld\);[ \t]*\r?\n[ \t]*vec3 V = normalize\(u_viewPosition - positionWorld\);/;const nvMatch=nvAnchorRe.exec(out);if(nvMatch){const at=nvMatch.index+nvMatch[0].length;out=out.slice(0,at)+'\n        if (u_ssrEnabled != 0) mx_ssr_trace(N, V);'+out.slice(at);}else if(notices){notices.push('screen-space reflection: shading normal/view anchor not found; reflections stay image based');}return out;};// Enables the Scene RGB-T payload on generated MaterialX shaders. The
...
let cursor=0;let sectionCount=0;let routedCount=0;const sectionAnchor='// Calculate the BSDF transmission for viewing direction';const opacityAnchor='// Compute and apply surface opacity';while(true){const begin=out.indexOf(sectionAnchor,cursor);if(begin===-1)break;sectionCount++;const end=out.indexOf(opacityAnchor,begin);if(end===-1){supported=false;break;}const section=out.slice(begin,end);// A generated viewing-transmission section has one terminal closure
...
const matches=[...section.matchAll(/(\w+)\.color\s*\+=\s*(\w+)\.response\s*;/g)];if(matches.length!==1){supported=false;break;}const match=matches[0];const terminal=match[0];const lhs=match[1];const rhs=match[2];// A refracting hit's response is a real colour sample, not a tint;
// it belongs in C like an ordinary BSDF term. A miss (flag set
// inside mx_scene_refraction) still routes to T as usual.
const routed='if (u_peelRgbt != 0 && (u_peelRefractsScene == 0 || mx_sceneRefractionMiss)) '+lhs+'.transparency = clamp('+rhs+'.response, vec3(0.0), vec3(1.0));\n'+'    else '+terminal;const at=begin+match.index;out=out.slice(0,at)+routed+out.slice(at+terminal.length);cursor=at+routed.length;routedCount++;}if(!sectionCount||routedCount!==sectionCount){mtlxWarn('mtlx-engine: RGB-T payload section anchors changed; using Scene legacy peel.');supported=false;}// The generated output is coverage-premultiplied C and transparency T.
...
const output=out.match(/\bout\s+vec4\s+(\w+)\s*;/);if(output){const v=output[1];// encodeDisplay() has already appended a second assignment to the
...
const assignments=[...out.matchAll(new RegExp('('+v+'\\s*=\\s*vec4\\([^;]+\\);)','g'))].filter(m=>/vec4\(\s*\w+\.color\s*,/.test(m[0]));const om=assignments.length===1?assignments[0]:null;if(!om)supported=false;if(om&&out.indexOf('u_peelRgbtPass',om.index)===-1){const surfaceMatch=om[0].match(/vec4\(\s*(\w+)\.color\s*,/);if(!surfaceMatch)supported=false;const surfaceVar=surfaceMatch?surfaceMatch[1]:'';const injectAt=om.index+om[0].length;// A refracting hit's colour already landed in C above; writing
// it into T too would double it via the compositor's t*opaque
// term, so T is zeroed there. A miss still writes T normally.
out=out.slice(0,injectAt)+'\n    if (u_peelRgbt != 0 && u_peelRgbtPass == 1) '+v+' = (u_peelRefractsScene != 0 && !mx_sceneRefractionMiss) ? vec4(0.0, 0.0, 0.0, 1.0) : vec4('+surfaceVar+'.transparency, 1.0);'+out.slice(injectAt);}// Clear transmissive emission has outAlpha=0 but still contributes C;
...
out=out.replace(/if\s*\(\s*outAlpha\s*<\s*u_alphaThreshold\s*\)\s*(?:\{\s*discard\s*;\s*\}|discard\s*;)/,'if (u_peelRgbt == 0 && outAlpha < u_alphaThreshold) { discard; }');}else supported=false;if(!supported)return original;// Declarations must precede every generated function: transmission
// helpers are commonly emitted before main().
const firstFn=out.search(/^(?:void|vec[234]|float|int|bool|mat[234])\s+\w+\s*\(/m);const decl='uniform int u_peelRgbt;\nuniform int u_peelRgbtPass;\nuniform int u_peelRgbtLayer;\n/* MX_RGBT_PAYLOAD_SUPPORTED */\n';if(firstFn===-1)return original;if(out.indexOf('uniform int u_peelRgbt;')===-1){out=out.slice(0,firstFn)+decl+out.slice(firstFn);}// The T-zero gate above always needs these; patchTransmissionAlpha may
// already have declared them (refraction path) or may not have run at
// all for this shading model, so guard separately here too.
if(out.indexOf('uniform int u_peelRefractsScene;')===-1){out=out.slice(0,firstFn)+'uniform int u_peelRefractsScene;\n'+out.slice(firstFn);}if(out.indexOf('bool mx_sceneRefractionMiss')===-1){out=out.slice(0,firstFn)+'bool mx_sceneRefractionMiss = false;\n'+out.slice(firstFn);}// patchTransmissionAlpha runs first to preserve the complete legacy
// source; its scalar floor is disabled only while RGB-T is active.
out=out.replace(/if\s*\(\s*u_peelMode\s*!=\s*0\s*\)\s*\{/g,'if (u_peelMode != 0 && u_peelRgbt == 0) {');return out;};// injectPeelDiscard(src), bakes the depth-peel OIT machinery into
...
const injectPeelDiscard=(src,sceneRgbt=false)=>{// Skip decls patchTransmissionAlpha may have already inserted.
const declIfAbsent=line=>src.indexOf(line)===-1?line+'\n':'';const decls=declIfAbsent('uniform int u_peelMode;')+declIfAbsent('uniform int u_peelHasPrev;')+declIfAbsent('uniform highp sampler2D u_peelPrevDepth;')+declIfAbsent('uniform highp sampler2D u_opaqueDepth;')+declIfAbsent('uniform int u_peelLinear;');const block='\n    if (u_peelMode != 0) {\n'+'        ivec2 _pc = ivec2(gl_FragCoord.xy);\n'+'        float _opaqueZ = texelFetch(u_opaqueDepth, _pc, 0).r;\n'+'        if (gl_FragCoord.z >= _opaqueZ) discard;\n'+'        if (u_peelHasPrev != 0) {\n'+'            float _prevZ = texelFetch(u_peelPrevDepth, _pc, 0).r;\n'+'            // PEEL_EPS: ~17 quanta of 24-bit depth precision; constant across cameras/near-far (revisit if coplanar shells misrender).\n'+'            if (gl_FragCoord.z <= _prevZ + 1e-6) discard;\n'+'        }\n'+'    }\n';const mainIdx=src.indexOf('void main');if(mainIdx===-1)throw new Error('injectPeelDiscard: no main() found (MaterialX output format may have changed)');const braceIdx=src.indexOf('{',mainIdx);if(braceIdx===-1)throw new Error('injectPeelDiscard: no main() body found (MaterialX output format may have changed)');let out=src.slice(0,mainIdx)+decls+src.slice(mainIdx,braceIdx+1)+block+src.slice(braceIdx+1);// Tail pass (u_peelMode==2) writes premultiplied color so it can
...
const outMatch=out.match(/\bout\s+vec4\s+(\w+)\s*;/);if(out.indexOf('MX_LIGHT_TRANSPORT_EARLY_RETURN')!==-1){// no-op: the tail is unreachable for this variant.
}else if(outMatch){const v=outMatch[1];const closeIdx=out.lastIndexOf('}');const premult='\n    if (u_peelMode == 2 && '+(sceneRgbt?'u_peelRgbt == 0':'true')+') { '+v+'.rgb *= '+v+'.a; }\n';out=out.slice(0,closeIdx)+premult+out.slice(closeIdx);}else{mtlxWarn('mtlx-engine: injectPeelDiscard could not locate the fragment output variable, tail-pass premultiply skipped.');}return out;};// Emscripten throws C++ exceptions as raw NUMBER pointers, not Error
...
const mxErr=(mx,e)=>{try{if(typeof e==='number'&&mx&&typeof mx.getExceptionMessage==='function'){const msg=mx.getExceptionMessage(e);// getExceptionMessage may return a string or [type, message]
if(Array.isArray(msg))return msg.filter(Boolean).join(': ');if(msg)return String(msg);}}catch(_){/* fall through to generic handling */}if(e&&e.message)return e.message;return String(e);};// CRITICAL: the wasm binding of setValueString is the TYPED
...
const mxWriteValue=(inp,str,type)=>{mxWarnIfLocked('mxWriteValue');// exported doc-mutating helper, see mxWarnIfLocked's header comment
try{if(typeof inp.setAttribute==='function'){inp.setAttribute('value',String(str));return;}}catch(e){/* fall through */}try{// Two-arg form sets value AND the correct type explicitly.
inp.setValueString(String(str),type||inp.getType());return;}catch(e){/* fall through */}inp.setValueString(String(str));try{if(type)inp.setType(type);}catch(e){/* best-effort */}};// MaterialX JS marshals std::vector either as a real JS array or as a
// {size(), get(i)} object depending on the binding; normalize to array.
const vecToArray=v=>{if(!v)return[];if(Array.isArray(v))return v;// this vendored build marshals vectors as real JS arrays
if(typeof v.size==='function'){const out=[];for(let i=0;i<v.size();i++)out.push(v.get(i));// embind-owned heap vector; materialized elements are independent
// shared_ptr handles, so free the wrapper. Audited: no caller
// retains the raw vector (js/ and scripts/ checked).
if(typeof v.delete==='function'){try{v.delete();}catch(e){/* already freed */}}return out;}return[];};const mxSafe=(fn,fb)=>{try{const v=fn();return v==null?fb:v;}catch(e){return fb;}};const mxElCat=el=>mxSafe(()=>el.getCategory(),'');const mxElType=el=>mxSafe(()=>String(el.getType()),'');const mxElName=el=>mxSafe(()=>el.getName(),'');const mxElAttr=(el,name)=>mxSafe(()=>el.getAttribute(name),'');// GLSL leaves pow(x, y) undefined for x < 0, even when y is an exact integer.
...
const materialPowerNodeNames=doc=>{if(!doc)return[];const nodes=[];try{nodes.push(...vecToArray(doc.getNodes?doc.getNodes():null));}catch(e){/* empty */}try{for(const graph of vecToArray(doc.getNodeGraphs?doc.getNodeGraphs():null)){nodes.push(...vecToArray(graph.getNodes?graph.getNodes():null));}}catch(e){/* empty */}return Array.from(new Set(nodes.filter(node=>mxElCat(node)==='power').map(node=>mxElName(node)).filter(Boolean)));};const materialGeompropDefaults=doc=>{const defaults=new Map();if(!doc)return defaults;const nodes=[];nodes.push(...vecToArray(mxSafe(()=>doc.getNodes?doc.getNodes():null,[])));for(const graph of vecToArray(mxSafe(()=>doc.getNodeGraphs?doc.getNodeGraphs():null,[]))){nodes.push(...vecToArray(mxSafe(()=>graph.getNodes?graph.getNodes():null,[])));}for(const node of nodes){if(mxElCat(node)!=='geompropvalue')continue;let propName='';let value='';for(const input of vecToArray(mxSafe(()=>node.getInputs(),[]))){const inputName=mxSafe(()=>String(input.getName()),'');const inputValue=mxElAttr(input,'value');if(inputName==='geomprop')propName=String(inputValue||'');else if(inputName==='default')value=String(inputValue||'');}if(!propName||!value)continue;const parts=value.split(',').map(part=>Number(part.trim())).filter(Number.isFinite);if(parts.length)defaults.set(propName,parts);}return defaults;};const POWER_NODE_REAL_GLSL=`
...
`;const patchMaterialPowerNodes=(fs,nodeNames)=>{if(!fs||!nodeNames||!nodeNames.length)return fs;let patched=fs;let replacements=0;for(const name of nodeNames){const escaped=String(name).replace(/[.*+?^${}()|[\]\\]/g,'\\$&');const assignment=new RegExp('(\\b'+escaped+'_out\\s*=\\s*)pow\\s*\\(','g');patched=patched.replace(assignment,(match,prefix)=>{replacements++;return prefix+'mx_preview_power_real(';});}if(!replacements||patched.includes('float mx_preview_power_real('))return patched;const firstFunction=patched.search(/\bvoid\s+\w+\s*\(/);return firstFunction>=0?patched.slice(0,firstFunction)+POWER_NODE_REAL_GLSL+'\n'+patched.slice(firstFunction):POWER_NODE_REAL_GLSL+'\n'+patched;};const mxElHasAttr=(el,name)=>mxSafe(()=>el.hasAttribute(name),false);// Exception-safe single-attribute writes, the wasm binding can throw on
// a detached/invalid element, which mxSafe swallows into a `false` return.
const mxSetAttr=(el,name,value)=>mxSafe(()=>{el.setAttribute(name,value);return true;},false);const mxRemoveAttr=(el,name)=>mxSafe(()=>{el.removeAttribute(name);return true;},false);// Tag an element's colorspace, preferring the typed setColorSpace()
...
const mxSetColorspace=(el,cs)=>{mxWarnIfLocked('mxSetColorspace');// exported doc-mutating helper, see mxWarnIfLocked's header comment
return mxSafe(()=>{if(typeof el.setColorSpace==='function')el.setColorSpace(cs);else el.setAttribute('colorspace',cs);return true;},false);};// Shortest `convert` hop chain fromType->toType (only conversions the
// library defines), a mismatched convert otherwise fails silently
// until GLSL compile. []=no convert needed, null=unreachable.
const findConvertChain=(doc,fromType,toType)=>{mxWarnIfLocked('findConvertChain');// exported doc-reading helper, see mxWarnIfLocked's header comment
if(fromType===toType)return[];const typeStr=t=>t&&t.getName?t.getName():String(t||'');// convert nodedefs -> directed edges inType -> outType
const convEdges={};for(const def of vecToArray(mxSafe(()=>doc.getMatchingNodeDefs('convert'),[]))){const ins=vecToArray(mxSafe(()=>def.getInputs(),[]));if(ins.length!==1)continue;const inT=typeStr(mxSafe(()=>ins[0].getType(),''));const outT=typeStr(mxSafe(()=>def.getType(),''));if(!inT||!outT||outT==='multioutput')continue;(convEdges[inT]=convEdges[inT]||new Set()).add(outT);}// BFS, shortest chain wins (converts are cheap but each hop is
// another generated function).
const prev={[fromType]:null};let frontier=[fromType];while(frontier.length){const next=[];for(const t of frontier){for(const n of convEdges[t]||[]){if(n in prev)continue;prev[n]=t;if(n===toType){const chain=[];for(let c=toType;c!==fromType;c=prev[c])chain.unshift(c);return chain;}next.push(n);}}frontier=next;}return null;};// Create-or-fetch an input on `node`, guaranteeing its TYPE, the only
...
const ensureTypedInput=(doc,node,inputName,wantedType)=>{mxWarnIfLocked('ensureTypedInput');// exported doc-mutating helper, see mxWarnIfLocked's header comment
let inp=mxSafe(()=>node.getInput(inputName),null);let how='existing';if(!inp){let defInput=null;const cat=mxElCat(node);for(const d of vecToArray(mxSafe(()=>doc.getMatchingNodeDefs(cat),[]))){const cand=mxSafe(()=>d.getInput(inputName),null)||mxSafe(()=>d.getActiveInput(inputName),null);if(!cand)continue;if(!defInput)defInput=cand;// fallback: first found
if(wantedType&&mxElType(cand)===wantedType){defInput=cand;break;}}inp=mxSafe(()=>node.addInput(inputName),null);how='added-bare';if(inp&&defInput){const copied=mxSafe(()=>{inp.copyContentFrom(defInput);return true;},false);if(copied){how='copied-from-nodedef';// The copy brings nodedef UI/doc metadata along, noisy in
// exports. defaultgeomprop is worse: MaterialX's validator
// rejects it outright on a node-instance input.
for(const attr of['uimin','uimax','uisoftmin','uisoftmax','uistep','uiname','uifolder','uiadvanced','doc','enum','enumvalues','defaultgeomprop']){mxRemoveAttr(inp,attr);}}}}// Enforce the caller's type UNCONDITIONALLY, a wrong-typed copy (see
// above) must not survive; the caller knows the graph typing, the
// copy only supplies defaults/metadata.
if(inp&&wantedType&&mxElType(inp)!==wantedType){mxSafe(()=>{if(typeof inp.setType==='function')inp.setType(wantedType);else inp.setAttribute('type',wantedType);return true;},false);if(mxElType(inp)!==wantedType){mxSetAttr(inp,'type',wantedType);}// A copied default VALUE is malformed for the corrected type,
// drop it; callers connect or re-value anyway.
mxRemoveAttr(inp,'value');}if(inp&&wantedType&&mxElType(inp)!==wantedType){mtlxWarn('ensureTypedInput: "'+inputName+'" is "'+mxElType(inp)+'" (wanted "'+wantedType+'"), path='+how);}return inp;};// Const-inputs kill switch, default ON. '0' disables it. Read per
// generation.
...
const DISPLACEMENT_CONST_INPUT_NAMES=CONST_INPUT_NAMES.concat(['type','style','clampoutput']);const DISPLACEMENT_CONST_INPUTS_KEY='mtlx_displacement_const_inputs';const readDisplacementConstInputs=()=>{try{return localStorage.getItem(DISPLACEMENT_CONST_INPUTS_KEY)!=='0';}catch(e){return true;}};// Deterministic GLSL literal for one introspected default; floats always
// carry a decimal point (or an exponent). Returns null when unusable.
const constInputLiteral=(type,data)=>{if(type==='boolean')return data?'true':'false';const n=Number(data);if(!Number.isFinite(n))return null;if(type==='integer')return String(n|0);const s=String(n);return /[.eE]/.test(s)?s:s+'.0';};// Which targeted input an introspected uniform is, or null. Prefers the
// MaterialX path's last segment; the flattened uniform name is the fallback.
const constInputKey=(u,names=CONST_INPUT_NAMES)=>{const name=String(u&&u.name||'');if(!name||name.indexOf('u_')===0)return null;// engine/private uniform
const seg=u.path?String(u.path).split('/').pop():'';if(seg&&names.indexOf(seg)>=0)return seg;for(const n of names){if(name===n||name.endsWith('_'+n))return n;}return null;};// Everything a usage scan must not see: line and block comments, the
...
const stripGlslForUsage=src=>String(src||'').replace(/\/\*[\s\S]*?\*\//g,' ').replace(/\/\/[^\n]*/g,' ').replace(/^[ \t]*#[^\n]*/gm,' ').replace(/^[ \t]*uniform\b[^;]*;/gm,' ');const USAGE_IDENT_RE=/[A-Za-z_]\w*/g;const addIdents=(text,out)=>{let m;USAGE_IDENT_RE.lastIndex=0;while(m=USAGE_IDENT_RE.exec(text))out.add(m[0]);};// Both operand sides of every comparison in a condition region.
const addComparisonIdents=(text,out)=>{let m;const left=/([A-Za-z_]\w*)[ \t]*(?:==|!=|<=|>=|<|>)/g;while(m=left.exec(text))out.add(m[1]);const right=/(?:==|!=|<=|>=|<|>)[ \t]*([A-Za-z_]\w*)/g;while(m=right.exec(text))out.add(m[1]);};// Balanced span starting at the opening bracket at `open`; returns the
// inner text, or null when the source is unbalanced.
const balancedSpan=(src,open,openCh,closeCh)=>{let depth=0;for(let i=open;i<src.length;i++){const c=src[i];if(c===openCh)depth++;else if(c===closeCh){depth--;if(depth===0)return src.slice(open+1,i);}}return null;};// Walks left from a ternary `?` to the start of its condition, stopping at
// the first unbalanced opener or statement separator at depth zero.
const ternaryCondition=(src,qm)=>{let depth=0;let i=qm-1;for(;i>=0;i--){const c=src[i];if(c===')'||c===']')depth++;else if(c==='('||c==='['){if(depth===0)break;depth--;}else if(depth===0&&(c===';'||c==='{'||c==='}'||c===','||c===':'||c==='?'))break;else if(depth===0&&c==='='&&src[i-1]!=='='&&src[i-1]!=='!'&&src[i-1]!=='<'&&src[i-1]!=='>'&&src[i+1]!=='=')break;}return src.slice(i+1,qm);};// Usage rule for integer uniforms: an int whose identifier reaches an array
...
const selectDynamicIndexUniforms=(vs,fs,introspected)=>{const picked=new Set();const candidates=new Set();for(const u of introspected||[]){const name=String(u&&u.name||'');if(!name||name.indexOf('u_')===0)continue;// engine/private uniform
if(u.type!=='integer'||CONST_INPUT_DENY.has(name))continue;candidates.add(name);}if(!candidates.size)return picked;const used=new Set();for(const raw of[vs,fs]){const src=stripGlslForUsage(raw);for(let i=0;i<src.length;i++){if(src[i]==='['){const inner=balancedSpan(src,i,'[',']');if(inner!=null)addIdents(inner,used);}else if(src[i]==='?'){addComparisonIdents(ternaryCondition(src,i),used);}}let m;const switchRe=/\bswitch[ \t\n]*\(/g;while(m=switchRe.exec(src)){const inner=balancedSpan(src,switchRe.lastIndex-1,'(',')');if(inner!=null)addIdents(inner,used);}const ifRe=/\bif[ \t\n]*\(/g;while(m=ifRe.exec(src)){const inner=balancedSpan(src,ifRe.lastIndex-1,'(',')');if(inner!=null)addComparisonIdents(inner,used);}}for(const name of candidates)if(used.has(name))picked.add(name);return picked;};// Rewrites `uniform T name;` into `const T name = <literal>;` for the
...
const SELECTOR_CONST_INPUTS=new Set(['mode','type','style']);const constifyInputUniforms=(vs,fs,introspected,names=CONST_INPUT_NAMES)=>{const constInputs=[];const kept=[];const byUsage=selectDynamicIndexUniforms(vs,fs,introspected);let outVs=vs;let outFs=fs;for(const u of introspected){const glslType=CONST_INPUT_GLSL_TYPES[u.type];const key=glslType?constInputKey(u,names):null;const usageHit=glslType?byUsage.has(String(u.name)):false;// `mode`, `type` and `style` are generic names: only take them when
// the generator typed them as an enum selector (integer), never a
// float or boolean input.
if(!key&&!usageHit||CONST_INPUT_DENY.has(String(u.name))||key&&!usageHit&&SELECTOR_CONST_INPUTS.has(key)&&u.type!=='integer'){kept.push(u);continue;}const literal=u.data==null?null:constInputLiteral(u.type,u.data);if(literal==null){kept.push(u);continue;}const escaped=String(u.name).replace(/[.*+?^${}()|[\]\\]/g,'\\$&');const declRe=new RegExp('(^|\\n)([ \\t]*)uniform[ \\t]+(?:(?:low|medium|high)p[ \\t]+)?'+glslType+'[ \\t]+'+escaped+'[ \\t]*;','g');const hits=(outVs.match(declRe)||[]).length+(outFs.match(declRe)||[]).length;if(hits!==1){kept.push(u);continue;}const replacement='$1$2const '+glslType+' '+u.name+' = '+literal+';';outVs=outVs.replace(declRe,replacement);outFs=outFs.replace(declRe,replacement);constInputs.push({name:u.name,path:u.path||null,value:u.data});}return{vs:outVs,fs:outFs,introspected:kept,constInputs};};// Sweep run before every writeToXmlString call, fixing two attributes
...
const stripValuesFromConnectedInputs=(doc,maxDepth)=>{mxWarnIfLocked('stripValuesFromConnectedInputs');// exported doc-mutating helper, see mxWarnIfLocked's header comment
const cap=typeof maxDepth==='number'?maxDepth:10;let stripped=0;const walk=(el,depth)=>{if(!el||depth>cap)return;const children=vecToArray(mxSafe(()=>el.getChildren(),[]));for(const child of children){if(mxElCat(child)==='input'){const connected=mxElAttr(child,'nodename')||mxElAttr(child,'nodegraph')||mxElAttr(child,'interfacename');// Presence, not truthiness: an empty value="" on a
// connected input is just as invalid as a non-empty one;
// mxElAttr's '' fallback can't tell absent from present-but-empty.
if(connected&&mxElHasAttr(child,'value')){const removed=mxRemoveAttr(child,'value');if(removed)stripped++;}// `el` (the loop's parent, already in scope) is this
// input's parent element, reused here instead of a
// second getParent() round trip.
const parentCat=mxElCat(el);if(parentCat!=='nodegraph'&&parentCat!=='nodedef'&&mxElHasAttr(child,'defaultgeomprop')){const removed=mxRemoveAttr(child,'defaultgeomprop');if(removed)stripped++;}}walk(child,depth+1);}};walk(doc,0);return stripped;};// MaterialX 1.39 colorspace names an author may still write from older
...
const COLORSPACE_ALIASES={srgb_tx:'srgb_texture',sRGB:'srgb_texture',srgb:'srgb_texture',Raw:null,raw:null,none:null};// Depth-capped walk (stripValuesFromConnectedInputs idiom) rewriting any
...
const applyColorspaceAliases=(doc,maxDepth)=>{mxWarnIfLocked('applyColorspaceAliases');// exported doc-mutating helper, see mxWarnIfLocked's header comment
const cap=typeof maxDepth==='number'?maxDepth:10;const rewrites=new Map();const restores=[];const visit=(el,depth)=>{if(!el||depth>cap)return;if(mxElHasAttr(el,'colorspace')){const cs=mxElAttr(el,'colorspace');if(Object.prototype.hasOwnProperty.call(COLORSPACE_ALIASES,cs)){const to=COLORSPACE_ALIASES[cs];const ok=to?mxSetAttr(el,'colorspace',to):mxRemoveAttr(el,'colorspace');if(ok){restores.push(()=>mxSetAttr(el,'colorspace',cs));const key=cs+' -> '+(to||'(removed)');rewrites.set(key,(rewrites.get(key)||0)+1);}}}const children=vecToArray(mxSafe(()=>el.getChildren(),[]));for(const child of children)visit(child,depth+1);};visit(doc,0);const restore=()=>{for(let i=restores.length-1;i>=0;i--)restores[i]();};return{restore,rewrites};};// cmlib nodes that convert an encoded texture to the lin_rec709 working
// space. Only spaces cmlib can actually transform appear here; anything
// else is reported rather than silently ignored.
const COLORSPACE_TO_WORKING_NODE={srgb_texture:'srgb_texture_to_lin_rec709',g22_rec709:'g22_rec709_to_lin_rec709',g18_rec709:'g18_rec709_to_lin_rec709',acescg:'acescg_to_lin_rec709',lin_ap1:'lin_ap1_to_lin_rec709',g22_ap1:'g22_ap1_to_lin_rec709',adobergb:'adobergb_to_lin_rec709',lin_adobergb:'lin_adobergb_to_lin_rec709',srgb_displayp3:'srgb_displayp3_to_lin_rec709',lin_displayp3:'lin_displayp3_to_lin_rec709',rec709_display:'rec709_display_to_lin_rec709'};// Inserts the colorspace transform MaterialX's own color management system
...
const applyColorspaceTransforms=(doc,maxDepth)=>{mxWarnIfLocked('applyColorspaceTransforms');// exported doc-mutating helper, see mxWarnIfLocked's header comment
const cap=typeof maxDepth==='number'?maxDepth:10;const restores=[];const converted=new Map();const unsupported=new Set();// cmlib converts INTO lin_rec709 only. A document working in another
// space would need the inverse leg too, so leave it alone and say so.
const docSpace=mxElAttr(doc,'colorspace');if(docSpace&&docSpace!=='lin_rec709'){return{restore:()=>{},converted,unsupported:new Set(['(document works in "'+docSpace+'", not lin_rec709)'])};}let serial=0;const visit=(parent,depth)=>{if(!parent||depth>cap)return;const children=vecToArray(mxSafe(()=>parent.getChildren(),[]));// Snapshot the child list first: the loop adds nodes to this parent.
const nodes=children.filter(c=>mxSafe(()=>typeof c.getInput==='function'&&typeof c.getCategory==='function',false));for(const node of nodes){const fileInput=mxSafe(()=>node.getInput('file'),null);if(!fileInput||!mxElHasAttr(fileInput,'colorspace')){visit(node,depth+1);continue;}const cs=mxElAttr(fileInput,'colorspace');const type=String(mxSafe(()=>node.getType(),''));if(type!=='color3'&&type!=='color4')continue;// float/vector images carry data, never color
const category=COLORSPACE_TO_WORKING_NODE[cs];if(!category){unsupported.add(cs);continue;}const nodeName=mxSafe(()=>node.getName(),null);if(!nodeName)continue;const cmName='__mtlx_cm_'+serial++ +'_'+nodeName;const cm=mxSafe(()=>parent.addNode(category,cmName,type),null);if(!cm){unsupported.add(cs);continue;}const cmIn=mxSafe(()=>cm.addInput('in',type),null);if(!cmIn||!mxSetAttr(cmIn,'nodename',nodeName)){mxSafe(()=>parent.removeChild(cmName),null);unsupported.add(cs);continue;}// Every consumer in this scope now reads the converted value.
...
const applyMaterialWorkspaceTransforms=(doc,maxDepth)=>{mxWarnIfLocked('applyMaterialWorkspaceTransforms');// exported doc-mutating helper, see mxWarnIfLocked's header comment
const cap=typeof maxDepth==='number'?maxDepth:10;const restores=[];let converted=0;// Same working-space guard as applyColorspaceTransforms: cmlib's
// acescg_to_lin_rec709 node converts INTO lin_rec709 only.
const docSpace=mxElAttr(doc,'colorspace');if(docSpace&&docSpace!=='lin_rec709'){return{restore:()=>{},converted:0,unsupported:new Set(['(document works in "'+docSpace+'", not lin_rec709)'])};}let serial=0;// Wraps `nodeName`'s output (already a color3/color4 node in `parent`)
...
const wrapNodeOutput=(parent,children,node,nodeName,type)=>{const cmName='__mtlx_ws_'+serial++ +'_'+nodeName;const cm=mxSafe(()=>parent.addNode('acescg_to_lin_rec709',cmName,type),null);if(!cm)return false;const cmIn=mxSafe(()=>cm.addInput('in',type),null);if(!cmIn||!mxSetAttr(cmIn,'nodename',nodeName)){mxSafe(()=>parent.removeChild(cmName),null);return false;}const redirect=el=>{if(mxElAttr(el,'nodename')!==nodeName)return;if(mxSetAttr(el,'nodename',cmName))restores.push(()=>mxSetAttr(el,'nodename',nodeName));};for(const sibling of children){if(sibling===node||sibling===cm)continue;redirect(sibling);for(const input of vecToArray(mxSafe(()=>sibling.getInputs(),[])))redirect(input);}restores.push(()=>mxSafe(()=>parent.removeChild(cmName),null));return true;};const visit=(parent,depth)=>{if(!parent||depth>cap)return;const children=vecToArray(mxSafe(()=>parent.getChildren(),[]));const nodes=children.filter(c=>mxSafe(()=>typeof c.getInputs==='function'&&typeof c.getCategory==='function',false));for(const node of nodes){const category=mxSafe(()=>node.getCategory(),'');const nodeType=mxSafe(()=>String(node.getType()),'');// geompropvalue's OUTPUT carries the primvar's colour (displayColor
// and friends): no attribute on it is ever a "value", so it's
// converted the same way a tagged image's output is, not as a
// literal input below.
if(category==='geompropvalue'&&(nodeType==='color3'||nodeType==='color4')&&!mxElHasAttr(node,'colorspace')){const nodeName=mxSafe(()=>node.getName(),null);if(nodeName&&wrapNodeOutput(parent,children,node,nodeName,nodeType))converted+=1;}const inputs=vecToArray(mxSafe(()=>node.getInputs(),[]));for(const inp of inputs){const type=mxSafe(()=>String(inp.getType()),'');if(type!=='color3'&&type!=='color4')continue;// vector3 and float values carry data, never colour
...
const value=mxSafe(()=>inp.getValueString(),null);if(value==null||value==='')continue;const inputName=mxSafe(()=>inp.getName(),null);const nodeName=mxSafe(()=>node.getName(),null);if(!inputName||!nodeName)continue;const cmName='__mtlx_ws_'+serial++ +'_'+nodeName+'_'+inputName;const cm=mxSafe(()=>parent.addNode('acescg_to_lin_rec709',cmName,type),null);if(!cm)continue;const cmIn=mxSafe(()=>cm.addInput('in',type),null);if(!cmIn||!mxSetAttr(cmIn,'value',value)){mxSafe(()=>parent.removeChild(cmName),null);continue;}if(!mxSetAttr(inp,'nodename',cmName)){mxSafe(()=>parent.removeChild(cmName),null);continue;}mxRemoveAttr(inp,'value');restores.push(()=>{mxRemoveAttr(inp,'nodename');mxSetAttr(inp,'value',value);});restores.push(()=>mxSafe(()=>parent.removeChild(cmName),null));converted+=1;}visit(node,depth+1);// nested nodegraphs/compound instances
}};visit(doc,0);const restore=()=>{for(let i=restores.length-1;i>=0;i--)restores[i]();};return{restore,converted};};// Doc-level renderable scan: returns [{ name, node }], one entry per
...
// leading ./ or /.
const normPath=p=>String(p||'').replace(/\\/g,'/').replace(/^\.?\//,'').toLowerCase();// AppleDouble and Finder metadata entries: never real documents, only
...
const expandZips=async map=>{let skipped=0;for(const key of Object.keys(map)){if(!/\.zip$/i.test(key))continue;const file=map[key];delete map[key];if(!window.JSZip){throw new Error('The JSZip library is not loaded: .zip files can\'t be expanded. Reload the page and try again.');}const zip=await JSZip.loadAsync(file);const names=Object.keys(zip.files);for(const name of names){const entry=zip.files[name];if(entry.dir)continue;if(isHiddenSideFile(name)){skipped++;continue;}map[name]=await entry.async('blob');}}if(skipped)console.info('expandZips: skipped '+skipped+' side file(s)');return map;};// Find a dropped file for a path referenced inside the document:
// exact normalized match → unique suffix match → unique basename match.
const findFileForRef=(fileMap,ref)=>{const want=normPath(ref);if(!want)return null;const keys=Object.keys(fileMap);const norm={};for(const k of keys)norm[normPath(k)]=k;if(norm[want])return{key:norm[want],how:'exact'};const suffix=keys.filter(k=>normPath(k).endsWith('/'+want)||normPath(k)===want);if(suffix.length===1)return{key:suffix[0],how:'suffix'};const base=want.split('/').pop();const byBase=keys.filter(k=>normPath(k).split('/').pop()===base);if(byBase.length===1)return{key:byBase[0],how:'basename'};return null;};// A <UDIM> reference names a tile set: every key whose path matches the
...
const findFilesForRef=(fileMap,ref)=>{const raw=String(ref||'');if(!/<UDIM>/i.test(raw)){const hit=findFileForRef(fileMap,raw);return hit?[{key:hit.key,how:hit.how,ref:raw}]:[];}const want=normPath(raw);const escaped=want.replace(/[.*+?^${}()|[\]\\]/g,'\\$&').replace(/<udim>/g,'(\\d{4})');const tries=[{how:'exact',re:new RegExp('^'+escaped+'$')},{how:'suffix',re:new RegExp('(^|/)'+escaped+'$')},{how:'basename',re:new RegExp('(^|/)'+escaped.split('/').pop()+'$')}];for(const{how,re}of tries){const hits=[];for(const key of Object.keys(fileMap)){const m=re.exec(normPath(key));if(m)hits.push({key,how,ref:raw.replace(/<UDIM>/gi,m[m.length-1])});}if(hits.length)return hits;}return[];};// Given a resolved file-map hit, prefer a sibling "<stem>.ktx2" in the same
...
const preferKtx2Sibling=(fileMap,hit)=>{if(!hit||/\.ktx2$/i.test(hit.key))return hit;const dot=hit.key.lastIndexOf('.');if(dot<0)return hit;const ktx2Key=hit.key.slice(0,dot)+'.ktx2';if(Object.prototype.hasOwnProperty.call(fileMap,ktx2Key)){return{key:ktx2Key,how:hit.how,substituted:true};}return hit;};// Inline <xi:include href="..."/> from the dropped files (MaterialX
...
const readMtlxText=async(entry,path,map)=>{const raw=await entry.text();const dir=path.indexOf('/')>=0?path.slice(0,path.lastIndexOf('/')):'';const resolved=/<xi:include\b/.test(raw)?await resolveIncludes(raw,map,dir):raw;return{raw,resolved};};// Parses MaterialX XML keeping comments as elements in document order, so a
// write puts them back where they were; builds without XmlReadOptions fall back.
const readMtlxXml=async(mx,doc,xml)=>{let opts=null;try{if(typeof mx.XmlReadOptions==='function'){opts=new mx.XmlReadOptions();opts.readComments=true;}}catch(e){opts=null;}return opts?mx.readFromXmlString(doc,xml,'',opts):mx.readFromXmlString(doc,xml);};// Comments outside <materialx> never become elements, so they are carried as
// strings. The site's own attribution is dropped here and re-added on export.
const isExportAttribution=comment=>/^<!--\s*Exported by MaterialX Playground/.test(String(comment||''));const splitXmlEnvelope=text=>{const src=String(text==null?'':text).replace(/\r\n?/g,'\n');const comments=s=>(s.match(/<!--[\s\S]*?-->/g)||[]).filter(c=>!isExportAttribution(c));const open=src.search(/<materialx\b/);const close=src.lastIndexOf('</materialx>');return{prolog:open>0?comments(src.slice(0,open).replace(/<\?xml[^>]*\?>/,'')):[],trailer:close>=0?comments(src.slice(close+'</materialx>'.length)):[]};};const withXmlEnvelope=(xml,envelope)=>{let text=xml==null?'':String(xml);const prolog=envelope&&envelope.prolog||[];const trailer=envelope&&envelope.trailer||[];if(prolog.length){const m=/^\s*<\?xml[^>]*\?>\n?/.exec(text);const at=m?m[0].length:0;text=text.slice(0,at)+(m&&!m[0].endsWith('\n')?'\n':'')+prolog.join('\n')+'\n'+text.slice(at);}const close=text.lastIndexOf('</materialx>');if(trailer.length&&close>=0){const end=close+'</materialx>'.length;text=text.slice(0,end)+'\n'+trailer.join('\n')+text.slice(end);}return text;};// Source formatting survives a save: tags and comments are matched between the
...
const XML_ENTITIES={quot:'"',apos:"'",lt:'<',gt:'>',amp:'&'};const xmlDecode=s=>s.replace(/&(quot|apos|lt|gt|amp|#\d+|#x[0-9a-f]+);/gi,(m,e)=>{if(e[0]!=='#')return XML_ENTITIES[e.toLowerCase()];return String.fromCodePoint(e[1]==='x'||e[1]==='X'?parseInt(e.slice(2),16):parseInt(e.slice(1),10));});// Element name, attributes with their value offsets, and where new attributes go.
const xmlTagParts=raw=>{const m=/^<([^\s/>!?]+)/.exec(raw);if(!m)return null;const attrs=new Map();const attrRe=/(\s*)([^\s=/>]+)\s*=\s*(?:"([^"]*)"|'([^']*)')/g;attrRe.lastIndex=m[0].length;let a,lastEnd=m[0].length;while(a=attrRe.exec(raw)){const quoted=a[3]!=null?a[3]:a[4];const valueEnd=a.index+a[0].length-1;attrs.set(a[2],{value:xmlDecode(quoted),quote:a[3]!=null?'"':"'",start:a.index,end:a.index+a[0].length,valueStart:valueEnd-quoted.length,valueEnd});lastEnd=a.index+a[0].length;}return{tag:m[1],attrs,insertAt:lastEnd,selfClosing:/\/\s*>$/.test(raw)};};const xmlTokenKey=raw=>{if(raw.startsWith('<!--'))return'<!--'+raw.slice(4,-3).replace(/\s+/g,' ').trim()+'-->';// Any XML declaration matches any other: quoting and encoding are layout.
if(raw.startsWith('<?xml'))return'<?xml?>';if(raw.startsWith('<?')||raw.startsWith('</'))return raw.replace(/\s+/g,'');const parts=xmlTagParts(raw);if(!parts)return raw.replace(/\s+/g,' ');let key='<'+parts.tag;parts.attrs.forEach((v,n)=>{key+=' '+n+'="'+v.value+'"';});return key+(parts.selfClosing?'/>':'>');};const xmlTokenize=text=>{const tokens=[];const re=/<!--[\s\S]*?-->|<[^>]*>|[^<]+/g;let m,gapStart=0;while(m=re.exec(text)){if(m[0][0]!=='<'&&!m[0].trim())continue;tokens.push({key:m[0][0]==='<'?xmlTokenKey(m[0]):m[0].trim(),raw:m[0],gap:text.slice(gapStart,m.index)});gapStart=m.index+m[0].length;}return{tokens,tail:text.slice(gapStart)};};// Rewrites a source tag to the output tag's attributes without touching its
// layout; null when they are different elements (tag, name or closing form).
const xmlPatchTag=(srcRaw,outRaw)=>{const s=xmlTagParts(srcRaw),o=xmlTagParts(outRaw);if(!s||!o||s.tag!==o.tag||s.selfClosing!==o.selfClosing)return null;const sName=s.attrs.get('name'),oName=o.attrs.get('name');if((sName&&sName.value)!==(oName&&oName.value))return null;const esc=(v,q)=>v.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(q==='"'?/"/g:/'/g,q==='"'?'&quot;':'&apos;');const edits=[];s.attrs.forEach((sa,n)=>{const oa=o.attrs.get(n);if(!oa)edits.push({start:sa.start,end:sa.end,text:''});else if(oa.value!==sa.value)edits.push({start:sa.valueStart,end:sa.valueEnd,text:esc(oa.value,sa.quote)});});let added='';o.attrs.forEach((oa,n)=>{if(!s.attrs.has(n))added+=' '+n+'="'+esc(oa.value,'"')+'"';});if(added)edits.push({start:s.insertAt,end:s.insertAt,text:added});edits.sort((x,y)=>y.start-x.start);let text=srcRaw;for(const e of edits)text=text.slice(0,e.start)+e.text+text.slice(e.end);return text;};// Myers diff over token keys; returns source index per output index (-1 when
// inserted), or null when the edit distance exceeds maxD.
const xmlTokenMatches=(a,b,maxD)=>{const n=a.length,m=b.length;const match=new Int32Array(m).fill(-1);let lo=0;while(lo<n&&lo<m&&a[lo]===b[lo]){match[lo]=lo;lo++;}let hiA=n,hiB=m;while(hiA>lo&&hiB>lo&&a[hiA-1]===b[hiB-1]){hiA--;hiB--;match[hiB]=hiA;}const A=a.slice(lo,hiA),B=b.slice(lo,hiB),N=A.length,M=B.length;if(!N||!M)return match;const off=N+M+1;const v=new Int32Array(2*off+2);const trace=[];let found=false;for(let d=0;d<=Math.min(N+M,maxD)&&!found;d++){trace.push(v.slice(off-d,off+d+1));for(let k=-d;k<=d;k+=2){let x=k===-d||k!==d&&v[off+k-1]<v[off+k+1]?v[off+k+1]:v[off+k-1]+1;let y=x-k;while(x<N&&y<M&&A[x]===B[y]){x++;y++;}v[off+k]=x;if(x>=N&&y>=M){found=true;break;}}}if(!found)return null;let x=N,y=M;for(let d=trace.length-1;d>=0;d--){const vd=trace[d];const k=x-y;if(d===0){while(x>0&&y>0){match[lo+y-1]=lo+x-1;x--;y--;}break;}const at=kk=>vd[kk+d];const prevK=k===-d||k!==d&&at(k-1)<at(k+1)?k+1:k-1;const prevX=at(prevK),prevY=prevX-prevK;while(x>prevX&&y>prevY){match[lo+y-1]=lo+x-1;x--;y--;}x=prevX;y=prevY;}return match;};const preserveSourceFormatting=(sourceText,writtenXml,{maxD=3000,maxLength=16*1024*1024}={})=>{const written=writtenXml==null?'':String(writtenXml);const source=sourceText==null?'':String(sourceText);if(!source||source.length>maxLength||written.length>maxLength)return written;const src=xmlTokenize(source);const out=xmlTokenize(written);const match=xmlTokenMatches(src.tokens.map(t=>t.key),out.tokens.map(t=>t.key),maxD);if(!match)return written;const crlf=/\r\n/.test(source);const nl=s=>crlf?s.replace(/\r?\n/g,'\r\n'):s;// Source tokens dropped by the diff, reusable as in-place edits of a
// changed output tag between the same matched neighbours.
const used=new Uint8Array(src.tokens.length);match.forEach(i=>{if(i>=0)used[i]=1;});let text='';let prevSrc=-1;for(let j=0;j<out.tokens.length;j++){let i=match[j];if(i>=0){text+=src.tokens[i].gap+src.tokens[i].raw;prevSrc=i;continue;}let nextSrc=src.tokens.length;for(let jj=j+1;jj<out.tokens.length;jj++)if(match[jj]>=0){nextSrc=match[jj];break;}let patched=null;for(let c=prevSrc+1;c<nextSrc&&patched==null;c++){if(used[c]||src.tokens[c].raw[0]!=='<')continue;const p=xmlPatchTag(src.tokens[c].raw,out.tokens[j].raw);if(p!=null){patched=p;i=c;}}if(patched!=null){used[i]=1;text+=src.tokens[i].gap+patched;prevSrc=i;}else{text+=nl(out.tokens[j].gap)+nl(out.tokens[j].raw);}}const last=out.tokens.length-1;const lastMatchedToEnd=last>=0&&match[last]===src.tokens.length-1;return text+(lastMatchedToEnd?src.tail:nl(out.tail));};// Session-lifetime texture cache, keyed by file identity, re-binding the
...
const TEXTURE_CACHE=new Map();const textureCacheKey=(blob,fallback)=>{if(blob&&blob.name!=null&&blob.size!=null&&blob.lastModified!=null){return blob.name+'|'+blob.size+'|'+blob.lastModified;}return fallback;// e.g. the fileMap key, when identity fields are missing
...
const normalizeSamplerAddressMode=value=>{const mode=String(value==null?'':value).trim().toLowerCase();return mode==='clamp'||mode==='mirror'||mode==='periodic'?mode:'periodic';};const readMxInputValue=input=>{if(!input)return'';const attr=mxElAttr(input,'value');if(attr!=null&&String(attr)!=='')return String(attr);return mxSafe(()=>input.getValueString&&input.getValueString(),'');};const collectImageSamplerModes=doc=>{// Qualified paths prevent same-named image nodes in separate nodegraphs
// from sharing sampler state. A leaf fallback is retained only when its
// name is unique in the document.
const modes=new Map();const leafModes=new Map();const leafCounts=new Map();if(!doc)return modes;const visit=(el,depth,parentPath)=>{if(!el||depth>32)return;const name=mxElName(el);const currentPath=name?parentPath.concat(String(name)):parentPath;if(mxElCat(el)==='image'){if(name){const u=mxSafe(()=>el.getInput('uaddressmode'),null);const v=mxSafe(()=>el.getInput('vaddressmode'),null);const samplerModes={u:normalizeSamplerAddressMode(readMxInputValue(u)),v:normalizeSamplerAddressMode(readMxInputValue(v))};modes.set(currentPath.join('/'),samplerModes);leafModes.set(String(name),samplerModes);leafCounts.set(String(name),(leafCounts.get(String(name))||0)+1);}}let children=vecToArray(mxSafe(()=>el.getChildren(),[]));if(el===doc&&!children.length){children=children.concat(vecToArray(mxSafe(()=>doc.getNodes&&doc.getNodes(),[])));children=children.concat(vecToArray(mxSafe(()=>doc.getNodeGraphs&&doc.getNodeGraphs(),[])));}for(const child of children)visit(child,depth+1,currentPath);};visit(doc,0,[]);const uniqueLeaves=new Map();for(const[name,samplerModes]of leafModes){if(leafCounts.get(name)===1)uniqueLeaves.set(name,samplerModes);}modes.uniqueLeaves=uniqueLeaves;return modes;};const samplerCacheKey=(baseKey,samplerModes)=>{const modes=samplerModes||{u:'periodic',v:'periodic'};return String(baseKey)+'|uaddressmode='+normalizeSamplerAddressMode(modes.u)+'|vaddressmode='+normalizeSamplerAddressMode(modes.v);};// Parses a dropped .exr Blob via THREE.EXRLoader (pinned to three@0.147.0,
...
const loadExrTexture=async blob=>{if(typeof THREE.EXRLoader==='undefined'){console.warn('mtlx-engine: THREE.EXRLoader unavailable (script blocked/offline); .exr textures keep the node default color.');return null;}try{const buf=await blob.arrayBuffer();const d=new THREE.EXRLoader().setDataType(THREE.FloatType).parse(buf);if(!d||!d.data)return null;// EXRLoader.parse writes scanlines bottom-up (row 0 = image bottom);
// reverse row order so row 0 = top, matching every other loader here.
const channels=d.data.length/(d.width*d.height);const stride=d.width*channels;for(let y=0;y<d.height>>1;y+=1){const top=y*stride,bottom=(d.height-1-y)*stride;for(let i=0;i<stride;i+=1){const t=d.data[top+i];d.data[top+i]=d.data[bottom+i];d.data[bottom+i]=t;}}const tex=new THREE.DataTexture(d.data,d.width,d.height,d.format,d.type);tex.minFilter=tex.magFilter=THREE.LinearFilter;return tex;}catch(e){console.warn('mtlx-engine: failed to parse dropped .exr texture, keeping the node default color:',e);return null;}};// Parses a dropped .hdr Blob via THREE.RGBELoader's synchronous .parse().
...
const loadTifTexture=async(blob,path)=>{if(typeof UTIF==='undefined'){console.warn('mtlx-engine: UTIF unavailable (script blocked/offline); .tif textures keep the node default color.');return null;}const label=path||'(unknown)';const buf=await blob.arrayBuffer();const ifds=UTIF.decode(buf);if(!ifds||!ifds.length)return null;const ifd=ifds[0];const compression=ifd.t259&&ifd.t259[0];if(compression===32946)ifd.t259[0]=8;// old Deflate: same zlib stream, UTIF applies the predictor itself
UTIF.decodeImage(buf,ifd);const rgba=UTIF.toRGBA8(ifd);if(!UTIF_SUPPORTED_COMPRESSION.has(compression)&&compression!==32946){throw new Error('TIF decode unsupported (compression '+compression+') for '+label);}let allZero=true;for(let i=0;i<rgba.length-2&&allZero;i+=97){if(rgba[i]!==0||rgba[i+1]!==0||rgba[i+2]!==0)allZero=false;}if(allZero)throw new Error('TIF decode unsupported (compression '+compression+') for '+label);const tex=new THREE.DataTexture(new Uint8Array(rgba),ifd.width,ifd.height,THREE.RGBAFormat,THREE.UnsignedByteType);tex.minFilter=tex.magFilter=THREE.LinearFilter;return tex;};// Loads the original (non-.ktx2) source for a resolved hit, used when a
...
const HEAVY_TEXTURE_DECODE_CONCURRENCY=2;const heavyTextureDecodeLimitEnabled=()=>{try{return localStorage.getItem('mtlx_texture_decode_limit')!=='0';}catch(e){return true;}};const heavyTextureDecodeSlots=Array.from({length:HEAVY_TEXTURE_DECODE_CONCURRENCY},()=>Promise.resolve());let heavyTextureDecodeNext=0;const runHeavyTextureDecode=startDecode=>{if(!heavyTextureDecodeLimitEnabled())return startDecode();const slot=heavyTextureDecodeNext%heavyTextureDecodeSlots.length;heavyTextureDecodeNext+=1;const chained=heavyTextureDecodeSlots[slot].catch(()=>{}).then(startDecode);heavyTextureDecodeSlots[slot]=chained.catch(()=>{});return chained;};// Scene snapshots can contain many UDIM tiles.  When a caller supplies a
...
const loadBoundedBitmapTexture=async(blob,maxSize,samplerModes)=>{if(typeof createImageBitmap!=='function')throw new Error('createImageBitmap is unavailable for bounded scene texture preview');addTexturePerf('count',1);const opts={colorSpaceConversion:'none',premultiplyAlpha:'none'};if(sceneTextureFastPathEnabled()){let dims=null;try{dims=await readImageDimensions(blob);}catch(e){dims=null;}if(dims&&dims.width>0&&dims.height>0){const needsResize=maxSize>0&&Math.max(dims.width,dims.height)>maxSize;const decodeOpts=!needsResize?opts:Object.assign({},opts,{resizeWidth:Math.max(1,Math.round(dims.width*(maxSize/Math.max(dims.width,dims.height)))),resizeHeight:Math.max(1,Math.round(dims.height*(maxSize/Math.max(dims.width,dims.height)))),resizeQuality:'high'});const t0=performance.now();let image=null;try{image=await createImageBitmap(blob,decodeOpts);}catch(e){image=null;}if(image){addTexturePerf(needsResize?'resizeMs':'decodeMs',performance.now()-t0);const texture=new THREE.Texture(image);configureLoadedTexture(texture);return texture;}// Header parsed but the sized decode failed; fall through to the
// slow path below rather than fail the whole texture.
}}const t0=performance.now();let source;try{source=await createImageBitmap(blob,opts);}catch(error){throw new Error('The source image could not be decoded for bounded scene preview.');}addTexturePerf('decodeMs',performance.now()-t0);let image=source;if(maxSize>0&&Math.max(source.width,source.height)>maxSize){const scale=maxSize/Math.max(source.width,source.height);const resizeOpts=Object.assign({},opts,{resizeWidth:Math.max(1,Math.round(source.width*scale)),resizeHeight:Math.max(1,Math.round(source.height*scale)),resizeQuality:'high'});const t1=performance.now();try{image=await createImageBitmap(blob,resizeOpts);}catch(error){if(source.close)source.close();throw error;}addTexturePerf('resizeMs',performance.now()-t1);if(source.close)source.close();}const texture=new THREE.Texture(image);configureLoadedTexture(texture,samplerModes);return texture;};// Reads pixel dimensions straight out of an encoded image blob's header,
...
const readImageDimensions=async blob=>{try{const buf=new Uint8Array(await blob.slice(0,65536).arrayBuffer());if(buf.length<8)return null;// PNG: 8-byte signature, then an IHDR chunk with width/height at a
// fixed offset (big-endian uint32 each).
if(buf[0]===0x89&&buf[1]===0x50&&buf[2]===0x4e&&buf[3]===0x47){const dv=new DataView(buf.buffer,buf.byteOffset,buf.byteLength);if(buf.length>=24)return{width:dv.getUint32(16),height:dv.getUint32(20)};return null;}// JPEG: walk markers; skip APPn/COM/other segments by their length
// field, stop at the first SOFn (0..2) marker for width/height.
if(buf[0]===0xff&&buf[1]===0xd8){const dv=new DataView(buf.buffer,buf.byteOffset,buf.byteLength);let offset=2;while(offset+9<buf.length){if(buf[offset]!==0xff){offset+=1;continue;}const marker=buf[offset+1];if(marker===0xd8||marker===0x01||marker>=0xd0&&marker<=0xd7){offset+=2;continue;}if(marker===0xd9)break;// EOI
const segLen=dv.getUint16(offset+2);if(marker>=0xc0&&marker<=0xcf&&marker!==0xc4&&marker!==0xc8&&marker!==0xcc){return{height:dv.getUint16(offset+5),width:dv.getUint16(offset+7)};}offset+=2+segLen;}return null;}// KTX2: 12-byte identifier, then a little-endian header:
// vkFormat(4), typeSize(4), pixelWidth(4), pixelHeight(4), ...
const KTX2_IDENTIFIER=[0xab,0x4b,0x54,0x58,0x20,0x32,0x30,0xbb,0x0d,0x0a,0x1a,0x0a];if(buf.length>=44&&KTX2_IDENTIFIER.every((b,i)=>buf[i]===b)){const dv=new DataView(buf.buffer,buf.byteOffset,buf.byteLength);return{width:dv.getUint32(20,true),height:dv.getUint32(24,true)};}// TIFF: byte-order mark, then a 4-byte IFD offset, then the IFD
// entry count and entries; tags 256 (width) and 257 (height) can be
// SHORT (3) or LONG (4).
const isLE=buf[0]===0x49&&buf[1]===0x49;const isBE=buf[0]===0x4d&&buf[1]===0x4d;if(isLE||isBE){const dv=new DataView(buf.buffer,buf.byteOffset,buf.byteLength);const ifdOffset=dv.getUint32(4,isLE);if(ifdOffset+2>buf.length)return null;const count=dv.getUint16(ifdOffset,isLE);let width=null,height=null;for(let i=0;i<count;i+=1){const entryOffset=ifdOffset+2+i*12;if(entryOffset+12>buf.length)break;const tag=dv.getUint16(entryOffset,isLE);const type=dv.getUint16(entryOffset+2,isLE);const value=type===3?dv.getUint16(entryOffset+8,isLE):dv.getUint32(entryOffset+8,isLE);if(tag===256)width=value;else if(tag===257)height=value;}return width!=null&&height!=null?{width,height}:null;}// OpenEXR: magic 0x76, 0x2f, 0x31, 0x01, then a version int, then a
...
if(buf[0]===0x76&&buf[1]===0x2f&&buf[2]===0x31&&buf[3]===0x01){const dv=new DataView(buf.buffer,buf.byteOffset,buf.byteLength);let offset=8;const readCString=()=>{const start=offset;while(offset<buf.length&&buf[offset]!==0)offset+=1;const str=String.fromCharCode.apply(null,buf.subarray(start,offset));offset+=1;return str;};while(offset<buf.length){const name=readCString();if(!name)break;const type=readCString();if(offset+4>buf.length)break;const size=dv.getUint32(offset,true);offset+=4;if(name==='dataWindow'&&type==='box2i'&&offset+16<=buf.length){const xMin=dv.getInt32(offset,true),yMin=dv.getInt32(offset+4,true);const xMax=dv.getInt32(offset+8,true),yMax=dv.getInt32(offset+12,true);return{width:xMax-xMin+1,height:yMax-yMin+1};}offset+=size;}return null;}// Radiance HDR: text header ending in a blank line, then a
// resolution line such as "-Y 1024 +X 2048".
if(buf[0]===0x23||String.fromCharCode(buf[0])==='#'){const text=String.fromCharCode.apply(null,buf.subarray(0,Math.min(buf.length,4096)));const m=text.match(/^[-+][XY]\s+(\d+)\s+[-+][XY]\s+(\d+)/m);if(m){const first=Number(m[1]),second=Number(m[2]);// "-Y h +X w" is the common orientation; a leading X line
// instead means the numbers are already width then height.
if(/^-Y|^\+Y/.test(text.match(/^[-+][XY]\s+\d+\s+[-+][XY]\s+\d+/m)[0])){return{width:second,height:first};}return{width:first,height:second};}return null;}return null;}catch(e){return null;}};// Resizes a decoded scene texture that came from an unbounded format loader
...
const bindDroppedTextures=(view,fileMap,onBound)=>{if(view&&typeof view.onDisplacementFileMap==='function')view.onDisplacementFileMap(fileMap);const bound=[],missing=[],udimFirstTile=[];const pending=[];const cache=view.textureCache||TEXTURE_CACHE;const isAlive=()=>typeof view.isAlive!=='function'||view.isAlive();let ktx2Substituted=0;for(const u of view.introspected){if(u.type!=='filename')continue;let ref='';try{if(typeof u.data==='string')ref=u.data;else if(u.data!=null)ref=String(u.data);}catch(e){ref='';}if(!ref)continue;// no file reference recorded
let hit=findFileForRef(fileMap,ref);// A UDIM set has no single file; the shaderball's UVs live in the
// first tile, so bind the lowest-numbered tile instead of nothing.
if(!hit&&/<UDIM>/i.test(ref)){const tiles=findFilesForRef(fileMap,ref).sort((a,b)=>a.ref.localeCompare(b.ref));if(tiles.length){hit={key:tiles[0].key,how:'udim-first-tile'};udimFirstTile.push(ref);}}if(!hit){missing.push(ref);continue;}const originalHit=hit;hit=preferKtx2Sibling(fileMap,hit);if(hit.substituted)ktx2Substituted+=1;const blob=fileMap[hit.key];const samplerModes=u.samplerModes||null;const cacheKey=samplerCacheKey(textureCacheKey(blob,hit.key),samplerModes);const cached=cache.get(cacheKey);if(cached){if(isAlive()){if(view.uniforms[u.name])view.uniforms[u.name].value=cached;if(onBound)onBound();}}else{const ext=(hit.key.split('.').pop()||ref.split('.').pop()||'').toLowerCase();if(ext==='ktx2'){const bindTex=tex=>{if(!tex)return;configureLoadedTexture(tex,samplerModes);if(!isAlive()){tex.dispose&&tex.dispose();return;}cache.set(cacheKey,tex);if(view.uniforms[u.name])view.uniforms[u.name].value=tex;if(onBound)onBound();};const pendingLoad=loadKtx2Texture(blob,view,hit.key).then(bindTex,error=>{if(error&&error.ktx2InvalidBaseLevel&&originalHit.key!==hit.key){const notice=`KTX2 texture ${hit.key} is not a multiple of 4; falling back to ${originalHit.key}`;if(view.notices)view.notices.push(notice);return loadTextureForHit(originalHit,fileMap[originalHit.key],view,samplerModes).then(bindTex,e2=>({error:e2}));}return{error};});pending.push(pendingLoad);}else if(ext==='exr'||ext==='hdr'||ext==='tif'||ext==='tiff'){const startDecode=()=>ext==='exr'?loadExrTexture(blob):ext==='hdr'?loadHdrTexture(blob):loadTifTexture(blob,hit.key);const parsePromise=runHeavyTextureDecode(startDecode);const pendingLoad=parsePromise.then(tex=>{if(!tex)return;// unsupported/corrupt, the node default color stands
configureLoadedTexture(tex,samplerModes);if(!isAlive()){tex.dispose&&tex.dispose();return;}cache.set(cacheKey,tex);if(view.uniforms[u.name])view.uniforms[u.name].value=tex;if(onBound)onBound();},error=>{console.warn('mtlx-engine: texture decode failed for '+hit.key+', keeping the node default color:',error);missing.push(ref);return{error};});pending.push(pendingLoad);}else if(view.maxTextureSize){const startBoundedLoad=()=>{if(!isAlive())return Promise.resolve(null);return typeof createImageBitmap==='function'?loadBoundedBitmapTexture(blob,Number(view.maxTextureSize),samplerModes):Promise.reject(new Error('createImageBitmap is unavailable for bounded scene texture preview'));};let boundedLoad;// The queue is a small round-robin set of chains (one chain
...
if(view.textureQueue&&Array.isArray(view.textureQueue.tails)){const q=view.textureQueue;const slot=q.next%q.tails.length;q.next+=1;const previous=q.tails[slot];boundedLoad=previous.catch(()=>{}).then(startBoundedLoad);q.tails[slot]=boundedLoad;}else if(view.textureQueue&&view.textureQueue.tail){const previous=view.textureQueue.tail;boundedLoad=previous.catch(()=>{}).then(startBoundedLoad);view.textureQueue.tail=boundedLoad;}else boundedLoad=startBoundedLoad();pending.push(boundedLoad.then(tex=>{if(!tex)return;if(!isAlive()){tex.dispose&&tex.dispose();if(tex.image&&tex.image.close)tex.image.close();return;}cache.set(cacheKey,tex);if(view.uniforms[u.name])view.uniforms[u.name].value=tex;if(onBound)onBound();},error=>({error})));}else{const url=URL.createObjectURL(blob);pending.push(new Promise(resolve=>{new THREE.TextureLoader().load(url,tex=>{configureLoadedTexture(tex,samplerModes);if(!isAlive()){tex.dispose&&tex.dispose();URL.revokeObjectURL(url);resolve();return;}cache.set(cacheKey,tex);if(view.uniforms[u.name])view.uniforms[u.name].value=tex;URL.revokeObjectURL(url);if(onBound)onBound();resolve();},undefined,error=>{URL.revokeObjectURL(url);resolve({error});});}));}}bound.push(ref+'  →  '+hit.key);}if(ktx2Substituted>0)console.info('bindDroppedTextures: '+ktx2Substituted+' texture(s) loaded from .ktx2 sibling(s)');if(udimFirstTile.length)console.info('bindDroppedTextures: '+udimFirstTile.length+' UDIM reference(s) bound to their first tile for the preview');return{bound,missing,pending,ktx2Substituted,udimFirstTile};};// Extracts a plain JS array from a real array or an embind vector-like
...
const mxDataToPlainArray=d=>{if(Array.isArray(d))return d;if(d&&typeof d.data==='function'){try{return Array.from(d.data());}catch(e){/* not iterable */}}if(d&&typeof d.size==='function'){const o=[];for(let i=0;i<d.size();i++)o.push(d.get(i));return o;}return null;};// Enumerate a ShaderStage's uniforms via MaterialX introspection. `data`
...
const collectMxUniforms=stage=>{mxWarnIfLocked('collectMxUniforms');// exported doc-reading helper (per shader-gen, not per-frame), see mxWarnIfLocked's header comment
const out=[];const blocks=[];// { key, blk }
let blockMap=null;try{blockMap=stage.getUniformBlocks&&stage.getUniformBlocks();}catch(e){/* older binding */}if(blockMap){if(typeof blockMap.keys==='function'){for(const k of vecToArray(blockMap.keys())){try{blocks.push({key:String(k),blk:blockMap.get(k)});}catch(e){/* skip */}}}else{for(const k of Object.keys(blockMap))blocks.push({key:k,blk:blockMap[k]});}}else{// HW shader generators register exactly these two blocks
// (HW::PUBLIC_UNIFORMS / HW::PRIVATE_UNIFORMS).
for(const name of['PublicUniforms','PrivateUniforms']){try{const b=stage.getUniformBlock(name);if(b)blocks.push({key:name,blk:b});}catch(e){/* absent */}}}for(const entry of blocks){const b=entry.blk;let n=0;try{n=typeof b.size==='function'?b.size():0;}catch(e){/* skip block */}for(let i=0;i<n;i++){try{const v=b.get(i);const name=v.getVariable&&v.getVariable()||v.getName&&v.getName();if(!name)continue;let type=null;try{const t=v.getType&&v.getType();type=t?t.getName&&t.getName()||String(t):null;}catch(e){/* type unreadable */}let data=null;try{const val=v.getValue&&v.getValue();if(val&&val.getData)data=val.getData();}catch(e){/* no default recorded */}// The MaterialX element path (e.g. "preview_node/amplitude")
// ties the uniform back to a node input, used by the
// dynamic parameter UI.
let path=null;try{path=v.getPath&&v.getPath()||null;}catch(e){/* absent */}out.push({name,type,data,path,block:entry.key});}catch(e){/* skip unreadable entry */}}}return out;};// Types whose collectMxUniforms `data` may be a live embind heap-backed
// view, needing mxDataToPlainArray to detach it. Scalars and
// filename/string values already arrive as plain JS, untouched.
const VECTOR_MX_TYPES=new Set(['vector2','vector3','vector4','color3','color4','matrix33','matrix44']);// Converts ONE collectMxUniforms() entry's `data` to plain JS, must run
...
const plainizeMxUniformData=u=>{if(u.data==null||!VECTOR_MX_TYPES.has(u.type))return u;return Object.assign({},u,{data:mxDataToPlainArray(u.data)});};const annotateFilenameSamplerModes=(introspected,doc)=>{const modes=collectImageSamplerModes(doc);if(!modes.size)return introspected;const uniqueLeaves=modes.uniqueLeaves||new Map();return introspected.map(u=>{if(!u||u.type!=='filename')return u;const pathParts=u.path?String(u.path).split(/[/\.:]/).filter(Boolean):[];// Prefer the longest qualified suffix from the introspected path.
// The leading document/material prefix can differ between bindings.
for(let start=0;start<pathParts.length;start+=1){for(let end=pathParts.length;end>start;end-=1){const candidate=pathParts.slice(start,end).join('/');const samplerModes=modes.get(candidate);if(samplerModes)return Object.assign({},u,{samplerModes});}}const leaf=u.name?String(u.name).replace(/_file(?:_.*)?$/,''):'';const samplerModes=uniqueLeaves.get(leaf);return samplerModes?Object.assign({},u,{samplerModes}):u;});};// Converts a MaterialX default value into a three.js uniform. Returns
...
const mxValueToThreeUniform=(type,data)=>{const arr=mxDataToPlainArray;switch(type){case'float':{const n=Number(data);return{value:isNaN(n)?0:n};}case'integer':{const n=Number(data);return{value:isNaN(n)?0:n|0};}case'boolean':return{value:!!data};case'vector2':{const a=arr(data)||[0,0];return{value:new THREE.Vector2(a[0],a[1])};}case'color3':case'vector3':{const a=arr(data)||[0,0,0];return{value:new THREE.Vector3(a[0],a[1],a[2])};}case'color4':case'vector4':{const a=arr(data)||[0,0,0,0];return{value:new THREE.Vector4(a[0],a[1],a[2],a[3])};}case'matrix33':{const a=arr(data);const m=new THREE.Matrix3();if(a&&a.length===9)m.fromArray(a);return{value:m};}case'matrix44':{const a=arr(data);const m=new THREE.Matrix4();if(a&&a.length===16)m.fromArray(a);return{value:m};}default:return null;}};// The parameter UI's color picker speaks LINEAR, like MaterialX itself:
...
const linToSrgb=c=>{const x=Math.max(0,Math.min(1,c));return x<=0.0031308?x*12.92:1.055*Math.pow(x,1/2.4)-0.055;};const srgbToLin=c=>c<=0.04045?c/12.92:Math.pow((c+0.055)/1.055,2.4);const rgbToHex=rgb=>'#'+rgb.slice(0,3).map(c=>{const h=Math.round(Math.max(0,Math.min(1,Number(c)||0))*255).toString(16);return h.length===1?'0'+h:h;}).join('');const hexToRgb=hex=>[1,3,5].map(i=>parseInt(hex.slice(i,i+2),16)/255);// MaterialXView parity: the generated GLSL takes `defaultval` and never
...
const DEFAULT_VALUE_TEXTURES=new Map();// Membership lives here, not on the texture: r128's Texture has no
// userData (it ends at onUpdate), so tagging one throws.
const DEFAULT_VALUE_TEXTURE_SET=new WeakSet();const defaultValueToRgba=(type,data)=>{if(type==='float'){const n=Number(data);return isNaN(n)?null:[n,n,n,1];}const a=mxDataToPlainArray(data);if(!a)return null;const c=i=>Number(a[i])||0;switch(type){case'vector2':return[c(0),c(1),0,1];case'color3':case'vector3':return[c(0),c(1),c(2),1];case'color4':case'vector4':return[c(0),c(1),c(2),a[3]==null?1:c(3)];default:return null;}};// Mirrors ImageSamplingProperties::setProperties: strip the sampler's
// trailing `_file` and read the sibling `_default`, or `_default_cm_in`
// when a colorspace on the image node renamed it.
const getFilenameDefaultTexture=(introspected,samplerName)=>{const cut=samplerName.lastIndexOf('_');if(cut<=0)return null;const root=samplerName.slice(0,cut);let port=null;for(const u of introspected){if(u.name===root+'_default'){port=u;break;}if(u.name===root+'_default_cm_in'&&!port)port=u;}if(!port||port.data==null)return null;const rgba=defaultValueToRgba(port.type,port.data);if(!rgba)return null;return defaultValueTexture(rgba);};// Upstream's own caveat rides along: the default is assumed to be in the
...
const defaultValueTexture=rgba=>{const key=rgba.join(',');const hit=DEFAULT_VALUE_TEXTURES.get(key);if(hit)return hit;const t=new THREE.DataTexture(new Float32Array(rgba),1,1,THREE.RGBAFormat,THREE.FloatType);t.minFilter=t.magFilter=THREE.NearestFilter;t.wrapS=t.wrapT=THREE.ClampToEdgeWrapping;t.needsUpdate=true;DEFAULT_VALUE_TEXTURE_SET.add(t);DEFAULT_VALUE_TEXTURES.set(key,t);return t;};const isFilenameDefaultTexture=t=>!!t&&DEFAULT_VALUE_TEXTURE_SET.has(t);// A sampler still showing our bake may be re-baked; one holding a real
// image must not be. Null counts as ours (a node with no default).
const samplerHoldsDefault=slot=>!!slot&&(!slot.value||isFilenameDefaultTexture(slot.value));// The generated GLSL ignores `defaultval`, so the sampler's 1x1 texture is
...
const rebindFilenameDefault=(uniforms,defaultUniformName,type,value)=>{const m=/^(.*)_default(?:_cm_in)?$/.exec(defaultUniformName||'');if(!m)return false;const slot=uniforms?uniforms[m[1]+'_file']:null;if(!samplerHoldsDefault(slot))return false;const rgba=defaultValueToRgba(type,value);if(!rgba)return false;slot.value=defaultValueTexture(rgba);return true;};// Configure a user-loaded texture the way the generated shaders expect
// to sample a `filename` input: repeat wrapping, no flipY, anisotropic
// filtering (three clamps to the device max at upload).
const configureLoadedTexture=(t,samplerModes)=>{const modes=samplerModes||{u:'periodic',v:'periodic'};const wrap=mode=>{switch(normalizeSamplerAddressMode(mode)){case'clamp':return THREE.ClampToEdgeWrapping;case'mirror':return THREE.MirroredRepeatWrapping;default:return THREE.RepeatWrapping;}};t.wrapS=wrap(modes.u);t.wrapT=wrap(modes.v);t.flipY=false;t.anisotropy=8;t.needsUpdate=true;return t;};// ---- Preview geometry ----
...
const UV_GEOMPROP_ALIASES=['i_geomprop_st','i_geomprop_uv','i_geomprop_UV0','i_geomprop_st0','i_geomprop_uv0','i_geomprop_map1'];const aliasUvGeomprops=geometry=>{const uv=geometry.getAttribute('uv');if(!uv)return;for(const name of UV_GEOMPROP_ALIASES){if(!geometry.getAttribute(name))geometry.setAttribute(name,uv);}};const prepGeometry=geometry=>{// Already prepped (e.g. a cached shaderball clone), skip re-running
...
if(geometry.getAttribute('i_tangent')&&geometry.getAttribute('i_bitangent')){aliasUvGeomprops(geometry);return geometry;}aliasUvGeomprops(geometry);const position=geometry.getAttribute('position');if(!position)return geometry;if(!geometry.getAttribute('normal'))geometry.computeVertexNormals();if(!geometry.getAttribute('uv')){// MaterialX shaders read texcoords; give degenerate UVs
// rather than an unbound attribute.
const count=position.count;geometry.setAttribute('uv',new THREE.BufferAttribute(new Float32Array(count*2),2));}aliasUvGeomprops(geometry);geometry.setAttribute('i_position',geometry.getAttribute('position'));geometry.setAttribute('i_normal',geometry.getAttribute('normal'));geometry.setAttribute('i_texcoord_0',geometry.getAttribute('uv'));let iTangent=null,iBitangent=null;const normal=geometry.getAttribute('normal');const writeFrame=(index,tx,ty,tz,sign,tangentOut,bitangentOut)=>{let nx=normal.getX(index),ny=normal.getY(index),nz=normal.getZ(index);const nlen=Math.hypot(nx,ny,nz);if(nlen>1e-10){nx/=nlen;ny/=nlen;nz/=nlen;}else{nx=0;ny=0;nz=1;}const ndot=tx*nx+ty*ny+tz*nz;tx-=ndot*nx;ty-=ndot*ny;tz-=ndot*nz;let tlen=Math.hypot(tx,ty,tz);if(tlen<1e-10){// Pick an axis least parallel to N, then form an orthogonal T.
const ax=Math.abs(nx)<0.9?1:0;const ay=ax?0:1;tx=ay*nz;ty=ax*nz;tz=-ay*nx-ax*ny;tlen=Math.hypot(tx,ty,tz)||1;}tx/=tlen;ty/=tlen;tz/=tlen;const bx=(ny*tz-nz*ty)*(sign<0?-1:1);const by=(nz*tx-nx*tz)*(sign<0?-1:1);const bz=(nx*ty-ny*tx)*(sign<0?-1:1);tangentOut[index*3]=tx;tangentOut[index*3+1]=ty;tangentOut[index*3+2]=tz;bitangentOut[index*3]=bx;bitangentOut[index*3+1]=by;bitangentOut[index*3+2]=bz;};// Prefer Three's source tangent (vec4) when a legacy alias is already
// present, so its handedness survives repair of the missing bitangent.
const tangent=geometry.getAttribute('tangent')||geometry.getAttribute('i_tangent');if(tangent){// Repair a geometry carrying the legacy 3-component tangent alias.
// A supplied vec4 tangent retains the authored/Three handedness in w.
const tangents=new Float32Array(position.count*3);const bitangents=new Float32Array(position.count*3);for(let i=0;i<position.count;i++){const tx=tangent.getX(i),ty=tangent.getY(i),tz=tangent.getZ(i);const sign=tangent.itemSize>=4?tangent.getW(i)<0?-1:1:1;writeFrame(i,tx,ty,tz,sign,tangents,bitangents);}iTangent=new THREE.BufferAttribute(tangents,3);iBitangent=new THREE.BufferAttribute(bitangents,3);}// r128's computeTangents CONSOLE.ERRORs (not throws) when
...
const canTangent=!!(geometry.getIndex()&&position&&normal&&geometry.getAttribute('uv'));if(!iTangent&&canTangent){try{geometry.computeTangents();const t=geometry.getAttribute('tangent');// vec4 (may be absent on silent failure)
if(t){const tri=new Float32Array(t.count*3);const signs=new Float32Array(t.count);for(let i=0;i<t.count;i++){tri[i*3]=t.getX(i);tri[i*3+1]=t.getY(i);tri[i*3+2]=t.getZ(i);signs[i]=t.itemSize>=4&&t.getW(i)<0?-1:1;}// Zero-UV-area triangles (and the sphere's poles) leave
...
const nrm=geometry.getAttribute('normal');let repaired=0;for(let i=0;i<t.count;i++){const x=tri[i*3],y=tri[i*3+1],z=tri[i*3+2];if(x*x+y*y+z*z<1e-10){const nx=nrm.getX(i),ny=nrm.getY(i),nz=nrm.getZ(i);const ax=Math.abs(nx)<0.9?1:0,ay=ax?0:1;let cx=-nz*ay,cy=nz*ax,cz=nx*ay-ny*ax;const len=Math.sqrt(cx*cx+cy*cy+cz*cz)||1;tri[i*3]=cx/len;tri[i*3+1]=cy/len;tri[i*3+2]=cz/len;repaired++;}}if(repaired)console.warn('[mtlx] repaired '+repaired+' degenerate tangents (zero-UV-area triangles) on geometry');const bitri=new Float32Array(t.count*3);for(let i=0;i<t.count;i++)writeFrame(i,tri[i*3],tri[i*3+1],tri[i*3+2],signs[i],tri,bitri);iTangent=new THREE.BufferAttribute(tri,3);iBitangent=new THREE.BufferAttribute(bitri,3);}}catch(e){/* fall through to constant tangent */}}// Three's computeTangents requires an index. For non-indexed USD meshes,
...
if(!iTangent&&!geometry.getIndex()&&position.count>=3){const uv=geometry.getAttribute('uv');const tangents=new Float32Array(position.count*3);const bitangents=new Float32Array(position.count*3);for(let base=0;base+2<position.count;base+=3){const ax=position.getX(base+1)-position.getX(base);const ay=position.getY(base+1)-position.getY(base);const az=position.getZ(base+1)-position.getZ(base);const bx=position.getX(base+2)-position.getX(base);const by=position.getY(base+2)-position.getY(base);const bz=position.getZ(base+2)-position.getZ(base);const du1=uv.getX(base+1)-uv.getX(base),dv1=uv.getY(base+1)-uv.getY(base);const du2=uv.getX(base+2)-uv.getX(base),dv2=uv.getY(base+2)-uv.getY(base);const det=du1*dv2-du2*dv1;if(Math.abs(det)<1e-10)continue;const inv=1/det;const tx=(ax*dv2-bx*dv1)*inv;const ty=(ay*dv2-by*dv1)*inv;const tz=(az*dv2-bz*dv1)*inv;const cx=(bx*du1-ax*du2)*inv;const cy=(by*du1-ay*du2)*inv;const cz=(bz*du1-az*du2)*inv;for(let j=0;j<3;j++){const i=base+j;tangents[i*3]=tx;tangents[i*3+1]=ty;tangents[i*3+2]=tz;bitangents[i*3]=cx;bitangents[i*3+1]=cy;bitangents[i*3+2]=cz;}}for(let i=0;i<position.count;i++){const tx=tangents[i*3],ty=tangents[i*3+1],tz=tangents[i*3+2];const nx=normal.getX(i),ny=normal.getY(i),nz=normal.getZ(i);const bx=ny*tz-nz*ty,by=nz*tx-nx*tz,bz=nx*ty-ny*tx;const sign=bx*bitangents[i*3]+by*bitangents[i*3+1]+bz*bitangents[i*3+2]<0?-1:1;writeFrame(i,tx,ty,tz,sign,tangents,bitangents);}iTangent=new THREE.BufferAttribute(tangents,3);iBitangent=new THREE.BufferAttribute(bitangents,3);}if(!iTangent){const vcount=position.count;const tangents=new Float32Array(vcount*3);const bitangents=new Float32Array(vcount*3);for(let i=0;i<vcount;i++){writeFrame(i,1,0,0,1,tangents,bitangents);}iTangent=new THREE.BufferAttribute(tangents,3);iBitangent=new THREE.BufferAttribute(bitangents,3);}geometry.setAttribute('i_tangent',iTangent);geometry.setAttribute('i_bitangent',iBitangent);return geometry;};// Sizes for the geomprop types this viewer can zero-fill. patchGeompropVaryings
...
const GEOMPROP_ITEM_SIZE={float:1,vec2:2,vec3:3,vec4:4};// Binds each declared geompropvalue vertex input that the geometry does not
...
const bindGeompropAttributes=(geometry,geomprops,notify,constants=null)=>{if(!geometry||!geomprops||!geomprops.length)return geometry;const uv=geometry.getAttribute('uv');for(const{name,type,defaultValue}of geomprops){const attrName='i_geomprop_'+name;if(geometry.getAttribute(attrName))continue;if(type==='vec2'&&uv){geometry.setAttribute(attrName,uv);continue;}const itemSize=GEOMPROP_ITEM_SIZE[type];// A constant the stage supplied wins, then the node's authored
// default; only a stream we know nothing about stays at zero.
const constant=constants&&constants[name];const fill=Array.isArray(constant)?constant:Array.isArray(defaultValue)?defaultValue:null;let filled='';if(itemSize){const count=geometry.getAttribute('position')?geometry.getAttribute('position').count:0;const data=new Float32Array(count*itemSize);if(fill&&fill.length){for(let i=0;i<count;i+=1){for(let c=0;c<itemSize;c+=1)data[i*itemSize+c]=Number(fill[Math.min(c,fill.length-1)])||0;}filled=fill.slice(0,itemSize).join(', ');}geometry.setAttribute(attrName,new THREE.BufferAttribute(data,itemSize));}if(typeof notify==='function'){notify(filled?`geompropvalue "${name}" (${type}) has no geometry stream in this viewer and reads ${filled}`:`geompropvalue "${name}" (${type}) has no geometry stream in this viewer and reads zeros, so this material will not look as authored`);}}return geometry;};// Center a geometry at the origin and scale it to bounding radius 1
...
const CUSTOM_GEOM={geometry:null,name:'',epoch:0};// Latest loadCustomPreviewGeomFromFile/Url call wins; bumped by both and by clearCustomPreviewGeom.
...
const GLOBAL_GEOM_KEY='mtlx_geom_global';const GLOBAL_GEOM_VALUES=['shaderball-scene','shaderball','shaderball-mtlx','sphere','cube','cloth','buffer2d','custom'];// Old per-tool keys, read once as a seed when the global key has never been written.
const LEGACY_GEOM_KEYS=['mtlx_preview_geom_choice','mtlx_graph_preview_geom'];const LEGACY_GEOM_SKIP=['custom','default','pernode'];let MTLX_GLOBAL_GEOM=null;// Runs once, on first getGlobalGeom/setGlobalGeom call.
const initGlobalGeom=()=>{let stored=null;try{stored=localStorage.getItem(GLOBAL_GEOM_KEY);}catch(e){/* privacy mode */}if(stored&&GLOBAL_GEOM_VALUES.includes(stored)&&stored!=='custom'){MTLX_GLOBAL_GEOM=stored;return;}if(!stored){for(const key of LEGACY_GEOM_KEYS){let legacy=null;try{legacy=localStorage.getItem(key);}catch(e){/* privacy mode */}if(legacy&&GLOBAL_GEOM_VALUES.includes(legacy)&&!LEGACY_GEOM_SKIP.includes(legacy)){MTLX_GLOBAL_GEOM=legacy;return;}}}MTLX_GLOBAL_GEOM='shaderball-scene';};const getGlobalGeom=()=>{if(MTLX_GLOBAL_GEOM===null)initGlobalGeom();return MTLX_GLOBAL_GEOM;};// Persists only from the top-realm page (embeds must not clobber the host's
// choice) and only for concrete values (custom is registry-local, session-only).
const setGlobalGeom=value=>{if(MTLX_GLOBAL_GEOM===null)initGlobalGeom();if(!GLOBAL_GEOM_VALUES.includes(value)||value===MTLX_GLOBAL_GEOM)return;MTLX_GLOBAL_GEOM=value;if(window.self===window.top&&value!=='custom'){try{localStorage.setItem(GLOBAL_GEOM_KEY,value);}catch(e){/* privacy mode */}}window.dispatchEvent(new CustomEvent('mtlx-global-geom',{detail:{value}}));};// ---- Global display transform selection (shared across every tool) ----
...
const DISPLAY_TRANSFORM_KEY='mtlx_display_transform';const DISPLAY_TRANSFORM_VALUES=['srgb','aces','neutral','lin_rec709'];// Camera exposure in stops, shared by every view. Unlike the transform this is
...
const broadcastDisplaySettings=()=>{LIVE_VIEWS.forEach(v=>{try{v.refreshDisplaySettings&&v.refreshDisplaySettings();}catch(e){/* view mid-teardown */}});};const DISPLAY_EXPOSURE_KEY='mtlx_display_exposure';let MTLX_DISPLAY_EXPOSURE=null;const initDisplayExposure=()=>{let stored=null;try{stored=localStorage.getItem(DISPLAY_EXPOSURE_KEY);}catch(e){/* privacy mode */}const ev=Number(stored);MTLX_DISPLAY_EXPOSURE=stored!=null&&stored!==''&&Number.isFinite(ev)?Math.max(-8,Math.min(8,ev)):0;};const getDisplayExposure=()=>{if(MTLX_DISPLAY_EXPOSURE===null)initDisplayExposure();return MTLX_DISPLAY_EXPOSURE;};// Linear scale for the uniform. Every seeding site goes through this so the
// stops-to-linear conversion cannot drift between them.
const displayExposureScale=()=>Math.pow(2,getDisplayExposure());const setDisplayExposure=ev=>{if(MTLX_DISPLAY_EXPOSURE===null)initDisplayExposure();const next=Math.max(-8,Math.min(8,Number(ev)||0));if(next===MTLX_DISPLAY_EXPOSURE)return;MTLX_DISPLAY_EXPOSURE=next;if(window.self===window.top){try{localStorage.setItem(DISPLAY_EXPOSURE_KEY,String(next));}catch(e){/* privacy mode */}}// Broadcast rather than rely on per-app listeners: every render view is a
...
const initDisplayTransform=()=>{let stored=null;try{stored=localStorage.getItem(DISPLAY_TRANSFORM_KEY);}catch(e){/* privacy mode */}MTLX_DISPLAY_TRANSFORM=DISPLAY_TRANSFORM_VALUES.includes(stored)?stored:'srgb';};// Exposed so a view that keeps its own transform (the Scene) can validate a
...
const commitCustomGeom=(root,label,seqId)=>{const built=buildCustomGeometryFromRoot(root,label);if(seqId!==customGeomLoadSeq){try{built.dispose();}catch(e){/* registry copy is never GPU-uploaded */}return null;}const prev=CUSTOM_GEOM.geometry;CUSTOM_GEOM.geometry=built;CUSTOM_GEOM.name=String(label||'custom');CUSTOM_GEOM.epoch+=1;if(prev){try{prev.dispose();}catch(e){/* registry copy is never GPU-uploaded */}}// Keep-alive hidden preview tools subscribe to this event to mirror the
...
const group=gltf.scene.clone(true);let glbCamera=null;let surfaceMesh=null;const ownedMaterials=[];group.traverse(obj=>{if(mode==='full'&&obj.isCamera&&!glbCamera){glbCamera=obj;return;}if(!obj.isMesh)return;if(obj.name==='material_surface'){// The generated MaterialX material lands here (both GLBs
// author this primitive with a NULL material),
// createMtlxRenderView assigns it via applyMaterialInternal.
surfaceMesh=obj;return;}if(/^backplane/.test(obj.name)){// Emitter panels: NULL glTF material + baked vertex COLOR_0,
// self-lit "light card" look. toneMapped:true keeps them on
// the same ACES curve as the MaterialX surface (encodeDisplay).
const m=new THREE.MeshBasicMaterial({vertexColors:true,toneMapped:true});obj.material=m;ownedMaterials.push(m);return;}if(obj.material){// Every other glTF-materialed mesh: clone() so this view
// OWNS its material instance, without it, setEnvExposure's
// envMapIntensity mutation would leak across cached views.
const wasArray=Array.isArray(obj.material);const clones=(wasArray?obj.material:[obj.material]).map(m=>m.clone());obj.material=wasArray?clones:clones[0];ownedMaterials.push(...clones);}});if(!surfaceMesh)return null;// Per-view geometry clone: prepGeometry MUTATES the geometry (adds
// i_position/i_normal/etc aliases), clone first so the cache's
// original geometry stays pristine for other views.
surfaceMesh.geometry=prepGeometry(surfaceMesh.geometry.clone());if(mode==='simple'){// Whole-scene analog of normalizeGeometry: centers the bounding
...
const bs=new THREE.Box3().setFromObject(group).getBoundingSphere(new THREE.Sphere());const outer=new THREE.Group();outer.add(group);if(bs.radius>0){const s=1/bs.radius;outer.scale.setScalar(s);outer.position.copy(bs.center).multiplyScalar(-s);}return{group:outer,surfaceMesh,glbCamera:null,ownedMaterials};}return{group,surfaceMesh,glbCamera,ownedMaterials};};// The MaterialX project's shader ball (models/shaderball_mtlx.glb, see
...
return CUSTOM_GEOM.geometry.clone();}return new THREE.SphereGeometry(1,64,64);};// Resolves how to preview a node from its nodedefs: handles overloaded
// defs and MULTI-OUTPUT defs (picks the first viewable output). Returns
// { kind, outType, outputName, multiOutput }.
const COLOR_VIEWABLE=['color3','color4','float','vector2','vector3','vector4'];// `defFilter` (optional) narrows matching nodedefs, categories aren't
...
const surf=candidates.find(c=>c.type==='surfaceshader');if(surf)return{kind:'surface',...surf};const bsdf=candidates.find(c=>c.type==='BSDF');if(bsdf)return{kind:'bsdf',...bsdf};const edf=candidates.find(c=>c.type==='EDF');if(edf)return{kind:'edf',...edf};for(const t of COLOR_VIEWABLE){const hit=candidates.find(c=>c.type===t);if(hit)return{kind:'color',outType:t,outputName:hit.outputName,multiOutput:hit.multiOutput};}return{kind:null,types:allTypes};};// Synthesizes a small equirect environment (LDR, filter/mip-safe): a
// sky-to-ground gradient with a soft overhead "sun" for speculars.
// Keeps the viewer self-contained when no HDR is loaded.
const makeEnvTexture=(w,h,blurred)=>{const data=new Uint8Array(w*h*4);const sky=[150,190,235],horizon=[225,225,220],ground=[70,66,60];for(let y=0;y<h;y++){const v=y/(h-1);// 0 top .. 1 bottom
for(let x=0;x<w;x++){let r,g,b;if(v<0.5){const t=v/0.5;r=sky[0]+(horizon[0]-sky[0])*t;g=sky[1]+(horizon[1]-sky[1])*t;b=sky[2]+(horizon[2]-sky[2])*t;}else{const t=(v-0.5)/0.5;r=horizon[0]+(ground[0]-horizon[0])*t;g=horizon[1]+(ground[1]-horizon[1])*t;b=horizon[2]+(ground[2]-horizon[2])*t;}if(!blurred){// soft sun highlight near the top-center
const u=x/(w-1);const d=Math.hypot(u-0.5,v-0.18);const sun=Math.max(0,1-d/0.16);const s=sun*sun*255;r=Math.min(255,r+s);g=Math.min(255,g+s);b=Math.min(255,b+s);}const i=(y*w+x)*4;data[i]=r;data[i+1]=g;data[i+2]=b;data[i+3]=255;}}const tex=new THREE.DataTexture(data,w,h,THREE.RGBAFormat);// Equirect mapping is irrelevant to the IBL sampler; the skybox gets
// its own copy via makeBackgroundTexture (see env-prep header above).
tex.mapping=THREE.EquirectangularReflectionMapping;tex.wrapS=THREE.RepeatWrapping;tex.wrapT=THREE.ClampToEdgeWrapping;tex.minFilter=blurred?THREE.LinearFilter:THREE.LinearMipmapLinearFilter;tex.magFilter=THREE.LinearFilter;tex.generateMipmaps=!blurred;tex.needsUpdate=true;return tex;};// Path to the app's default equirect environment: a studio EXR, parsed
...
const KEYLIGHT_STORAGE_KEY='mtlx_env_keylight';let keyLightEnabled=true;try{const saved=localStorage.getItem(KEYLIGHT_STORAGE_KEY);if(saved!==null)keyLightEnabled=saved!=='0';}catch(e){/* localStorage unavailable, default stays on */}// Pristine (pre-extraction) bytes behind the default/override env, so
// the toggle can re-parse + rebuild without a re-fetch/re-drop.
let defaultEnvSource=null,overrideEnvSource=null;// Registry of live render-view handles, so environment imports/resets
...
const registerLiveView=handle=>{if(handle)LIVE_VIEWS.add(handle);};const unregisterLiveView=handle=>{if(handle)LIVE_VIEWS.delete(handle);};// ---- Environment preparation: OFFICIAL VIEWER PARITY ----
...
const padToRGBA=tex=>{const img=tex.image;if(!img||!img.data)return tex;const n=img.width*img.height;if(img.data.length>=n*4)return tex;// already RGBA
const C=img.data.constructor;const out=new C(n*4);const one=C===Uint16Array?0x3C00/* half 1.0 */:1.0;for(let i=0;i<n;i++){out[i*4]=img.data[i*3];out[i*4+1]=img.data[i*3+1];out[i*4+2]=img.data[i*3+2];out[i*4+3]=one;}const t=new THREE.DataTexture(out,img.width,img.height,THREE.RGBAFormat,tex.type);t.flipY=tex.flipY;return t;};const prepareEnv=tex=>{const t=padToRGBA(tex);t.mapping=THREE.EquirectangularReflectionMapping;t.wrapS=THREE.RepeatWrapping;t.wrapT=THREE.ClampToEdgeWrapping;t.minFilter=THREE.LinearMipmapLinearFilter;t.magFilter=THREE.LinearFilter;t.generateMipmaps=true;t.anisotropy=8;// three clamps to the device max at upload
t.encoding=THREE.LinearEncoding;t.needsUpdate=true;return t;};// Builds the skybox mesh's visible backdrop from a prepared radiance
...
const makeBackgroundTexture=src=>{const img=src.image;const bg=new THREE.DataTexture(img.data,img.width,img.height,src.format,src.type);bg.flipY=!src.flipY;// skybox sphere needs the opposite V orientation of the IBL texture
bg.mapping=THREE.EquirectangularReflectionMapping;bg.wrapS=THREE.RepeatWrapping;bg.wrapT=THREE.ClampToEdgeWrapping;// Sampled directly by the skybox mesh, no mip chain needed.
bg.minFilter=THREE.LinearFilter;bg.magFilter=THREE.LinearFilter;bg.generateMipmaps=false;bg.encoding=src.encoding;bg.needsUpdate=true;return bg;};// IEEE-754 float32 → float16 (for building half-float DataTextures).
const _f32=new Float32Array(1);const _u32=new Uint32Array(_f32.buffer);const floatToHalf=val=>{_f32[0]=val;const x=_u32[0];const sign=x>>16&0x8000;const exp=(x>>23&0xFF)-127+15;if(exp<=0)return sign;// underflow → signed 0
if(exp>=31)return sign|0x7BFF;// clamp to max half
return sign|exp<<10|(x&0x7FFFFF)>>13;};// r128's toHalfFloat does not clamp: a finite float above the half
...
const sanitizeHalfEnvData=(data,stride)=>{let replaced=0;for(let i=0;i<data.length;i+=stride){for(let c=0;c<3;c++){const h=data[i+c];if((h&0x7C00)===0x7C00){data[i+c]=0x7BFF;replaced++;}else if(h&0x8000){data[i+c]=0;replaced++;}}if(stride===4)data[i+3]=0x3C00;// half 1.0: an EXR's own alpha must not reach the backdrop
}return replaced;};const halfToFloat=h=>{const sign=h&0x8000?-1:1;const exp=h>>10&0x1F;const frac=h&0x3FF;if(exp===0)return sign*frac*Math.pow(2,-24);if(exp===31)return frac?NaN:sign*Infinity;return sign*(1+frac/1024)*Math.pow(2,exp-15);};// True SH (l<=2) cosine-convolution irradiance (Ramamoorthi & Hanrahan
...
const shIrradianceFromEquirect=tex=>{try{const srcImg=tex.image;const srcStride=srcImg.data.length/(srcImg.width*srcImg.height);// 3 or 4
const srcIsHalf=srcImg.data.constructor===Uint16Array;const readPx=idx=>[srcIsHalf?halfToFloat(srcImg.data[idx]):srcImg.data[idx],srcIsHalf?halfToFloat(srcImg.data[idx+1]):srcImg.data[idx+1],srcIsHalf?halfToFloat(srcImg.data[idx+2]):srcImg.data[idx+2]];// Pass 0: pre-downsample box-average to a float buffer, capping
// the Pass 1 projection loop below at <=128x64 texels regardless
// of source size.
let W=srcImg.width,H=srcImg.height,get;if(W>128||H>64){const dW=Math.min(W,128),dH=Math.min(H,64);const bx=Math.max(1,Math.floor(W/dW));const by=Math.max(1,Math.floor(H/dH));const buf=new Float32Array(dW*dH*3);for(let y=0;y<dH;y++){for(let x=0;x<dW;x++){let r=0,g=0,b=0,cnt=0;for(let oy=0;oy<by;oy++){for(let ox=0;ox<bx;ox++){const spx=x*bx+ox,spy=y*by+oy;if(spx>=W||spy>=H)continue;const px=readPx((spy*W+spx)*srcStride);r+=px[0];g+=px[1];b+=px[2];cnt++;}}const o=(y*dW+x)*3;buf[o]=r/cnt;buf[o+1]=g/cnt;buf[o+2]=b/cnt;}}W=dW;H=dH;get=(x,y)=>{const o=(y*W+x)*3;return[buf[o],buf[o+1],buf[o+2]];};}else{get=(x,y)=>readPx((y*W+x)*srcStride);}// Pass 1: project radiance onto the 9 SH basis functions,
...
for(let y=0;y<H;y++){const theta=Math.PI*(y+0.5)/H;const sinT=Math.sin(theta),cosT=Math.cos(theta);const dOmega=2*Math.PI/W*(Math.PI/H)*sinT;for(let x=0;x<W;x++){const phi=2*Math.PI*(x+0.5)/W;const sx=sinT*Math.cos(phi),sy=cosT,sz=sinT*Math.sin(phi);const[r,g,b]=get(x,y);const Y=[0.282095,// Y00
0.488603*sz,// Y1-1
...
];for(let i=0;i<9;i++){const yw=Y[i]*dOmega;c[i*3]+=r*yw;c[i*3+1]+=g*yw;c[i*3+2]+=b*yw;}}}// Pass 2: evaluate cosine-convolved irradiance per output texel
...
const OW=64,OH=32;const A0=Math.PI,A1=2*Math.PI/3,A2=Math.PI/4;const A=[A0,A1,A1,A1,A2,A2,A2,A2,A2];const out=new Uint16Array(OW*OH*4);for(let y=0;y<OH;y++){const theta=Math.PI*(y+0.5)/OH;const sinT=Math.sin(theta),cosT=Math.cos(theta);for(let x=0;x<OW;x++){const phi=2*Math.PI*(x+0.5)/OW;const sx=sinT*Math.cos(phi),sy=cosT,sz=sinT*Math.sin(phi);const Y=[0.282095,0.488603*sz,0.488603*sy,0.488603*sx,1.092548*sx*sz,1.092548*sz*sy,1.092548*sx*sy,0.315392*(3*sy*sy-1),0.546274*(sx*sx-sz*sz)];let r=0,g=0,b=0;for(let i=0;i<9;i++){const aw=A[i]*Y[i];r+=aw*c[i*3];g+=aw*c[i*3+1];b+=aw*c[i*3+2];}r=Number.isFinite(r)?Math.max(0,r/Math.PI):0;g=Number.isFinite(g)?Math.max(0,g/Math.PI):0;b=Number.isFinite(b)?Math.max(0,b/Math.PI):0;const o=(y*OW+x)*4;out[o]=floatToHalf(r);out[o+1]=floatToHalf(g);out[o+2]=floatToHalf(b);out[o+3]=0x3C00;// half 1.0, alpha unused by the IBL sampler
}}// Row↔latitude convention mirrors the input, so upload with the
// same flipY as the source texture.
const out_tex=new THREE.DataTexture(out,OW,OH,THREE.RGBAFormat,THREE.HalfFloatType);out_tex.flipY=tex.flipY;return out_tex;}catch(e){console.warn('SH irradiance projection failed:',e);return null;}};// Parses a raw environment ArrayBuffer into a bare DataTexture, shared
...
const parseEnvBuffer=(buf,ext)=>{try{if(ext==='.hdr'){if(typeof THREE.RGBELoader==='undefined')return null;// r128's RGBELoader defaults to UnsignedByteType (RGBE-
// encoded data only built-in materials can decode);
// HalfFloatType makes it decode to linear float at parse.
const d=new THREE.RGBELoader().setDataType(THREE.HalfFloatType).parse(buf);if(!d||!d.data)return null;const replaced=sanitizeHalfEnvData(d.data,d.data.length/(d.width*d.height));if(replaced)console.info('[env-sanitize] clamped '+replaced+' overflowed half-float texel channel(s) in .hdr environment');const tex=new THREE.DataTexture(d.data,d.width,d.height,d.format,d.type);// RGBELoader keeps rows top-first, which already matches
// MaterialX's v=0-at-top, no flip.
tex.flipY=false;return tex;}if(ext==='.exr'){if(typeof THREE.EXRLoader==='undefined')return null;// HalfFloatType, not FloatType (unlike loadExrTexture's
// sampler use above): RGBA16F is core mip-able on WebGL2,
// while RGBA32F needs optional extensions.
const d=new THREE.EXRLoader().setDataType(THREE.HalfFloatType).parse(buf);if(!d||!d.data)return null;const replaced=sanitizeHalfEnvData(d.data,d.data.length/(d.width*d.height));if(replaced)console.info('[env-sanitize] clamped '+replaced+' overflowed half-float texel channel(s) in .exr environment');const tex=new THREE.DataTexture(d.data,d.width,d.height,d.format,d.type);// EXRLoader flips rows at decode (data row 0 = image bottom),
// so flip at upload to restore MaterialX's v=0-at-top.
tex.flipY=true;return tex;}return null;// unrecognized extension
}catch(e){return null;}};// ---- Automatic key-light extraction ----
...
const KEYLIGHT_MIN_CONTRAST=64;const KEYLIGHT_RADIUS_RAD=0.10;// Shared data-space -> world direction mapping (extractKeyLight AND
...
const dataDirToWorld=(tex,x,y,W,H)=>{const U=(x+0.5)/W;const gamma=2*Math.PI*U-Math.PI;const vRow=tex.flipY?H-1-y:y;const thetaV=Math.PI*(vRow+0.5)/H;const sinV=Math.sin(thetaV),cosV=Math.cos(thetaV);return new THREE.Vector3(sinV*Math.cos(gamma),cosV,sinV*Math.sin(gamma)).negate();};const extractKeyLight=tex=>{try{const img=tex.image;const W=img.width,H=img.height;const stride=img.data.length/(W*H);const isHalf=img.data.constructor===Uint16Array;if(!Number.isInteger(W)||!Number.isInteger(H)||W<1||H<1||!Number.isInteger(stride)||stride!==3&&stride!==4||(isHalf?!(img.data instanceof Uint16Array):!(img.data instanceof Float32Array)))return null;const rd=i=>isHalf?halfToFloat(img.data[i]):img.data[i];const quantize=v=>isHalf?halfToFloat(floatToHalf(v)):Math.fround(v);// Pass 1: per-texel luminance + solid-angle weight -> mean + peak.
const lum=new Float32Array(W*H);let sumW=0,sumLW=0,peakL=-1,peakX=0,peakY=0;for(let y=0;y<H;y++){const theta=Math.PI*(y+0.5)/H;const dOmega=Math.sin(theta)*(2*Math.PI/W)*(Math.PI/H);for(let x=0;x<W;x++){const idx=(y*W+x)*stride;const r=rd(idx),g=rd(idx+1),b=rd(idx+2);// A failed extraction must leave the texture byte-identical.
// Reject invalid radiance before either the cluster or annulus
// can turn it into a partially-mutated environment.
if(![r,g,b].every(v=>Number.isFinite(v)&&v>=0))return null;const L=0.2126*r+0.7152*g+0.0722*b;lum[y*W+x]=L;sumW+=dOmega;sumLW+=L*dOmega;if(L>peakL){peakL=L;peakX=x;peakY=y;}}}const meanL=sumW>0?sumLW/sumW:0;if(!(peakL>=KEYLIGHT_MIN_CONTRAST*Math.max(meanL,1e-6)))return null;// no sun-like source
// Peak direction (data space), used below for angular clustering.
const pTheta=Math.PI*(peakY+0.5)/H,pPhi=2*Math.PI*(peakX+0.5)/W;const pDir=[Math.sin(pTheta)*Math.cos(pPhi),Math.cos(pTheta),Math.sin(pTheta)*Math.sin(pPhi)];// Pass 2: cluster around the peak (angle + luminance-floor gated).
// The annulus is solid-angle weighted: equirect texels do not have
// equal area, particularly near the poles.
const Lfloor=Math.max(8*meanL,0.02*peakL);let annR=0,annG=0,annB=0,annW=0;const clusterIdx=[];for(let y=0;y<H;y++){const theta=Math.PI*(y+0.5)/H;const dOmega=Math.sin(theta)*(2*Math.PI/W)*(Math.PI/H);const sinT=Math.sin(theta),cosT=Math.cos(theta);for(let x=0;x<W;x++){const phi=2*Math.PI*(x+0.5)/W;const dx=sinT*Math.cos(phi),dy=cosT,dz=sinT*Math.sin(phi);const cosAng=dx*pDir[0]+dy*pDir[1]+dz*pDir[2];const ang=Math.acos(Math.min(1,Math.max(-1,cosAng)));const idx=(y*W+x)*stride;const L=lum[y*W+x];if(ang<=KEYLIGHT_RADIUS_RAD&&L>=Lfloor){const r=rd(idx),g=rd(idx+1),b=rd(idx+2);clusterIdx.push({idx,x,y,dOmega,rgb:[r,g,b]});}else if(ang>KEYLIGHT_RADIUS_RAD&&ang<=2*KEYLIGHT_RADIUS_RAD){annR+=rd(idx)*dOmega;annG+=rd(idx+1)*dOmega;annB+=rd(idx+2)*dOmega;annW+=dOmega;}}}if(!clusterIdx.length||!(annW>0)||!Number.isFinite(annW))return null;const annColor=[annR/annW,annG/annW,annB/annW];if(!annColor.every(v=>Number.isFinite(v)&&v>=0))return null;// Quantize the replacement before measuring removed energy, so the
// analytic light receives exactly what the texture no longer
// contains, including Float32/half storage conversion.
let Er=0,Eg=0,Eb=0;const moment=new THREE.Vector3();const writes=[];for(const entry of clusterIdx){const retained=entry.rgb.map((value,channel)=>quantize(Math.min(value,annColor[channel])));const removed=entry.rgb.map((value,channel)=>value-retained[channel]);if(!retained.every(v=>Number.isFinite(v)&&v>=0)||!removed.every(v=>Number.isFinite(v)&&v>=0))return null;Er+=removed[0]*entry.dOmega;Eg+=removed[1]*entry.dOmega;Eb+=removed[2]*entry.dOmega;const Y=0.2126*removed[0]+0.7152*removed[1]+0.0722*removed[2];moment.addScaledVector(dataDirToWorld(tex,entry.x,entry.y,W,H),Y*entry.dOmega);writes.push({idx:entry.idx,retained});}const maxE=Math.max(Er,Eg,Eb);if(!(maxE>0)||!Number.isFinite(maxE)||![Er,Eg,Eb].every(Number.isFinite)||!(moment.lengthSq()>0)||!Number.isFinite(moment.lengthSq()))return null;const direction=moment.normalize();if(![direction.x,direction.y,direction.z].every(Number.isFinite))return null;// Direction: removed-energy luminance moment, not the raw cluster
// centroid, so a partially clamped edge texel weighs in proportion.
for(const{idx,retained}of writes){img.data[idx]=isHalf?floatToHalf(retained[0]):retained[0];img.data[idx+1]=isHalf?floatToHalf(retained[1]):retained[1];img.data[idx+2]=isHalf?floatToHalf(retained[2]):retained[2];}return{direction,color:[Er/maxE,Eg/maxE,Eb/maxE],intensity:maxE};}catch(e){console.warn('key-light extraction failed:',e);return null;}};// Cheaper direction-only estimate for the studio shadow when
...
const extractSoftKeyDir=tex=>{try{const img=tex.image;const W=img.width,H=img.height;const stride=img.data.length/(W*H);const isHalf=img.data.constructor===Uint16Array;const rd=i=>isHalf?halfToFloat(img.data[i]):img.data[i];const lum=new Float32Array(W*H);let sumW=0,sumLW=0;for(let y=0;y<H;y++){const theta=Math.PI*(y+0.5)/H;const dOmega=Math.sin(theta)*(2*Math.PI/W)*(Math.PI/H);for(let x=0;x<W;x++){const idx=(y*W+x)*stride;const L=0.2126*rd(idx)+0.7152*rd(idx+1)+0.0722*rd(idx+2);lum[y*W+x]=L;sumW+=dOmega;sumLW+=L*dOmega;}}const Lfloor=2*(sumW>0?sumLW/sumW:0);let cxW=0,cyW=0,cW=0;for(let y=0;y<H;y++){const theta=Math.PI*(y+0.5)/H;const dOmega=Math.sin(theta)*(2*Math.PI/W)*(Math.PI/H);for(let x=0;x<W;x++){const L=lum[y*W+x];if(L<Lfloor)continue;const w=L*dOmega;cxW+=x*w;cyW+=y*w;cW+=w;}}if(!Number.isFinite(cW)||cW<=0)return null;return dataDirToWorld(tex,cxW/cW,cyW/cW,W,H);}catch(e){return null;}};// Rotates the extracted key light to track env rotation (rig lights are
// historically fixed, only this one rotates). RotY(-rad): env content
// shifts by +rad, so the light direction shifts by -rad to match.
const keyLightRotationMatrix=rad=>new THREE.Matrix4().makeRotationY(-rad);// Rig lights (fixed) + the active env's extracted key light (rotates
...
const makeLightEntry=over=>Object.assign({type:0,position:new THREE.Vector3(),direction:new THREE.Vector3(0,-1,0),color:new THREE.Vector3(),intensity:0,decay_rate:2,inner_angle:0,outer_angle:0,sourceKind:0},over||{});// Slot layout is fixed for the life of a program: [rig..., key, stage...].
...
const currentLights=(rigLights,keyLight,rotRad,stageLights,envScale,lightScales=null,maxLights=null)=>{const rig=rigLights||[];const total=Number.isFinite(maxLights)&&maxLights>rig.length?maxLights:rig.length+1+STAGE_LIGHT_SLOTS;const stage=(stageLights||[]).slice(0,Math.max(0,total-rig.length-1));const out=rig.map(l=>makeLightEntry({type:l.type,direction:l.direction.clone(),color:l.color.clone(),intensity:l.intensity}));if(keyLight){out.push(makeLightEntry({type:LIGHT_TYPE_DIRECTIONAL,direction:keyLight.direction.clone().applyMatrix4(keyLightRotationMatrix(rotRad||0)),color:new THREE.Vector3(keyLight.color[0],keyLight.color[1],keyLight.color[2]),intensity:keyLight.intensity*(Number.isFinite(envScale)?envScale:1)}));}else{out.push(makeLightEntry({type:LIGHT_TYPE_DIRECTIONAL}));}for(const l of stage)out.push(makeLightEntry(l));// The array length must equal MAX_LIGHT_SOURCES exactly; three walks
// every declared index and an absent element throws.
while(out.length<total)out.push(makeLightEntry());// Scene diagnostics may isolate one direct source without changing the
// fixed slot layout. Absent scales preserve the ordinary lighting path.
if(lightScales){for(let i=0;i<out.length;i++){const scale=Number(lightScales[i]);out[i].intensity*=Number.isFinite(scale)?Math.max(0,scale):1;}}return out;};// Slots actually evaluated. Stage lights sit past the key slot, so reaching
// them means counting it too; an unused key slot is inert (intensity 0).
const activeLightCount=(rigLights,keyLight,stageLights,maxLights=null)=>{const rigCount=(rigLights||[]).length;const slots=Number.isFinite(maxLights)&&maxLights>rigCount?maxLights-rigCount-1:STAGE_LIGHT_SLOTS;const stageCount=Math.min((stageLights||[]).length,Math.max(0,slots));if(stageCount)return rigCount+1+stageCount;return rigCount+(keyLight?1:0);};// Live-updates ONLY the key-light slot (last entry) of an already-bound
...
const updateKeyLightUniformEntry=(uniforms,rigCount,keyLight,rotRad,envScale)=>{const entry=uniforms&&uniforms.u_lightData&&uniforms.u_lightData.value&&uniforms.u_lightData.value[rigCount];if(!entry)return;if(keyLight){entry.direction.copy(keyLight.direction).applyMatrix4(keyLightRotationMatrix(rotRad||0));entry.color.set(keyLight.color[0],keyLight.color[1],keyLight.color[2]);entry.intensity=keyLight.intensity*(Number.isFinite(envScale)?envScale:1);}else{entry.direction.set(0,-1,0);entry.color.set(0,0,0);entry.intensity=0;}if(uniforms.u_numActiveLightSources)uniforms.u_numActiveLightSources.value=rigCount+(keyLight?1:0);};// Builds the full { radiance, irradiance, mips, background,
...
const PREFILTER_SAMPLES=1024;const PREFILTER_GLSL=['precision highp float;','const float M_PI = 3.1415926535897932;','const float M_PI_INV = 0.31830988618379067;','const float M_FLOAT_EPS = 1e-8;','uniform sampler2D uSource;','uniform float uMip;','uniform float uMaxMip;','uniform vec2 uTargetSize;','out vec4 fragColor;','float mx_square(float x) { return x * x; }',// Return the alpha associated with the given mip level in a prefiltered environment.
'float mx_latlong_lod_to_alpha(float lod) {','    float lodBias = lod / uMaxMip;','    return (lodBias < 0.5) ? mx_square(lodBias) : 2.0 * (lodBias - 0.375);','}','vec3 mx_latlong_map_projection_inverse(vec2 uv) {','    float latitude = (uv.y - 0.5) * M_PI;','    float longitude = (uv.x - 0.5) * M_PI * 2.0;','    float x = -cos(latitude) * sin(longitude);','    float y = -sin(latitude);','    float z = cos(latitude) * cos(longitude);','    return vec3(x, y, z);','}','vec2 mx_latlong_projection(vec3 dir) {','    float latitude = -asin(clamp(dir.y, -1.0, 1.0)) * M_PI_INV + 0.5;','    float longitude = atan(dir.x, -dir.z) * M_PI_INV * 0.5 + 0.5;','    return vec2(longitude, latitude);','}','vec3 mx_latlong_map_lookup(vec3 dir, float lod) {','    return textureLod(uSource, mx_latlong_projection(normalize(dir)), lod).rgb;','}','float mx_latlong_compute_lod(vec3 dir, float pdf, float maxMipLevel, int envSamples) {','    const float MIP_LEVEL_OFFSET = 1.5;','    float effectiveMaxMipLevel = maxMipLevel - MIP_LEVEL_OFFSET;','    float distortion = sqrt(1.0 - mx_square(dir.y));','    return max(effectiveMaxMipLevel - 0.5 * log2(float(envSamples) * pdf * distortion), 0.0);','}','mat3 mx_orthonormal_basis(vec3 N) {','    float sgn = (N.z < 0.0) ? -1.0 : 1.0;','    float a = -1.0 / (sgn + N.z);','    float b = N.x * N.y * a;','    vec3 X = vec3(1.0 + sgn * N.x * N.x * a, sgn * b, -sgn * N.x);','    vec3 Y = vec3(b, sgn + N.y * N.y * a, -N.y);','    return mat3(X, Y, N);','}','float mx_golden_ratio_sequence(int i) {','    const float GOLDEN_RATIO = 1.6180339887498948;','    return fract((float(i) + 1.0) * GOLDEN_RATIO);','}','vec2 mx_spherical_fibonacci(int i, int numSamples) {','    return vec2((float(i) + 0.5) / float(numSamples), mx_golden_ratio_sequence(i));','}','float mx_ggx_NDF(vec3 H, vec2 alpha) {','    vec2 He = H.xy / alpha;','    float denom = dot(He, He) + mx_square(H.z);','    return 1.0 / (M_PI * alpha.x * alpha.y * mx_square(denom));','}','vec3 mx_ggx_importance_sample_VNDF(vec2 Xi, vec3 V, vec2 alpha) {','    V = normalize(vec3(V.xy * alpha, V.z));','    float phi = 2.0 * M_PI * Xi.x;','    float z = (1.0 - Xi.y) * (1.0 + V.z) - V.z;','    float sinTheta = sqrt(clamp(1.0 - z * z, 0.0, 1.0));','    vec3 c = vec3(sinTheta * cos(phi), sinTheta * sin(phi), z);','    vec3 H = c + V;','    return normalize(vec3(H.xy * alpha, max(H.z, 0.0)));','}','float mx_ggx_VNDF_reflection_PDF(vec3 H, vec2 alpha, float G1V, float NdotV) {','    return mx_ggx_NDF(H, alpha) * G1V / (4.0 * NdotV);','}','float mx_ggx_smith_G1(float cosTheta, float alpha) {','    float cosTheta2 = mx_square(cosTheta);','    float tanTheta2 = (1.0 - cosTheta2) / cosTheta2;','    return 2.0 / (1.0 + sqrt(1.0 + mx_square(alpha) * tanTheta2));','}','float mx_ggx_smith_G2(float NdotL, float NdotV, float alpha) {','    float alpha2 = mx_square(alpha);','    float lambdaL = sqrt(alpha2 + (1.0 - alpha2) * mx_square(NdotL));','    float lambdaV = sqrt(alpha2 + (1.0 - alpha2) * mx_square(NdotV));','    return 2.0 * NdotL * NdotV / (lambdaL * NdotV + lambdaV * NdotL);','}','void main() {','    vec2 uv = gl_FragCoord.xy / uTargetSize;',// +0.5 in longitude, because mx_latlong_map_projection_inverse is NOT
...
'    vec3 worldN = mx_latlong_map_projection_inverse(vec2(uv.x + 0.5, uv.y));','    float alpha = mx_latlong_lod_to_alpha(uMip);',// A mirror lobe has no width to integrate; sampling it would just add
// noise, so level 0 is the source unchanged.
'    if (alpha <= 0.0) { fragColor = vec4(mx_latlong_map_lookup(worldN, 0.0), 1.0); return; }','    vec3 V = vec3(0.0, 0.0, 1.0);','    float NdotV = 1.0;','    mat3 tangentToWorld = mx_orthonormal_basis(worldN);','    float G1V = mx_ggx_smith_G1(NdotV, alpha);','    vec3 radiance = vec3(0.0);','    float weight = 0.0;','    const int envRadianceSamples = '+PREFILTER_SAMPLES+';','    for (int i = 0; i < envRadianceSamples; i++) {','        vec2 Xi = mx_spherical_fibonacci(i, envRadianceSamples);','        vec3 H = mx_ggx_importance_sample_VNDF(Xi, V, vec2(alpha));','        vec3 L = -V + 2.0 * H.z * H;','        float NdotL = clamp(L.z, M_FLOAT_EPS, 1.0);','        float G = mx_ggx_smith_G2(NdotL, NdotV, alpha);','        vec3 Lw = tangentToWorld * L;','        float pdf = mx_ggx_VNDF_reflection_PDF(H, vec2(alpha), G1V, NdotV);','        float lod = mx_latlong_compute_lod(Lw, pdf, uMaxMip, envRadianceSamples);','        radiance += G * mx_latlong_map_lookup(Lw, lod);','        weight += G;','    }','    fragColor = vec4(radiance / max(weight, M_FLOAT_EPS), 1.0);','}'].join('\n');// Builds env.radiancePrefiltered once per environment, on the first view
...
const legacyPrefilterLatch=(()=>{try{return localStorage.getItem('mtlx_scene_prefilter_fix')==='0';}catch(e){return false;}})();const ensurePrefilteredEnv=(renderer,env)=>{if(!env||!env.radiance||env.prefilterTried)return env;if(!renderer&&!legacyPrefilterLatch)return env;if(legacyPrefilterLatch)env.prefilterTried=true;if(getSpecularEnvMethod()!=='prefilter')return env;if(!renderer||!renderer.capabilities||!renderer.capabilities.isWebGL2)return env;env.prefilterTried=true;// Float targets are the only type readRenderTargetPixels can be relied
// on to return here; without them the chain cannot be read back.
if(!renderer.extensions.get('EXT_color_buffer_float')){mtlxWarn('mtlx-engine: EXT_color_buffer_float missing, keeping the FIS specular environment.');return env;}const src=env.radiance;const w=src.image&&src.image.width,h=src.image&&src.image.height;if(!w||!h)return env;const levels=env.mips||Math.trunc(Math.log2(Math.max(w,h)))+1;const t0=performance.now();const previousTarget=renderer.getRenderTarget();const material=new THREE.RawShaderMaterial({glslVersion:THREE.GLSL3,vertexShader:'in vec3 position;\nvoid main() { gl_Position = vec4(position, 1.0); }',fragmentShader:PREFILTER_GLSL,uniforms:{uSource:{value:src},uMip:{value:0},uMaxMip:{value:Math.max(1,levels-1)},uTargetSize:{value:new THREE.Vector2(w,h)}},depthTest:false,depthWrite:false});const scene=new THREE.Scene();scene.add(new THREE.Mesh(new THREE.PlaneGeometry(2,2),material));const camera=new THREE.OrthographicCamera(-1,1,1,-1,0,1);const mipmaps=[];let failed=false;try{for(let level=0;level<levels;level++){const lw=Math.max(1,w>>level),lh=Math.max(1,h>>level);const target=new THREE.WebGLRenderTarget(lw,lh,{minFilter:THREE.LinearFilter,magFilter:THREE.LinearFilter,format:THREE.RGBAFormat,type:THREE.FloatType,depthBuffer:false,stencilBuffer:false,generateMipmaps:false});material.uniforms.uMip.value=level;material.uniforms.uTargetSize.value.set(lw,lh);renderer.setRenderTarget(target);renderer.render(scene,camera);const pixels=new Float32Array(lw*lh*4);renderer.readRenderTargetPixels(target,0,0,lw,lh,pixels);target.dispose();// Half float keeps the chain linear-filterable in core WebGL2
// (full float filtering needs OES_texture_float_linear) and
// halves the upload, at a precision the source already has.
const half=new Uint16Array(lw*lh*4);for(let i=0;i<half.length;i++)half[i]=floatToHalf(pixels[i]);mipmaps.push({data:half,width:lw,height:lh});}}catch(error){failed=true;mtlxWarn('mtlx-engine: GGX environment prefilter failed, keeping the FIS chain: '+(error&&error.message||error));}renderer.setRenderTarget(previousTarget);material.dispose();scene.children[0].geometry.dispose();if(failed||!mipmaps.length)return env;const base=mipmaps[0];const tex=new THREE.DataTexture(base.data,base.width,base.height,THREE.RGBAFormat,THREE.HalfFloatType);tex.mapping=THREE.EquirectangularReflectionMapping;tex.wrapS=THREE.RepeatWrapping;tex.wrapT=THREE.ClampToEdgeWrapping;tex.minFilter=THREE.LinearMipmapLinearFilter;tex.magFilter=THREE.LinearFilter;// Always false, never copied from the source: the chain was written in
// framebuffer space, where row 0 is v = 0, and readRenderTargetPixels
// hands the rows back in that same order.
tex.flipY=false;tex.encoding=THREE.LinearEncoding;tex.anisotropy=8;// Levels are supplied, not derived: three uploads texture.mipmaps for a
// DataTexture and turns generateMipmaps off itself when it does.
tex.mipmaps=mipmaps;tex.generateMipmaps=false;tex.needsUpdate=true;env.radiancePrefiltered=tex;if(window.MTLX_PERF_LOG){console.log('[mtlx-perf] env prefilter: '+(performance.now()-t0).toFixed(1)+'ms ('+levels+' levels, '+w+'x'+h+')');}return env;};// The radiance sampler the SHADING path binds. The backdrop, the PMREM
...
// generated for it.
const envRadianceForShading=env=>{if(!env)return null;if(getSpecularEnvMethod()==='prefilter'&&env.radiancePrefiltered)return env.radiancePrefiltered;return env.radiance;};// GPU cosine-convolved diffuse irradiance map: replaces the SH l<=2
...
const IRRADIANCE_CONV_W=128;const IRRADIANCE_CONV_H=64;const IRRADIANCE_OUT_W=64;const IRRADIANCE_OUT_H=32;const IRRADIANCE_GLSL=['precision highp float;','const float M_PI = 3.1415926535897932;','const float M_PI_INV = 0.31830988618379067;','uniform sampler2D uSource;','uniform float uSrcLod;','uniform vec2 uTargetSize;','out vec4 fragColor;','vec3 mx_latlong_map_projection_inverse(vec2 uv) {','    float latitude = (uv.y - 0.5) * M_PI;','    float longitude = (uv.x - 0.5) * M_PI * 2.0;','    float x = -cos(latitude) * sin(longitude);','    float y = -sin(latitude);','    float z = cos(latitude) * cos(longitude);','    return vec3(x, y, z);','}','void main() {','    vec2 uv = gl_FragCoord.xy / uTargetSize;',// Same +0.5 longitude rule as PREFILTER_GLSL: texel uv holds the
// value for this direction, which is what mx_latlong_projection
// reads back.
'    vec3 N = normalize(mx_latlong_map_projection_inverse(vec2(uv.x + 0.5, uv.y)));','    const int CW = '+IRRADIANCE_CONV_W+';','    const int CH = '+IRRADIANCE_CONV_H+';','    float dPhi = 2.0 * M_PI / float(CW);','    float dTheta = M_PI / float(CH);','    vec3 E = vec3(0.0);','    for (int j = 0; j < CH; j++) {','        float sv = (float(j) + 0.5) / float(CH);','        for (int i = 0; i < CW; i++) {','            float su = (float(i) + 0.5) / float(CW);','            vec3 L = normalize(mx_latlong_map_projection_inverse(vec2(su + 0.5, sv)));','            float NdotL = dot(N, L);','            if (NdotL <= 0.0) continue;',// Lat-long solid angle: sin(polar) = cos(latitude) = sqrt(1 - L.y*L.y).
'            float sinT = sqrt(max(0.0, 1.0 - L.y * L.y));','            vec3 Li = textureLod(uSource, vec2(su, sv), uSrcLod).rgb;','            E += Li * NdotL * sinT * dPhi * dTheta;','        }','    }',// The 1/PI matches mx_environment_irradiance's units, the same scale
// shIrradianceFromEquirect's Pass 2 applies above.
'    fragColor = vec4(max(E * M_PI_INV, vec3(0.0)), 1.0);','}'].join('\n');// Builds env.irradianceConvolved once per environment, on the first view
...
const ensureConvolvedIrradiance=(renderer,env)=>{if(!env||!env.radiance||env.irradianceTried)return env;if(getDiffuseEnvMethod()!=='convolve')return env;if(!renderer||!renderer.capabilities||!renderer.capabilities.isWebGL2)return env;env.irradianceTried=true;if(!renderer.extensions.get('EXT_color_buffer_float')){mtlxWarn('mtlx-engine: EXT_color_buffer_float missing, keeping the SH irradiance.');return env;}const src=env.radiance;const srcW=src.image&&src.image.width||IRRADIANCE_CONV_W;const srcH=src.image&&src.image.height||IRRADIANCE_CONV_H;const t0=performance.now();const previousTarget=renderer.getRenderTarget();let material=null,scene=null,target=null;try{material=new THREE.RawShaderMaterial({glslVersion:THREE.GLSL3,vertexShader:'in vec3 position;\nvoid main() { gl_Position = vec4(position, 1.0); }',fragmentShader:IRRADIANCE_GLSL,uniforms:{uSource:{value:src},uSrcLod:{value:Math.max(0,Math.log2(srcW/IRRADIANCE_CONV_W))},uTargetSize:{value:new THREE.Vector2(IRRADIANCE_OUT_W,IRRADIANCE_OUT_H)}},depthTest:false,depthWrite:false});scene=new THREE.Scene();scene.add(new THREE.Mesh(new THREE.PlaneGeometry(2,2),material));const camera=new THREE.OrthographicCamera(-1,1,1,-1,0,1);target=new THREE.WebGLRenderTarget(IRRADIANCE_OUT_W,IRRADIANCE_OUT_H,{minFilter:THREE.LinearFilter,magFilter:THREE.LinearFilter,format:THREE.RGBAFormat,type:THREE.FloatType,depthBuffer:false,stencilBuffer:false,generateMipmaps:false});renderer.setRenderTarget(target);renderer.render(scene,camera);const pixels=new Float32Array(IRRADIANCE_OUT_W*IRRADIANCE_OUT_H*4);renderer.readRenderTargetPixels(target,0,0,IRRADIANCE_OUT_W,IRRADIANCE_OUT_H,pixels);// Retained verbatim for the diffuse bounce v3 bake
...
env.irradianceConvolvedData=pixels;env.irradianceConvolvedSize=[IRRADIANCE_OUT_W,IRRADIANCE_OUT_H];const half=new Uint16Array(pixels.length);for(let i=0;i<half.length;i++)half[i]=floatToHalf(pixels[i]);const tex=new THREE.DataTexture(half,IRRADIANCE_OUT_W,IRRADIANCE_OUT_H,THREE.RGBAFormat,THREE.HalfFloatType);tex.mapping=THREE.EquirectangularReflectionMapping;tex.wrapS=THREE.RepeatWrapping;tex.wrapT=THREE.ClampToEdgeWrapping;tex.minFilter=THREE.LinearFilter;tex.magFilter=THREE.LinearFilter;tex.generateMipmaps=false;// Framebuffer space, same reasoning as ensurePrefilteredEnv's tex.
tex.flipY=false;tex.encoding=THREE.LinearEncoding;tex.needsUpdate=true;env.irradianceConvolved=tex;if(window.MTLX_PERF_LOG){console.log('[mtlx-perf] env irradiance convolve: '+(performance.now()-t0).toFixed(1)+'ms ('+srcW+'x'+srcH+' -> '+IRRADIANCE_OUT_W+'x'+IRRADIANCE_OUT_H+')');}}catch(error){mtlxWarn('mtlx-engine: irradiance convolution failed, keeping the SH map: '+(error&&error.message||error));}renderer.setRenderTarget(previousTarget);if(target)target.dispose();if(material)material.dispose();if(scene&&scene.children[0])scene.children[0].geometry.dispose();return env;};// The irradiance sampler the SHADING path binds. env.irradiance (the SH
...
const envIrradianceForShading=env=>{if(!env)return null;if(getDiffuseEnvMethod()==='convolve'&&env.irradianceConvolved)return env.irradianceConvolved;return env.irradiance;};const buildEnvFromParsedTexture=raw=>{// Extraction mutates raw's pixels (clamps the sun) BEFORE mips/SH/
// background are built below, so it disappears from all three,
// matching official "split" env assets.
const keyLight=keyLightEnabled?extractKeyLight(raw):null;// extractKeyLight only mutates raw on a SUCCESSFUL extraction (both
// its null-return paths run before the clamp), so raw is still
// pristine here whenever the soft fallback is actually needed.
const softKeyDir=keyLight?null:extractSoftKeyDir(raw);const radiance=prepareEnv(raw);const irrSrc=shIrradianceFromEquirect(raw);const irradiance=irrSrc?prepareEnv(irrSrc):radiance;const img=radiance.image;const mips=Math.trunc(Math.log2(Math.max(img.width,img.height)))+1;// Correctly-oriented copy for the visible skybox mesh, see
// makeBackgroundTexture and the env-prep header above.
const background=makeBackgroundTexture(radiance);return{radiance,irradiance,irradianceConvolved:null,mips,background,prefilteredIrr:false,keyLight,softKeyDir};};const getEnvironment=()=>{if(!envPromise){// fetch() -> ArrayBuffer -> parseEnvBuffer, mirroring
...
const built=buildEnvFromParsedTexture(raw);return built;});}return envPromise;};// Builds an environment from raw bytes into the same shape getEnvironment()
...
if(lower==='.hdr'&&typeof THREE.RGBELoader==='undefined'){throw new Error('RGBELoader unavailable (script blocked/offline), cannot load .hdr environments.');}if(lower==='.exr'&&typeof THREE.EXRLoader==='undefined'){throw new Error('EXRLoader unavailable (script blocked/offline), cannot load .exr environments.');}const raw=parseEnvBuffer(buf,lower);if(!raw||!raw.image||!raw.image.data){throw new Error('Failed to parse the environment image "'+label+'".');}if(remember)overrideEnvSource={buf,ext:lower};return buildEnvFromParsedTexture(raw);};// Constant-colour environment in the same shape, for a USD dome light that
...
const setEnvOverride=env=>{envOverride=env||null;if(envOverride){// Import: apply the new environment to every live view right away.
...
getEnvironment().then(def=>{if(!envOverride){LIVE_VIEWS.forEach(v=>{try{v.setEnvironment(def);}catch(e){/* view has no lighting/env, no-op */}});}});}};const getEnvOverride=()=>envOverride;// Key-light toggle (UI-facing): rebuilds the ACTIVE env from its cached
...
const getKeyLightEnabled=()=>keyLightEnabled;const setKeyLightEnabled=on=>{keyLightEnabled=!!on;try{localStorage.setItem(KEYLIGHT_STORAGE_KEY,keyLightEnabled?'1':'0');}catch(e){/* unavailable */}const src=envOverride?overrideEnvSource:defaultEnvSource;if(!src)return;// nothing loaded yet; the next load already honors the flag
const raw=parseEnvBuffer(src.buf,src.ext);if(!raw||!raw.image||!raw.image.data)return;const rebuilt=buildEnvFromParsedTexture(raw);if(envOverride){setEnvOverride(rebuilt);// re-broadcasts via each view's setEnvironment()
}else{envPromise=Promise.resolve(rebuilt);LIVE_VIEWS.forEach(v=>{try{v.setEnvironment(rebuilt);}catch(e){/* view has no lighting/env, no-op */}});}};// Standard MaterialX color spaces accepted on filename inputs. Changing
...
const warmKey=(vs,fs)=>{let h=5381;const s=vs+' '+fs;for(let i=0;i<s.length;i++)h=(h*33^s.charCodeAt(i))>>>0;return s.length+':'+h;};// Pre-compiles vs/fs on the hidden warm context; never throws. The
...
const prewarmShaderCompile=async({vs,fs,isMounted,label,timeoutMs})=>{const ctx=getWarmContext();if(!ctx)return'skipped';const key=warmKey(vs,fs);if(MTLX_WARMED_SOURCES.has(key)){if(window.MTLX_PERF_LOG){console.log('[mtlx-perf] GL prewarm skipped, source already warmed this session (target: '+label+')');}return'skipped';}const{gl,ext}=ctx;const __warmPerfStart=window.MTLX_PERF_LOG?performance.now():0;let warmProgram=null,warmVShader=null,warmFShader=null;try{warmVShader=gl.createShader(gl.VERTEX_SHADER);gl.shaderSource(warmVShader,'#version 300 es\n'+vs);gl.compileShader(warmVShader);warmFShader=gl.createShader(gl.FRAGMENT_SHADER);gl.shaderSource(warmFShader,'#version 300 es\n'+fs);gl.compileShader(warmFShader);warmProgram=gl.createProgram();gl.attachShader(warmProgram,warmVShader);gl.attachShader(warmProgram,warmFShader);gl.linkProgram(warmProgram);}catch(e){// Defensive only: any failure here just skips the warm-up, falls
// through to today's (unwarmed) compile behavior.
try{if(warmProgram)gl.deleteProgram(warmProgram);}catch(e2){/* context lost etc. */}try{if(warmVShader)gl.deleteShader(warmVShader);}catch(e2){/* ditto */}try{if(warmFShader)gl.deleteShader(warmFShader);}catch(e2){/* ditto */}return'skipped';}if(window.MTLX_PERF_LOG){console.log('[mtlx-perf] GL compile submit: '+(performance.now()-__warmPerfStart).toFixed(1)+'ms (target: '+label+')');}const cleanup=()=>{try{if(warmProgram)gl.deleteProgram(warmProgram);}catch(e){/* context lost etc. */}try{if(warmVShader)gl.deleteShader(warmVShader);}catch(e){/* ditto */}try{if(warmFShader)gl.deleteShader(warmFShader);}catch(e){/* ditto */}};const WAIT_POLL_MS=50,WAIT_POLL_FAST_MS=16,WAIT_POLL_FAST_TICKS=6;const WAIT_TIMEOUT_MS=typeof timeoutMs==='number'&&timeoutMs>0?timeoutMs:15000;const __waitStart=performance.now();let timedOut=false;// isProgram() is the silent validity check: false for a
...
const isWarmDone=()=>{try{if(gl.isContextLost())return true;if(!gl.isProgram(warmProgram))return true;const v=gl.getProgramParameter(warmProgram,ext.COMPLETION_STATUS_KHR);// A GL error (invalid/deleted program) returns null WITHOUT
// throwing, treat it as "nothing left to wait for" instead
// of polling (and console-spamming) until the timeout cap.
return v===null?true:!!v;}catch(e){// Disposed/invalid handle, nothing left to wait for.
return true;}};// Check once immediately, before the first sleep, a fast background
// compile may already be done before we'd otherwise pay a single poll
// tick of latency.
let tick=0;for(;;){if(isWarmDone())break;// Safety cap: on timeout, stop polling and proceed; the real
// compile then blocks for whatever time remains, so this is
// never WORSE than not pre-warming, only equal or better.
if(performance.now()-__waitStart>WAIT_TIMEOUT_MS){timedOut=true;break;}// Escalating poll interval: fast compiles resolve within about a
// frame, so the first ~6 ticks poll at 16ms; the 50ms tick only
// matters for multi-second compiles.
const pollMs=tick<WAIT_POLL_FAST_TICKS?WAIT_POLL_FAST_MS:WAIT_POLL_MS;tick++;await new Promise(resolve=>setTimeout(resolve,pollMs));// Lifecycle bail: a superseded build must stop and clean up rather
// than keep polling GL objects for a view nobody wants.
if(!isMounted()){cleanup();return'bailed';}}if(window.MTLX_PERF_LOG){console.log('[mtlx-perf] GL compile wait: '+(performance.now()-__waitStart).toFixed(1)+'ms (target: '+label+')');}if(!timedOut)MTLX_WARMED_SOURCES.add(key);cleanup();return'done';};// Submits a generated standalone displacement program to the warm context
...
const prewarmPreviewTarget=async({mx,gen,genContext,buildRenderable,label,isMounted=()=>true})=>{// No warm context (no WebGL2 / no KHR_parallel_shader_compile) means
// generating sources here would only be thrown away, skip the work.
if(!getWarmContext())return'skipped';let srcs=null;try{srcs=await mxExclusive(()=>{const built=buildRenderable();if(!built||!built.renderable)return null;try{return generatePreviewSourcesUnlocked({mx,gen,genContext,renderable:built.renderable,label,isMounted,stageLightCount:PREVIEW_STAGE_LIGHT_COUNT,sceneFeatureOptions:PREVIEW_FEATURE_OPTIONS});}finally{// Best-effort, ALWAYS: the transient __pv_* wrappers must
...
// ------------------------------------------------------------------
const checkTargetTransparency=async({mx,gen,buildRenderable})=>{try{return await mxExclusive(()=>{const built=buildRenderable();if(!built||!built.renderable)return null;try{if(typeof mx.isTransparentSurface!=='function')return null;return!!mx.isTransparentSurface(built.renderable,gen.getTarget());}catch(e){return null;}finally{try{built.cleanup&&built.cleanup();}catch(e){/* best-effort */}}});}catch(e){return null;}};// MaterialX has no implicit type coercion; a mismatched connection compiles
...
const mxOutputTypeAndNode=(node,outputName)=>{let outs=[];try{outs=vecToArray(node.getOutputs?node.getOutputs():null);}catch(e){outs=[];}if(outs.length>1||mxElType(node)==='multioutput'){const out=outputName?mxSafe(()=>node.getOutput(outputName),null)||outs.find(o=>mxElName(o)===outputName):outs[0];if(!out)return null;return{type:mxElType(out),producer:mxSafe(()=>out.getConnectedNode(),null)};}return{type:mxElType(node),producer:node};};const mxResolveConnection=(input,doc)=>{const graphName=mxElAttr(input,'nodegraph');if(graphName){const ng=mxSafe(()=>doc.getNodeGraph(graphName),null);if(!ng)return null;const resolved=mxOutputTypeAndNode(ng,mxElAttr(input,'output'));return resolved?{type:resolved.type,sourceName:graphName,node:resolved.producer}:null;}const nodeName=mxElAttr(input,'nodename');if(nodeName){const node=mxSafe(()=>input.getConnectedNode(),null);if(!node)return null;const resolved=mxOutputTypeAndNode(node,mxElAttr(input,'output'));return resolved?{type:resolved.type,sourceName:nodeName,node:resolved.producer}:null;}const interfaceName=mxElAttr(input,'interfacename');if(interfaceName){// The promoted graph-level input: one hop only, its own value is
// the binding, not a further per-instance override to chase.
const iface=mxSafe(()=>input.getInterfaceInput(),null);if(iface&&iface!==input)return mxResolveConnection(iface,doc);}return null;};// Depth-capped upstream walk from the renderable surface node, through every
...
const MTLX_TYPE_WALK_MAX_DEPTH=32;const findTypeMismatches=(renderable,mx)=>{let doc=null;try{doc=renderable.getDocument();}catch(e){return null;}if(!doc)return null;const visited=new Set();let found=null;const walk=(node,depth)=>{if(found||!node||depth>MTLX_TYPE_WALK_MAX_DEPTH)return;const nodeName=mxElName(node);const visitKey=nodeName+'|'+mxElCat(node);if(visited.has(visitKey))return;visited.add(visitKey);const inputs=vecToArray(mxSafe(()=>node.getInputs?node.getInputs():null,[]));for(const input of inputs){if(found)return;const connected=mxElAttr(input,'nodename')||mxElAttr(input,'nodegraph')||mxElAttr(input,'interfacename');if(!connected)continue;const inputType=mxElType(input);const resolved=mxResolveConnection(input,doc);if(!resolved||!resolved.type||!inputType)continue;if(resolved.type!==inputType){found={nodeName,inputName:mxElName(input),inputType,sourceName:resolved.sourceName,sourceType:resolved.type};return;}if(resolved.node)walk(resolved.node,depth+1);}};walk(renderable,0);return found;};// MaterialX reports an unresolved node by its INSTANCE name only ("could
...
const describeUnresolvedNodes=renderable=>{let doc=null;try{doc=renderable.getDocument();}catch(e){return[];}if(!doc)return[];// Library-owned graphs carry a source URI; user-authored ones don't.
// Without that filter this would walk the whole standard library.
const nodes=[];try{nodes.push(...(doc.getNodes()||[]));}catch(e){/* keep going */}try{for(const g of doc.getNodeGraphs()||[]){if(g.getSourceUri&&g.getSourceUri())continue;nodes.push(...(g.getNodes()||[]));}}catch(e){/* keep going */}const out=[];for(const node of nodes){try{if(node.getNodeDef())continue;}catch(e){/* unresolved counts as a finding */}let category='';let known=false;try{category=node.getCategory()||'';}catch(e){/* unnamed */}try{known=(doc.getMatchingNodeDefs(category)||[]).length>0;}catch(e){/* assume not */}let name='';try{name=node.getName()||'';}catch(e){/* unnamed */}out.push({name,category,known});}return out;};// One sentence per unresolved node. `known` separates "this build has no
...
const unresolvedNodesText=found=>found.map(u=>u.known?`Node "${u.name}" (type "${u.category}") exists in this MaterialX build, but no definition matches its inputs.`:`Node "${u.name}" (type "${u.category}") has no definition in this MaterialX build.`).join(' ');// Image-family nodes that cost one sampler2D each in the generated ESSL.
const MERGEABLE_IMAGE_CATEGORIES=new Set(['image','tiledimage','gltf_image','gltf_colorimage','gltf_normalmap','gltf_anisotropy_image','gltf_iridescence_thickness']);// Editor-only attributes, never part of what a node reads.
const MERGE_IGNORED_ATTRIBUTES=new Set(['name','xpos','ypos','doc']);// Identity of one node: category, type, its own attributes and every input's
...
const mxNodeSignature=node=>{const parts=[mxElCat(node),mxElType(node)];for(const attr of vecToArray(mxSafe(()=>node.getAttributeNames(),[])).slice().sort()){if(MERGE_IGNORED_ATTRIBUTES.has(attr))continue;parts.push('@'+attr+'='+mxElAttr(node,attr));}const inputs=vecToArray(mxSafe(()=>node.getInputs(),[])).map(input=>{const attrs=vecToArray(mxSafe(()=>input.getAttributeNames(),[])).slice().sort();return mxElName(input)+'{'+attrs.map(a=>a+'='+mxElAttr(input,a)).join(',')+'}';}).sort();return parts.join('|')+'|'+inputs.join('|');};// Two image nodes that read the same file the same way cost two texture
...
const mergeDuplicateImageNodes=doc=>{const restores=[];const groups=[];let merged=0;if(!doc)return{restore:()=>{},merged,groups};mxWarnIfLocked('mergeDuplicateImageNodes');// exported doc-mutating helper, see mxWarnIfLocked's header comment
const scopes=[doc].concat(vecToArray(mxSafe(()=>doc.getNodeGraphs?doc.getNodeGraphs():null,[])));for(const scope of scopes){const children=vecToArray(mxSafe(()=>scope.getChildren(),[]));const bySignature=new Map();const renames=new Map();// duplicate name -> canonical name
for(const node of children){if(!MERGEABLE_IMAGE_CATEGORIES.has(mxElCat(node)))continue;// An interface-bound input can be rebound per instance; leave those alone.
const bound=vecToArray(mxSafe(()=>node.getInputs(),[])).some(input=>mxElHasAttr(input,'interfacename'));if(bound)continue;const signature=mxNodeSignature(node);const first=bySignature.get(signature);if(!first){bySignature.set(signature,node);continue;}renames.set(mxElName(node),mxElName(first));}if(!renames.size)continue;for(const child of children){const ports=[child].concat(vecToArray(mxSafe(()=>child.getChildren(),[])));for(const port of ports){if(!mxElHasAttr(port,'nodename'))continue;const from=mxElAttr(port,'nodename');const to=renames.get(from);if(!to||to===from)continue;if(mxSetAttr(port,'nodename',to))restores.push(()=>mxSetAttr(port,'nodename',from));}}for(const[from,to]of renames)groups.push({kept:to,merged:from});merged+=renames.size;}const restore=()=>{for(let i=restores.length-1;i>=0;i--)restores[i]();};return{restore,merged,groups};};// ------------------------------------------------------------------
...
// ------------------------------------------------------------------
const generatePreviewSourcesUnlocked=({mx,gen,genContext,renderable,label,materialName=null,isMounted=()=>true,document:documentArg=null,sceneRgbt=false,lightTransport=false,sceneFeatureOptions=null,stageLightCount=null,allowConstInputs=true})=>{// Sampler-budget drops, requested only by compileMtlxSceneMaterial's
// recompile loop; every other caller keeps the full feature set.
const skipSkyVis=!!(sceneFeatureOptions&&sceneFeatureOptions.skipSkyVis);const skipAoVolume=!!(sceneFeatureOptions&&sceneFeatureOptions.skipAoVolume);const skipBounce=!!(sceneFeatureOptions&&sceneFeatureOptions.skipBounce);const dropThicknessMap=!!(sceneFeatureOptions&&sceneFeatureOptions.dropThicknessMap);const skipTransmittance=!!(sceneFeatureOptions&&sceneFeatureOptions.skipTransmittance);const skipRefraction=!!(sceneFeatureOptions&&sceneFeatureOptions.skipRefraction);// Feature gating: a feature that is off, or impossible in this tool, is
// never generated. The kill switch forces both back to "generate".
const featureGated=readFeatureGated();const skipShadowMap=featureGated&&!!(sceneFeatureOptions&&sceneFeatureOptions.skipShadowMap);const skipOcclusion=featureGated&&!!(sceneFeatureOptions&&sceneFeatureOptions.skipOcclusion);// Screen-space reflections are parked (see SCENE_SSR_PARKED in the renderer): skip the patch.
const skipSsr=true||!!(sceneFeatureOptions&&sceneFeatureOptions.skipSsr);const skipLocalEnv=!!(sceneFeatureOptions&&sceneFeatureOptions.skipLocalEnv);// Off by default (Viewer/Compare/Builder/Graph previews and every
...
const specularAA=!!(sceneFeatureOptions&&sceneFeatureOptions.specularAA);// OFFICIAL PARITY: per-material generation options on SHARED
...
let transparent=false;try{genContext.getOptions().hwTransparency=false;}catch(e){/* option absent */}try{if(typeof mx.isTransparentSurface==='function'){const t=!!mx.isTransparentSurface(renderable,gen.getTarget());genContext.getOptions().hwTransparency=t;transparent=t;// set only after the option write succeeded
}}catch(e){transparent=false;/* reset above already put the option at the deterministic false default */}try{if(mx.ShaderInterfaceType){genContext.getOptions().shaderInterfaceType=mx.ShaderInterfaceType.SHADER_INTERFACE_COMPLETE;}}catch(e){/* default interface */}// MaterialX's premultiplied BSDF-add mode combines a diffuse closure's
...
try{genContext.getOptions().premultipliedBsdfAdd=false;}catch(e){/* option absent in older bindings */}// Specular environment method. The setter takes the EMBIND ENUM VALUE,
...
try{const methods=mx.HwSpecularEnvironmentMethod;const wanted=getSpecularEnvMethod()==='fis'?methods.SPECULAR_ENVIRONMENT_FIS:methods.SPECULAR_ENVIRONMENT_PREFILTER;if(wanted)genContext.getOptions().hwSpecularEnvironmentMethod=wanted;}catch(e){/* enum absent in older bindings, keep the generator default */}// Shadow-map sampling is a generator option, so a tool or a scene with
// shadows off never compiles it. Written on EVERY generation, both ways,
// so no material leaks the previous one's setting.
try{genContext.getOptions().hwShadowMap=!skipShadowMap;}catch(e){/* option absent */}// Light limit: MAX_LIGHT_SOURCES is baked into the source, so a tool
...
let maxLights=mxRigLightCount+1+STAGE_LIGHT_SLOTS;if(readLightLimit()){maxLights=mxRigLightCount+1+chooseStageLightTier(stageLightCount);try{genContext.getOptions().hwMaxActiveLightSources=maxLights;}catch(e){/* option absent */}}// Bail before the ~expensive shader-generation call if this
// build was superseded (mounted flipped while awaiting above),
// nothing GL-side exists yet, so there's nothing to dispose.
if(!isMounted())return null;// Colorspace aliases are normalized on the LIVE document for the
// duration of gen.generate only; restore in finally so the authored
// document (and any export of it) never actually changes.
const colorspaceDoc=mxSafe(()=>renderable.getDocument(),null)||documentArg;let colorspaceAliasResult=null;let colorspaceTransformResult=null;let materialWorkspaceResult=null;let imageMergeResult=null;if(colorspaceDoc){colorspaceAliasResult=applyColorspaceAliases(colorspaceDoc);// After the aliases (so two spellings of one colorspace still match)
// and before the transforms, which would otherwise insert one
// conversion chain per duplicate image.
imageMergeResult=mergeDuplicateImageNodes(colorspaceDoc);if(DEBUG_SHADERS&&imageMergeResult.merged){console.log('[mtlx] merged '+imageMergeResult.merged+' duplicate image node(s) for "'+label+'":',imageMergeResult.groups);}// Must follow the aliases: an authored "srgb_tx" only becomes a
// name cmlib knows once applyColorspaceAliases has normalized it.
colorspaceTransformResult=applyColorspaceTransforms(colorspaceDoc);// Scene-only "Material working space" setting: never set outside the
...
if(sceneFeatureOptions&&sceneFeatureOptions.materialWorkspace==='acescg'){try{materialWorkspaceResult=applyMaterialWorkspaceTransforms(colorspaceDoc);}catch(e){// Safe-fail: leave the material in Rec.709 rather than throw.
mtlxWarn('Material working space (ACEScg) conversion failed, staying in Rec.709: '+(e&&e.message||e));materialWorkspaceResult=null;}}}// Catches a MaterialX type mismatch (no implicit coercion) BEFORE
...
const powerNodeNames=materialPowerNodeNames(colorspaceDoc);const mismatch=findTypeMismatches(renderable,mx);if(mismatch){throw new Error(`Material "${label}": input "${mismatch.inputName}" on "${mismatch.nodeName}" `+`(${mismatch.inputType}) is connected to "${mismatch.sourceName}" (${mismatch.sourceType}); `+'MaterialX requires matching types');}let mxShader;const __genPerfStart=window.MTLX_PERF_LOG?performance.now():0;try{try{mxShader=gen.generate('PreviewShader',renderable,genContext);}catch(genErr){// Decode the REAL MaterialX error (Emscripten throws
// numeric pointers) instead of a generic string, then name the
// node types behind it, which MaterialX's own message omits.
const detail=unresolvedNodesText(describeUnresolvedNodes(renderable));throw new Error(`Shader generation failed for "${label}": ${mxErr(mx,genErr)}`+(detail?`. ${detail}`:''));}}finally{// Reverse order: the transforms were layered on top of the aliases.
if(materialWorkspaceResult)materialWorkspaceResult.restore();if(colorspaceTransformResult)colorspaceTransformResult.restore();if(imageMergeResult)imageMergeResult.restore();if(colorspaceAliasResult)colorspaceAliasResult.restore();}if(window.MTLX_PERF_LOG){console.log('[mtlx-perf] gen.generate: '+(performance.now()-__genPerfStart).toFixed(1)+'ms (target: '+label+')');}// Stage identifiers: some JS builds don't expose the mx.Stage enum
// object ("Cannot read ... 'VERTEX'"). The underlying constants are
// just the strings "vertex"/"pixel", which getSourceCode accepts.
const VERTEX_STAGE=mx.Stage&&mx.Stage.VERTEX||'vertex';const PIXEL_STAGE=mx.Stage&&mx.Stage.PIXEL||'pixel';let vs=stripVersion(mxShader.getSourceCode(VERTEX_STAGE));// hwSrgbEncodeOutput=false means raw linear output, so encodeDisplay()'s
...
let fs=stripVersion(mxShader.getSourceCode(PIXEL_STAGE));fs=patchMaterialPowerNodes(fs,powerNodeNames);({vs,fs}=patchGeompropVaryings(vs,fs));const vertexInputs=parseVertexInputs(vs);// geompropvalue nodes carry a `default` for when the stream is absent;
// collect it so binding can use it instead of zeros.
const geompropDefaults=materialGeompropDefaults(colorspaceDoc);const geomprops=vertexInputs.filter(v=>v.name.startsWith('i_geomprop_')).map(v=>{const name=v.name.slice('i_geomprop_'.length);const defaultValue=geompropDefaults.get(name)||null;return defaultValue?{name,type:v.type,defaultValue}:{name,type:v.type};});const notices=[];if(colorspaceAliasResult){for(const[key,count]of colorspaceAliasResult.rewrites){const[from,to]=key.split(' -> ');notices.push(`Colorspace "${from}" is not a MaterialX 1.39 name; ${count} inputs treated as `+(to==='(removed)'?'no conversion':to));}}if(colorspaceTransformResult){for(const[cs,count]of colorspaceTransformResult.converted){notices.push(`Colorspace "${cs}": ${count} texture(s) converted to lin_rec709 in the shader`);}for(const cs of colorspaceTransformResult.unsupported){notices.push(`Colorspace "${cs}" has no conversion available; those textures are sampled unconverted`);}}fs=patchUnlitLightingRefs(fs);fs=patchScenePhysicalLightFalloff(fs,sceneRgbt);fs=patchSceneThinWalledTransmission(fs,sceneRgbt,notices);const outDeclMatch=fs.match(/\bout\s+vec4\s+(\w+)\s*;/);const outVar=outDeclMatch?outDeclMatch[1]:null;const outAssignments=outVar?fs.match(new RegExp('\\b'+outVar+'\\s*=[^;]*;','g')):null;if(!outVar||!outAssignments||!outAssignments.length){mtlxWarn(`mtlx-engine: could not locate the fragment output assignment for "${label}", skipping encodeDisplay() as a fail-safe (cannot verify it's safe to inject ACES+sRGB without double-encoding).`);}else if(/srgb/i.test(outAssignments.join('\n'))){// Self-encoding materials skip the epilogue entirely (no
...
mtlxWarn(`mtlx-engine: the fragment output assignment for "${label}" already calls an sRGB encode (despite hwSrgbEncodeOutput=false): skipping encodeDisplay() to avoid double-encoding (ACES tone mapping will NOT be applied to this material).`);}else{fs=encodeDisplay(fs);}// Local reflections substitute the dome lookup's `Li` BEFORE screen-space
...
fs=patchLocalEnvironmentRadiance(fs,{skipLocalEnv,notices});// Screen-space reflections wrap the generated IBL call before the
// refraction/peel patches touch the shader.
fs=patchScreenSpaceReflection(fs,{skipSsr,notices});// Folds transmission into peel-pass alpha; must precede injectPeelDiscard (see its u_peelMode guard).
fs=patchTransmissionAlpha(fs,{skipRefraction});let payloadSupported=false;if(sceneRgbt){fs=patchRgbtPayload(fs);payloadSupported=fs.indexOf('/* MX_RGBT_PAYLOAD_SUPPORTED */')!==-1;}let lightTransportSupported=false;if(lightTransport){fs=patchLightTransportPayload(fs,notices,lightTransport);lightTransportSupported=fs.indexOf('MX_LIGHT_TRANSPORT_TERMINAL_RETURN')!==-1;}fs=patchShadowBounds(fs);fs=patchShadowLightScope(fs,{skipTransmittance});fs=patchLightSourceKindStruct(fs);fs=patchAreaLightSourceCosine(fs);// skipOcclusion drops the whole block (screen-space AO included), which
// only ever ran at strength 0 in a tool that cannot bind an AO pass.
if(!skipOcclusion)fs=patchAmbientOcclusion(fs,{skipSkyVis,skipAoVolume});fs=patchDiffuseBounceAdd(fs,{skipSkyVis,skipBounce});fs=patchSpecularAA(fs,{specularAA});fs=patchTransmissionThickness(fs,{dropThicknessMap});// Depth-peel machinery: baked into every fragment shader
...
fs=injectPeelDiscard(fs,payloadSupported);// Uniform introspection, still fully inside the mxExclusive lock:
...
let introspected=[];for(const stageName of[VERTEX_STAGE,PIXEL_STAGE]){let st=null;try{st=mxShader.getStage(stageName);}catch(e){/* stage absent */}if(st)introspected=introspected.concat(collectMxUniforms(st));}introspected=introspected.map(plainizeMxUniformData);introspected=annotateFilenameSamplerModes(introspected,colorspaceDoc||documentArg);// uv_scale/uv_offset are ALWAYS emitted as uniforms (never inline
...
fs=applyHeightToNormalTexel(fs,notices,introspected);// Const inputs: the last source pass, so every regex above still sees
// the declarations it matches on. Pruned entries leave `introspected`
// so nothing tries to bind a uniform that no longer exists.
let constInputs=[];if(allowConstInputs!==false&&readConstInputs()){const constified=constifyInputUniforms(vs,fs,introspected);vs=constified.vs;fs=constified.fs;introspected=constified.introspected;constInputs=constified.constInputs;}// Last reference to mxShader, free it here, still inside the lock.
...
try{mxShader.delete();}catch(e){/* already deleted */}// Displacement is generated as its own tiny standalone program, never
// let a failure here break the surface material; any problem becomes
// a notice on the returned sources instead of a throw.
let displacement=null;try{const __dispGenStart=window.MTLX_PERF_LOG?performance.now():0;displacement=generateDisplacementSourcesUnlocked({mx,gen,genContext,renderable,materialName,allowConstInputs});if(displacement&&window.MTLX_PERF_LOG)displacement.genMs=performance.now()-__dispGenStart;if(displacement&&displacement.notices&&displacement.notices.length){notices.push(...displacement.notices);}}catch(e){notices.push('Displacement skipped: '+(e&&e.message?e.message:String(e)));displacement=null;}// What this source does NOT contain, so uniform seeding can skip the
// same features instead of binding samplers no program declares.
const featureSkips={shadowMap:skipShadowMap,occlusion:skipOcclusion,skyVis:skipOcclusion||skipSkyVis,aoVolume:skipOcclusion||skipSkyVis||skipAoVolume};return{vs,fs,introspected,transparent,vertexInputs,geomprops,notices,payloadSupported,lightTransportSupported,displacement,maxLights,constInputs,featureSkips};};// Follows one displacementshader-typed input to the element to generate
// from: the connected node, or the connected nodegraph's Output element
// when the input names a nodegraph and output.
const mxFollowDisplacementInput=(input,doc)=>{if(!input)return null;const nodeName=mxElAttr(input,'nodename');if(nodeName)return mxSafe(()=>input.getConnectedNode(),null);const graphName=mxElAttr(input,'nodegraph');if(graphName){const ng=mxSafe(()=>doc.getNodeGraph(graphName),null);const outName=mxElAttr(input,'output');return ng&&outName?mxSafe(()=>ng.getOutput(outName),null):null;}return null;};// resolveDisplacementSource: finds the element to generate the standalone
...
const resolveDisplacementSource=({mx,renderable,materialName,notices=null})=>{if(!renderable)return null;if(mxElType(renderable)==='displacementshader')return renderable;const doc=mxSafe(()=>renderable.getDocument(),null);if(!doc)return null;if(materialName){const matNode=mxSafe(()=>doc.getNode(materialName),null);if(matNode&&mxElType(matNode)==='material'){const el=mxFollowDisplacementInput(mxSafe(()=>matNode.getInput('displacementshader'),null),doc);if(el)return el;}}const renderableName=mxElName(renderable);let allNodes=[];try{allNodes=vecToArray(doc.getNodes?doc.getNodes():null);}catch(e){allNodes=[];}let found=null;const distinct=new Set();for(const n of allNodes){if(mxElType(n)!=='material')continue;const surfInput=mxSafe(()=>n.getInput('surfaceshader'),null);if(!surfInput||mxElAttr(surfInput,'nodename')!==renderableName)continue;const el=mxFollowDisplacementInput(mxSafe(()=>n.getInput('displacementshader'),null),doc);if(!el)continue;distinct.add(mxElName(el)+'|'+mxElCat(el));if(!found)found=el;}if(found&&distinct.size>1&&notices){notices.push('Multiple materials connect a different displacement to "'+renderableName+'"; using the first one found');}return found;};// detectDisplacementMode: a `displacement` node's own input type; a `mix`
// of displacementshader recurses into fg/bg; a nodegraph output recurses
// into its connected node; anything else is `auto`.
const detectDisplacementMode=element=>{if(!element)return'auto';const category=mxElCat(element);if(category==='displacement'){const t=mxElType(mxSafe(()=>element.getInput('displacement'),null));return t==='float'||t==='vector3'?t:'auto';}if(category==='mix'&&mxElType(element)==='displacementshader'){const fgMode=detectDisplacementMode(mxSafe(()=>{const i=element.getInput('fg');return i?i.getConnectedNode():null;},null));const bgMode=detectDisplacementMode(mxSafe(()=>{const i=element.getInput('bg');return i?i.getConnectedNode():null;},null));if(fgMode==='float'&&bgMode==='float')return'float';if(fgMode==='vector3'||bgMode==='vector3')return'vector3';// mixed float+vector3 treated as tangent-space vector
return'auto';}const connected=mxSafe(()=>element.getConnectedNode?element.getConnectedNode():null,null);return connected&&connected!==element?detectDisplacementMode(connected):'auto';};// FNV-1a over a UTF-16 JS string, good enough for a change-detection key
// (not cryptographic). Returns an 8-char hex string.
const fnv1aHex=str=>{let h=0x811c9dc5;for(let i=0;i<str.length;i++){h^=str.charCodeAt(i);h=Math.imul(h,0x01000193);}return(h>>>0).toString(16).padStart(8,'0');};// generateDisplacementSourcesUnlocked: generates the standalone displacement
...
const generateDisplacementSourcesUnlocked=({mx,gen,genContext,renderable,materialName,allowConstInputs})=>{const notices=[];const source=resolveDisplacementSource({mx,renderable,materialName,notices});if(!source)return null;const mode=detectDisplacementMode(source);const document=mxSafe(()=>source.getDocument(),null);let colorspaceAliasResult=null;let colorspaceTransformResult=null;if(document){colorspaceAliasResult=applyColorspaceAliases(document);colorspaceTransformResult=applyColorspaceTransforms(document);}const powerNodeNames=materialPowerNodeNames(document);let prevHwTransparency;let hadHwTransparency=true;try{prevHwTransparency=genContext.getOptions().hwTransparency;}catch(e){hadHwTransparency=false;}let shader=null;try{try{genContext.getOptions().hwTransparency=false;}catch(e){/* option absent */}try{shader=gen.generate('mtlx_displacement',source,genContext);}catch(genErr){const msg=mxErr(mx,genErr);if(/mix/i.test(msg)&&/implementation/i.test(msg)){throw new Error('Displacement mixing is not supported by the MaterialX shader generator.');}throw new Error('shader generation failed ('+msg.slice(0,160)+')');}const VERTEX_STAGE=mx.Stage&&mx.Stage.VERTEX||'vertex';const PIXEL_STAGE=mx.Stage&&mx.Stage.PIXEL||'pixel';let vs=stripVersion(shader.getSourceCode(VERTEX_STAGE));let fs=stripVersion(shader.getSourceCode(PIXEL_STAGE));fs=patchMaterialPowerNodes(fs,powerNodeNames);({vs,fs}=patchGeompropVaryings(vs,fs));const vertexInputs=parseVertexInputs(vs);const geompropDefaults=materialGeompropDefaults(document);const geomprops=vertexInputs.filter(input=>input.name.startsWith('i_geomprop_')).map(input=>{const name=input.name.slice('i_geomprop_'.length);const defaultValue=geompropDefaults.get(name)||null;return defaultValue?{name,type:input.type,defaultValue}:{name,type:input.type};});let introspected=[];for(const stageName of[VERTEX_STAGE,PIXEL_STAGE]){let st=null;try{st=shader.getStage(stageName);}catch(e){/* stage absent */}if(st)introspected=introspected.concat(collectMxUniforms(st));}introspected=introspected.map(plainizeMxUniformData);introspected=annotateFilenameSamplerModes(introspected,document);// Const inputs, before the splices so the rewritten declarations are
// still the plain generator output the regexes below expect.
let constInputs=[];if(allowConstInputs!==false&&readDisplacementConstInputs()){const constified=constifyInputUniforms(vs,fs,introspected,DISPLACEMENT_CONST_INPUT_NAMES);vs=constified.vs;fs=constified.fs;introspected=constified.introspected;constInputs=constified.constInputs;}// Pixel splice: retarget the discarded final assignment into a
// little-endian floatBitsToUint pack of one offset*scale component.
const outMatch=fs.match(/\bout\s+vec4\s+(\w+)\s*;/);const structMatches=[...fs.matchAll(/displacementshader\s+(\w+)\s*=/g)];const outVar=outMatch?outMatch[1]:null;const structVar=structMatches.length?structMatches[structMatches.length-1][1]:null;let psSpliced=false;if(outVar&&structVar){const finalRe=new RegExp('\\b'+outVar+'\\s*=\\s*vec4\\(\\s*0\\.0\\s*,\\s*0\\.0\\s*,\\s*0\\.0\\s*,\\s*1\\.0\\s*\\)\\s*;(?=\\s*\\})');if(finalRe.test(fs)){const pack='{\n'+'        uint dispBits = floatBitsToUint(('+structVar+'.offset * '+structVar+'.scale)[u_dispComponent]);\n'+'        '+outVar+' = vec4(float(dispBits & 0xFFu), float((dispBits >> 8u) & 0xFFu), '+'float((dispBits >> 16u) & 0xFFu), float((dispBits >> 24u) & 0xFFu)) / 255.0;\n'+'    }';fs=fs.replace(finalRe,pack);fs=fs.replace(/\bvoid\s+main\s*\(/,'uniform int u_dispComponent;\nvoid main(');psSpliced=true;}}// Vertex splice: rasterize into a readback grid instead of the view.
const vsAnchorRe=/gl_Position\s*=\s*u_viewProjectionMatrix\s*\*\s*hPositionWorld\s*;/;let vsSpliced=false;if(vsAnchorRe.test(vs)){vs=vs.replace(vsAnchorRe,'gl_Position = vec4(i_dispTexel, 0.0, 1.0);\n    gl_PointSize = 1.0;');vs=vs.replace(/\bvoid\s+main\s*\(/,'in vec2 i_dispTexel;\nvoid main(');vsSpliced=true;}if(!psSpliced||!vsSpliced){mtlxWarn('mtlx-engine: displacement splice anchor missing, '+(!vsSpliced?'vertex: '+vs.slice(0,200):'pixel: '+fs.slice(0,200)));throw new Error('generated shader anchors not found (vertex or pixel); MaterialX codegen changed');}const key=fnv1aHex(vs+String.fromCharCode(0)+fs+String.fromCharCode(0)+JSON.stringify(introspected.map(u=>({name:u.name,type:u.type,data:u.data,samplerModes:u.samplerModes||null}))));return{vs,fs,introspected,geomprops,mode,key,notices,constInputs};}finally{if(colorspaceTransformResult)colorspaceTransformResult.restore();if(colorspaceAliasResult)colorspaceAliasResult.restore();if(hadHwTransparency){try{genContext.getOptions().hwTransparency=prevHwTransparency;}catch(e){/* option absent */}}try{shader&&shader.delete&&shader.delete();}catch(e){/* already deleted */}}};// Public entry point: serializes generatePreviewSourcesUnlocked against
...
const generatePreviewSources=(...args)=>mxExclusive(()=>generatePreviewSourcesUnlocked(...args));// ANGLE D3D11 reports 16 texture image units; a Scene material bakes twelve
...
const DEFAULT_SAMPLER_BUDGET=16;// Drop order when a program exceeds the sampler budget; each { key, label }
...
const joinWithAnd=items=>items.length<=1?items.join(''):items.slice(0,-1).join(', ')+' and '+items[items.length-1];const SAMPLER_BUDGET_DROP_ORDER=[// Parked with screen-space reflections (always skipped for now).
// { key: 'skipSsr', label: 'screen-space reflection (u_opaqueColor)' },
{key:'skipLocalEnv',label:'local reflection capture (u_localEnvRadiance)',userLabel:'local reflections'},{key:'skipBounce',label:'bounce volume (u_skyBounceMap)',userLabel:'diffuse bounce'},{key:'skipAoVolume',label:'occlusion volume (u_aoVolumeMap)',userLabel:'ambient occlusion'},{key:'skipSkyVis',label:'sky visibility (u_skyVisMap)',userLabel:'sky visibility'},{key:'dropThicknessMap',label:'thickness map (u_thicknessMap)',userLabel:'transmission thickness'},{key:'skipTransmittance',label:'shadow transmittance (u_shadowTransmittance)',userLabel:'colored shadows through transparent materials'},{key:'skipRefraction',label:'refraction colour (u_opaqueColor)',userLabel:'refraction'}];// One accurate sentence for a material that ran out of texture units:
// what it needed, where those samplers came from, and what was turned off.
const samplerBudgetNotice=({needed,limit,effects,plural})=>'needs '+needed.count+' texture samplers ('+needed.material+' from the material, '+needed.scene+' from the scene) but this GPU allows '+limit+', so '+effects+(plural?' are':' is')+' turned off for it';// Viewer-path generation with the same sampler budget as the Scene: drops
...
const generatePreviewSourcesWithinBudget=async args=>{const overrideBudget=typeof window!=='undefined'?window.__mtlxSamplerBudgetOverride:undefined;const budget=Number.isFinite(overrideBudget)?overrideBudget:DEFAULT_SAMPLER_BUDGET;const dropped=[];let srcs=null;let neededInfo=null;// the full-feature count, what the notice reports
for(let attempt=0;;attempt++){// The caller's feature gating is the base; budget drops add to it.
const sceneFeatureOptions=Object.assign({},args.sceneFeatureOptions||null);for(const d of dropped)sceneFeatureOptions[d.key]=true;srcs=await generatePreviewSources(Object.assign({},args,{sceneFeatureOptions}));if(!srcs)return null;const info=countFragmentSamplers(srcs.fs);if(!neededInfo)neededInfo=info;if(info.count<=budget)break;if(attempt>=SAMPLER_BUDGET_DROP_ORDER.length)break;dropped.push(SAMPLER_BUDGET_DROP_ORDER[attempt]);}if(dropped.length){const info=countFragmentSamplers(srcs.fs);const effects=joinWithAnd(dropped.map(d=>d.userLabel));const needed=samplerBudgetNotice({needed:neededInfo||info,limit:budget,effects,plural:dropped.length>1});srcs.notices=(srcs.notices||[]).concat(['Texture slots: this material '+needed]);srcs.samplerBudget={limit:budget,count:info.count,material:info.material,scene:info.scene,needed:(neededInfo||info).count,notice:needed,dropped:dropped.map(d=>d.label),droppedLabels:dropped.map(d=>d.userLabel)};}return srcs;};// Scene-view material compiler. This deliberately exposes the preview shader
...
const sceneFeatureOptions=Object.assign({materialWorkspace,specularAA},featureOptions||null);for(const d of dropped)sceneFeatureOptions[d.key]=true;srcs=await generatePreviewSources({mx,gen,genContext,renderable,label,materialName,isMounted,document:documentArg,sceneRgbt,lightTransport,sceneFeatureOptions,stageLightCount});if(!srcs)return null;samplerInfo=countFragmentSamplers(srcs.fs);if(!neededInfo)neededInfo=samplerInfo;if(samplerInfo.count<=budget)break;if(attempt>=SAMPLER_BUDGET_DROP_ORDER.length)break;// hooks exhausted, still over
dropped.push(SAMPLER_BUDGET_DROP_ORDER[attempt]);}const declared=parseUniforms(srcs.vs).concat(parseUniforms(srcs.fs));const overBudget=samplerInfo.count>budget;const uniformInfo=estimateFragmentUniformVectors(srcs.fs);return{...srcs,declared,// Program identity excludes uniforms and object transforms. Source
...
notice:dropped.length?samplerBudgetNotice({needed:neededInfo||samplerInfo,limit:budget,effects:joinWithAnd(dropped.map(d=>d.userLabel)),plural:dropped.length>1}):null},samplerOverBudget:overBudget,budgetAttempts,fragmentUniformVectors:{estimate:uniformInfo.estimate,limit:uniformLimit,largest:uniformInfo.largest},fragmentUniformOverBudget:uniformInfo.estimate>uniformLimit};};// Create a detached uniform map for one scene object. Every call returns a
...
const createMtlxSceneUniforms=({compiled,env=null,lightData=[],stageLights=null,shadowMap=null,shadowMatrix=null,ssaoMap=null,ssaoTexel=null,ssaoStrength=1,thicknessMap=null,thicknessTexel=null,thicknessScale=1,refractionTwoSided=false,sceneRadius=1,envTilt=null,envRotationRad=0,envExposure=1,environmentIndirectScale=1,environmentKeyScale=1,lightScales=null,shadowDiagnosticVisibilityScale=1,displayTransform=null,shadowAtlas=null,shadowMatrices=null,shadowTiles=null,shadowDepthPlanes=null,shadowDepthRanges=null,shadowSourceRadii=null,shadowTexelSizes=null,shadowFaceOrigins=null,shadowFaceValid=null,shadowFaceBasisX=null,shadowFaceBasisY=null,shadowFaceBasisZ=null,shadowSlotFace=null,shadowSlotFaceCount=null,shadowTransmittance=null,shadowRecordCells=null,skyVisMap=null,skyVisMin=null,skyVisSize=null,skyVisStrength=1,skyVisCell=0,aoVolumeMap=null,aoVolumeMin=null,aoVolumeSize=null,aoVolumeStrength=1,aoVolumeCell=0,skyBounceMap=null,skyBounceMin=null,skyBounceSize=null,skyBounceStrength=0,skyBounceCell=0,bounceScale=0,bounceTint=null,localEnvMap=null,localEnvMips=1,localEnvStrength=0,localEnvProbe=null,localEnvBoxMin=null,localEnvBoxMax=null,localEnvParallax=0})=>{if(!compiled)throw new Error('Cannot create scene uniforms without compiled MaterialX source.');const uniforms={u_worldMatrix:{value:new THREE.Matrix4()},u_viewProjectionMatrix:{value:new THREE.Matrix4()},u_worldInverseTransposeMatrix:{value:new THREE.Matrix4()},u_viewPosition:{value:new THREE.Vector3()},u_peelMode:{value:0},u_peelHasPrev:{value:0},u_peelPrevDepth:{value:getDummyTex()},u_opaqueDepth:{value:getDummyTexWhite()},u_peelLinear:{value:0},// Injected by encodeDisplay, so these are never in MaterialX's own
...
u_displayExposure:{value:displayExposureScale()},u_displayTransform:{value:displayTransformId(displayTransform||getDisplayTransform())}};// A feature this source never generated declares no uniform for it, so
...
const featureSkips=compiled&&compiled.featureSkips||{};if(!featureSkips.shadowMap)Object.assign(uniforms,{// Shadow atlas. Seeded whenever the source carries shadow sampling,
// for the same sampler-unit reason as the sky volume below, and
// defaulted to "no caster on any slot": an exact no-op.
u_shadowAtlas:{value:shadowAtlas||getDummyTexWhite()},u_shadowMatrices:{value:shadowMatrices&&shadowMatrices.length===SHADOW_FACE_SLOTS?shadowMatrices:Array.from({length:SHADOW_FACE_SLOTS},()=>new THREE.Matrix4())},u_shadowTiles:{value:shadowTiles&&shadowTiles.length===SHADOW_FACE_SLOTS?shadowTiles:Array.from({length:SHADOW_FACE_SLOTS},()=>new THREE.Vector4(0,0,1,1))},u_shadowDepthPlanes:{value:shadowDepthPlanes&&shadowDepthPlanes.length===SHADOW_FACE_SLOTS?shadowDepthPlanes:Array.from({length:SHADOW_FACE_SLOTS},()=>new THREE.Vector4(0,0,0,1))},u_shadowDepthRanges:{value:shadowDepthRanges&&shadowDepthRanges.length===SHADOW_FACE_SLOTS?shadowDepthRanges:Array.from({length:SHADOW_FACE_SLOTS},()=>new THREE.Vector2(0,1))},u_shadowSourceRadii:{value:shadowSourceRadii&&shadowSourceRadii.length===SHADOW_FACE_SLOTS?shadowSourceRadii:Array.from({length:SHADOW_FACE_SLOTS},()=>new THREE.Vector4())},u_shadowTexelWorldSize:{value:shadowTexelSizes&&shadowTexelSizes.length===SHADOW_FACE_SLOTS?shadowTexelSizes:new Array(SHADOW_FACE_SLOTS).fill(0)},// Light position for a cube-group face, and whether a face actually
...
u_shadowFaceOrigin:{value:shadowFaceOrigins&&shadowFaceOrigins.length===SHADOW_FACE_SLOTS?shadowFaceOrigins:Array.from({length:SHADOW_FACE_SLOTS},()=>new THREE.Vector3())},u_shadowFaceValid:{value:shadowFaceValid&&shadowFaceValid.length===SHADOW_FACE_SLOTS?shadowFaceValid:new Array(SHADOW_FACE_SLOTS).fill(0)},u_shadowFaceBasisX:{value:shadowFaceBasisX&&shadowFaceBasisX.length===SHADOW_FACE_SLOTS?shadowFaceBasisX:Array.from({length:SHADOW_FACE_SLOTS},()=>new THREE.Vector3(1,0,0))},u_shadowFaceBasisY:{value:shadowFaceBasisY&&shadowFaceBasisY.length===SHADOW_FACE_SLOTS?shadowFaceBasisY:Array.from({length:SHADOW_FACE_SLOTS},()=>new THREE.Vector3(0,1,0))},u_shadowFaceBasisZ:{value:shadowFaceBasisZ&&shadowFaceBasisZ.length===SHADOW_FACE_SLOTS?shadowFaceBasisZ:Array.from({length:SHADOW_FACE_SLOTS},()=>new THREE.Vector3(0,0,1))},// Cloned, not aliased: applyShadowMatrix() writes into this uniform's
// own array in place, and a diagnostic swap must never corrupt the
// renderer's live shadowSlotFace/shadowSlotFaceCount state.
u_shadowSlotFace:{value:shadowSlotFace&&shadowSlotFace.length===SHADOW_LIGHT_SLOTS_MAX?new Int32Array(shadowSlotFace):new Int32Array(SHADOW_LIGHT_SLOTS_MAX).fill(-1)},u_shadowSlotFaceCount:{value:shadowSlotFaceCount&&shadowSlotFaceCount.length===SHADOW_LIGHT_SLOTS_MAX?new Int32Array(shadowSlotFaceCount):new Int32Array(SHADOW_LIGHT_SLOTS_MAX).fill(0)},u_shadowDiagnosticVisibilityScale:{value:Number.isFinite(Number(shadowDiagnosticVisibilityScale))?Math.max(0,Number(shadowDiagnosticVisibilityScale)):1}});if(!featureSkips.occlusion)Object.assign(uniforms,{// Baked sky visibility. Seeded whenever the occlusion block was
...
u_skyVisMap:{value:skyVisMap||getDummyTex3DWhite()},u_skyVisMin:{value:skyVisMin?skyVisMin.clone():new THREE.Vector3()},u_skyVisSize:{value:skyVisSize?skyVisSize.clone():new THREE.Vector3(1,1,1)},u_skyVisCell:{value:skyVisCell||0},u_skyVisStrength:{value:skyVisMap?skyVisStrength:0},// Baked occlusion volume, same sampler-unit hazard as u_skyVisMap
// above: a highp sampler3D is invisible to has()/parseUniforms, so
// this is seeded alongside it. White at strength 0: no-op.
u_aoVolumeMap:{value:aoVolumeMap||getDummyTex3DWhite()},u_aoVolumeMin:{value:aoVolumeMin?aoVolumeMin.clone():new THREE.Vector3()},u_aoVolumeSize:{value:aoVolumeSize?aoVolumeSize.clone():new THREE.Vector3(1,1,1)},u_aoVolumeCell:{value:aoVolumeCell||0},u_aoVolumeStrength:{value:aoVolumeMap?aoVolumeStrength:0}});// Baked diffuse bounce is patched in independently of the
// occlusion gate, so it is seeded whatever that gate did.
Object.assign(uniforms,{// Baked diffuse bounce, same sampler-unit hazard as the two volumes
...
u_skyBounceMap:{value:skyBounceMap||getDummyTex3DWhite()},u_skyBounceMin:{value:skyBounceMin?skyBounceMin.clone():new THREE.Vector3()},u_skyBounceSize:{value:skyBounceSize?skyBounceSize.clone():new THREE.Vector3(1,1,1)},u_skyBounceCell:{value:skyBounceCell||0},u_skyBounceStrength:{value:skyBounceMap?skyBounceStrength:0},// Denormalizes the baked scalar SH1 encoding and reintroduces the
...
u_bounceScale:{value:Number.isFinite(bounceScale)?Math.max(0,bounceScale):0},u_bounceTint:{value:bounceTint?bounceTint.clone():new THREE.Vector3(1,1,1)}});if(compiled.payloadSupported){// Scene RGB-T is opt-in at compile time and remains inactive until
// the compositor sets these selectors.
uniforms.u_peelRgbt={value:0};uniforms.u_peelRgbtPass={value:0};uniforms.u_peelRgbtLayer={value:0};}applyIntrospectedUniformDefaults(uniforms,compiled.introspected||[]);const declared=new Set((compiled.declared||[]).map(u=>u.name));const has=name=>declared.has(name);const radiance=envRadianceForShading(env)||getDummyTex();const irradiance=envIrradianceForShading(env)||radiance;const mips=env&&env.mips!=null?env.mips:1;if(has('u_time'))uniforms.u_time={value:MTLX_CLOCK.time};if(has('u_frame'))uniforms.u_frame={value:MTLX_CLOCK.frame};if(has('u_envRadiance'))uniforms.u_envRadiance={value:radiance};if(has('u_envIrradiance'))uniforms.u_envIrradiance={value:irradiance};for(const u of compiled.declared||[]){if(!/sampler/i.test(u.type)||!/env/i.test(u.name))continue;// "u_envIrradiance" contains "radiance", so the irradiance test
// must run first or the diffuse term binds the sharp radiance map.
if(/irradiance|diffuse/i.test(u.name))uniforms[u.name]={value:irradiance};else if(/radiance|specular|prefilter/i.test(u.name))uniforms[u.name]={value:radiance};}// envTilt carries a dome light's non-vertical orientation. The rotation
...
if(has('u_localEnvRadiance'))uniforms.u_localEnvRadiance={value:localEnvMap||getDummyTexWhite()};if(has('u_localEnvMips'))uniforms.u_localEnvMips={value:Number.isFinite(localEnvMips)?localEnvMips:1};if(has('u_localEnvStrength'))uniforms.u_localEnvStrength={value:localEnvMap?localEnvStrength:0};if(has('u_localEnvProbe'))uniforms.u_localEnvProbe={value:localEnvProbe?localEnvProbe.clone():new THREE.Vector3()};if(has('u_localEnvBoxMin'))uniforms.u_localEnvBoxMin={value:localEnvBoxMin?localEnvBoxMin.clone():new THREE.Vector3()};if(has('u_localEnvBoxMax'))uniforms.u_localEnvBoxMax={value:localEnvBoxMax?localEnvBoxMax.clone():new THREE.Vector3()};if(has('u_localEnvParallax'))uniforms.u_localEnvParallax={value:localEnvParallax?1:0};if(has('u_ssaoMap'))uniforms.u_ssaoMap={value:ssaoMap||getDummyTexWhite()};if(has('u_ssaoTexel'))uniforms.u_ssaoTexel={value:ssaoTexel?ssaoTexel.clone():new THREE.Vector2()};if(has('u_ssaoStrength'))uniforms.u_ssaoStrength={value:ssaoMap?ssaoStrength:0};// Zero scale means zero path length, which is clear glass: the safe
...
const authored=Number(uniforms.transmission_depth&&uniforms.transmission_depth.value);uniforms.u_thicknessReferencePath={value:has('u_thicknessMap')?0:Number.isFinite(authored)&&authored>0?authored:0};}// Squares the tint for a closed solid, where the ray crosses the surface
...
if(has('u_refractionTwoSided'))uniforms.u_refractionTwoSided={value:!!refractionTwoSided};// mx_scene_refraction: u_opaqueColor/Levels bind per peel pass like
...
if(has('u_opaqueColor'))uniforms.u_opaqueColor={value:getDummyTexWhite()};if(has('u_opaqueColorLevels'))uniforms.u_opaqueColorLevels={value:0};if(has('u_peelRefractsScene'))uniforms.u_peelRefractsScene={value:0};if(has('u_sceneRadius'))uniforms.u_sceneRadius={value:Math.max(0,Number(sceneRadius)||0)};// Screen-space reflections: off until the renderer's per-frame history
// is valid (see applySsrHistory in js/usd-scene-renderer.js).
if(has('u_ssrEnabled'))uniforms.u_ssrEnabled={value:0};if(has('u_ssrStrength'))uniforms.u_ssrStrength={value:1};if(has('u_ssrMaxRoughness'))uniforms.u_ssrMaxRoughness={value:0.5};if(has('u_historyViewProjectionMatrix'))uniforms.u_historyViewProjectionMatrix={value:new THREE.Matrix4()};if(has('u_historyViewProjectionInverseMatrix'))uniforms.u_historyViewProjectionInverseMatrix={value:new THREE.Matrix4()};if(has('u_historyViewPosition'))uniforms.u_historyViewPosition={value:new THREE.Vector3()};if(has('u_viewProjectionInverseMatrix'))uniforms.u_viewProjectionInverseMatrix={value:new THREE.Matrix4()};if(has('u_shadowMap'))uniforms.u_shadowMap={value:shadowMap||getDummyTexWhite()};if(has('u_shadowMatrix'))uniforms.u_shadowMatrix={value:shadowMatrix?shadowMatrix.clone():shadowOffMatrix()};if(has('u_lightData')){const entries=currentLights(lightData,env&&env.keyLight,envRotationRad,stageLights,envExposure*Math.max(0,Number(environmentKeyScale)||0),lightScales,compiled.maxLights);uniforms.u_lightData={value:entries};}if(has('u_numActiveLightSources'))uniforms.u_numActiveLightSources={value:activeLightCount(lightData,env&&env.keyLight,stageLights,compiled.maxLights)};return uniforms;};// Uniform map for a transfer light-transport variant: texture uniforms are
...
// ------------------------------------------------------------------
const PREVIEW_TRANSFORM_UNIFORM_NAMES=new Set(['u_worldMatrix','u_viewProjectionMatrix','u_worldInverseTransposeMatrix','u_viewPosition']);const applyIntrospectedUniformDefaults=(uniforms,introspected,{overwrite=false}={})=>{if(!overwrite){for(const u of introspected){if(uniforms[u.name]||u.data==null)continue;// explicit bindings win; no default → leave for WebGL 0
const tu=mxValueToThreeUniform(u.type,u.data);if(tu)uniforms[u.name]=tu;}// A filename sampler with no image samples a 1x1 texture of the
// node's `default` input; null (three's empty texture, so black)
// only when codegen published no default for it.
for(const u of introspected){if(u.type==='filename'&&!uniforms[u.name]){uniforms[u.name]={value:getFilenameDefaultTexture(introspected,u.name)};}}return;}// Fast-refresh: same values just recomputed from a re-generated
// (but byte-identical-source) shader, overwrite in place.
for(const u of introspected){// ONLY the public block: PrivateUniforms (transforms, env,
...
if(u.block!=='PublicUniforms')continue;if(u.data==null)continue;if(u.type==='filename')continue;// Belt-and-suspenders: the transforms are private-block (so the
// block guard above already skips them), but they're the one
// thing that would visibly break every frame if ever touched.
if(PREVIEW_TRANSFORM_UNIFORM_NAMES.has(u.name))continue;const tu=mxValueToThreeUniform(u.type,u.data);if(!tu)continue;if(uniforms[u.name])uniforms[u.name].value=tu.value;else uniforms[u.name]=tu;}// Fast-refresh keeps the live sampler bindings, so a `default` that
// changed has to re-bake its 1x1 texture here as well.
for(const u of introspected){if(u.type!=='filename')continue;const slot=uniforms[u.name];if(!samplerHoldsDefault(slot))continue;slot.value=getFilenameDefaultTexture(introspected,u.name);}};// evaluateDisplacement: draws one THREE.Points per vertex into an RGBA8
...
srcs=await generatePreviewSourcesWithinBudget({mx,gen,genContext,renderable,label,materialName,isMounted,stageLightCount:PREVIEW_STAGE_LIGHT_COUNT,sceneFeatureOptions:PREVIEW_FEATURE_OPTIONS,allowConstInputs:view?view.allowConstInputs!==false:true});}catch(e){return{refreshed:false,srcs:null};}if(!srcs)return{refreshed:false,srcs:null};// Belt-and-suspenders: compare the transparency verdict explicitly
...
const BG_BASE=Math.PI;const BG_SIGN=-1;// Neutral-material env rotation: r128 lacks a scene.environment rotation knob (arrives r162+), so
...
const effectiveFullSceneVFov=(authoredFovDeg,authoredAspect,canvasAspect)=>{if(canvasAspect>=authoredAspect)return authoredFovDeg;const authoredHalfVFov=authoredFovDeg*Math.PI/180/2;const authoredHalfHFov=Math.atan(Math.tan(authoredHalfVFov)*authoredAspect);const effHalfVFov=Math.atan(Math.tan(authoredHalfHFov)/canvasAspect);return effHalfVFov*2*180/Math.PI;};// ------------------------------------------------------------------
...
const STUDIO_SHADOW_OPACITY=0.28;const STUDIO_SHADOW_OPACITY_DARK=0.4;// dark backdrop needs a denser catcher to read against it
...
const STUDIO_SHADOW_MAP_SIZE=1024;// Procedural gradient shader, replacing a baked canvas texture: pixel-
...
const hexToVec3=hex=>{const n=parseInt(hex.slice(1),16);return new THREE.Vector3((n>>16&255)/255,(n>>8&255)/255,(n&255)/255);};// Light mirrors a paper cyclorama, dark mirrors the site's own
// background family (Tailwind gray-900, #111827).
const STUDIO_GRADIENT_STOPS={light:[hexToVec3('#f6f6f6'),hexToVec3('#ffffff'),hexToVec3('#e3e3e3'),hexToVec3('#c8c8c8')],dark:[hexToVec3('#1d2635'),hexToVec3('#242e40'),hexToVec3('#131a28'),hexToVec3('#0c1220')]};// Soft hotspot high on the wall, reads as a key-light wash with no
// actual scene light. Position/radius are baked into the fragment shader below.
const STUDIO_HOTSPOT={light:{color:new THREE.Vector3(1,1,1),alpha:0.55},dark:{color:new THREE.Vector3(151/255,170/255,200/255),alpha:0.18}};const STUDIO_GRADIENT_VERTEX_SHADER=`
varying vec2 vUv;
void main() {
    vUv = uv;
...
const applyStudioVariantUniforms=(material,dark)=>{const stops=dark?STUDIO_GRADIENT_STOPS.dark:STUDIO_GRADIENT_STOPS.light;const hotspot=dark?STUDIO_HOTSPOT.dark:STUDIO_HOTSPOT.light;material.uniforms.uStop0.value.copy(stops[0]);material.uniforms.uStop1.value.copy(stops[1]);material.uniforms.uStop2.value.copy(stops[2]);material.uniforms.uStop3.value.copy(stops[3]);material.uniforms.uHotspotColor.value.copy(hotspot.color);material.uniforms.uHotspotA.value=hotspot.alpha;};// Shared lathe profile, a CLOSED room: floor centre, flat floor, fillet,
...
const buildStudioProfile=()=>{// LatheGeometry sets uv.y from the point INDEX, not arc length, so
// the profile is emitted at a uniform step. A coarse floor/wall
// would otherwise squeeze the whole gradient into the fillet.
const wallH=STUDIO_WALL_H-STUDIO_FILLET_R;const filletLen=Math.PI/2*STUDIO_FILLET_R;const segsFor=len=>Math.max(1,Math.round(len/STUDIO_PROFILE_STEP));const floorSegs=segsFor(STUDIO_FLOOR_R);const filletSegs=segsFor(filletLen);const wallSegs=segsFor(wallH);const points=[];for(let i=0;i<=floorSegs;i++){points.push(new THREE.Vector2(i/floorSegs*STUDIO_FLOOR_R,0));}for(let i=1;i<=filletSegs;i++){const t=i/filletSegs*(Math.PI/2);points.push(new THREE.Vector2(STUDIO_FLOOR_R+Math.sin(t)*STUDIO_FILLET_R,(1-Math.cos(t))*STUDIO_FILLET_R));}for(let i=1;i<=wallSegs;i++){points.push(new THREE.Vector2(STUDIO_WALL_R,STUDIO_FILLET_R+i/wallSegs*wallH));}// Ceiling: the floor's fillet and disc mirrored, closing the room
// so no camera angle inside it can see past the rim to the page.
const ceilY=STUDIO_WALL_H+STUDIO_FILLET_R;for(let i=1;i<=filletSegs;i++){const t=i/filletSegs*(Math.PI/2);points.push(new THREE.Vector2(STUDIO_FLOOR_R+Math.cos(t)*STUDIO_FILLET_R,STUDIO_WALL_H+Math.sin(t)*STUDIO_FILLET_R));}for(let i=1;i<=floorSegs;i++){points.push(new THREE.Vector2((1-i/floorSegs)*STUDIO_FLOOR_R,ceilY));}return points;};// Offsets a profile inward by `inset`, from the local tangent at each
// point (forward diff at the first, backward at the last, central
// elsewhere) rotated +90 degrees: (x,y) -> (-y,x). Points into the room.
const insetStudioProfile=(points,inset)=>{const last=points.length-1;return points.map((p,i)=>{const prev=points[Math.max(0,i-1)];const next=points[Math.min(last,i+1)];const tx=next.x-prev.x;const ty=next.y-prev.y;const len=Math.hypot(tx,ty)||1;return new THREE.Vector2(p.x+-ty/len*inset,p.y+tx/len*inset);});};// Shared bowl geometry, guarded so a missing THREE.LatheGeometry can't
// throw here.
let studioLatheGeometry=null;const getStudioGeometry=()=>{if(studioLatheGeometry)return studioLatheGeometry;try{if(!THREE.LatheGeometry)return null;studioLatheGeometry=new THREE.LatheGeometry(buildStudioProfile(),64);}catch(e){studioLatheGeometry=null;// no studio backdrop this session; bgMesh/no-backdrop modes still work
}return studioLatheGeometry;};// The BACKDROP gets its own copy, pushed OUTWARD off the true bowl, so the
...
let studioBackdropLatheGeometry=null;const getStudioBackdropGeometry=()=>{if(studioBackdropLatheGeometry)return studioBackdropLatheGeometry;try{if(!THREE.LatheGeometry)return null;const outset=insetStudioProfile(buildStudioProfile(),-STUDIO_BACKDROP_OFFSET);studioBackdropLatheGeometry=new THREE.LatheGeometry(outset,64);}catch(e){studioBackdropLatheGeometry=null;// caller degrades along with getStudioGeometry
}return studioBackdropLatheGeometry;};// Small public bridge for the USD scene view. The scene renderer must use the
...
const createUsdSceneStudioMaterial=(dark=false)=>{const material=new THREE.ShaderMaterial({uniforms:{uStop0:{value:new THREE.Vector3()},uStop1:{value:new THREE.Vector3()},uStop2:{value:new THREE.Vector3()},uStop3:{value:new THREE.Vector3()},uHotspotColor:{value:new THREE.Vector3()},uHotspotA:{value:0},uLinearOut:{value:0}},vertexShader:STUDIO_GRADIENT_VERTEX_SHADER,fragmentShader:STUDIO_GRADIENT_FRAGMENT_SHADER(getDisplayTransform()),side:THREE.BackSide,fog:false});applyStudioVariantUniforms(material,!!dark);return material;};// Refresh the display-baked fragment stage on an existing USD backdrop. The
...
const refreshUsdSceneStudioMaterial=(material,dark=false)=>{if(!material)return false;material.fragmentShader=STUDIO_GRADIENT_FRAGMENT_SHADER(getDisplayTransform());applyStudioVariantUniforms(material,!!dark);material.needsUpdate=true;return true;};const getUsdSceneStudioGeometry=()=>{const geometry=getStudioBackdropGeometry();return geometry&&geometry.clone?geometry.clone():geometry;};const getUsdSceneStudioCatcherGeometry=()=>{const geometry=getStudioGeometry();return geometry&&geometry.clone?geometry.clone():geometry;};// Shared studio shadow rig for the USD Scene Viewer, so its cast shadow
...
const createUsdSceneStudioLight=(scale=1)=>{const light=new THREE.SpotLight(0xffffff,0);const target=new THREE.Object3D();light.target=target;light.castShadow=true;light.angle=Math.atan(STUDIO_LIGHT_CONE_R/STUDIO_LIGHT_DISTANCE);light.penumbra=0.5;light.shadow.camera.near=(STUDIO_LIGHT_DISTANCE-4)*scale;light.shadow.camera.far=(STUDIO_LIGHT_DISTANCE+STUDIO_WALL_R+2)*scale;light.shadow.mapSize.set(STUDIO_SHADOW_MAP_SIZE,STUDIO_SHADOW_MAP_SIZE);// Stage meshes rest on the floor, so the contact shadow must start at
// the base: a smaller blur and normal bias than the shaderball rig.
light.shadow.radius=6;light.shadow.bias=-0.0005;light.shadow.normalBias=0.004*scale;return{light,target};};// Mirrors placeStudioLight (js/mtlx-engine.js:4657-4678) but relative to
...
const placeUsdSceneStudioLight=(light,center,direction,scale=1)=>{if(!light)return;const toLightDir=direction.clone().negate();const minY=Math.sin(STUDIO_LIGHT_MIN_ELEV_RAD);if(toLightDir.y<minY){const horizLen=Math.hypot(toLightDir.x,toLightDir.z);if(horizLen>1e-6){const s=Math.sqrt(Math.max(0,1-minY*minY))/horizLen;toLightDir.x*=s;toLightDir.z*=s;toLightDir.y=minY;}}light.position.copy(center).addScaledVector(toLightDir,STUDIO_LIGHT_DISTANCE*scale);if(light.target)light.target.position.copy(center);light.shadow.camera.near=(STUDIO_LIGHT_DISTANCE-4)*scale;light.shadow.camera.far=(STUDIO_LIGHT_DISTANCE+STUDIO_WALL_R+2)*scale;if(light.shadow.camera.updateProjectionMatrix)light.shadow.camera.updateProjectionMatrix();};window.MtlxStudio=Object.assign(window.MtlxStudio||{},{createUsdSceneStudioMaterial,refreshUsdSceneStudioMaterial,applyUsdSceneStudioVariant:applyStudioVariantUniforms,getUsdSceneStudioGeometry,getUsdSceneStudioCatcherGeometry,createUsdSceneStudioLight,placeUsdSceneStudioLight,backdropBaseRotation:BG_BASE,backdropRotationSign:BG_SIGN,keyLightRotationMatrix:rad=>keyLightRotationMatrix(rad),studioMaxPolar:STUDIO_MAX_POLAR,studioMaxOrbitDistance:STUDIO_MAX_ORBIT_DISTANCE,studioFloorClearance:STUDIO_FLOOR_CLEARANCE});// applyPeelMaterialMode(material, active): blend/depth flags for one
...
const applyPeelMaterialMode=(material,active)=>{if(!material)return;const blending=active?THREE.NoBlending:THREE.NormalBlending;const changed=material.blending!==blending;material.blending=blending;material.transparent=false;material.depthTest=true;material.depthWrite=true;if(material.uniforms&&material.uniforms.u_peelMode)material.uniforms.u_peelMode.value=0;if(changed)material.needsUpdate=true;};// Three keeps a target's configured rectangle separate from the active GL
...
cameraDistance=3.6,// false (default) = fixed, non-interactive authored GLB camera (graph
// editor); true (docs/viewer) = OrbitControls with pivot/zoom/polar
// clamp and Box3 containment. Ignored outside full-scene mode.
sceneOrbit=false,// Caps setPixelRatio; lower it for cheap side-by-side/compare views.
maxPixelRatio=2,// Names the <material> element whose displacementshader input to
// follow (see resolveDisplacementSource); null scans the document.
materialName=null,// Highest triangle count this view's subdivided preview mesh may
...
triangleBudget=PREVIEW_TRIANGLE_BUDGET})=>{// See the isAlive doc above: defaulting to isMounted here preserves
// today's exact behavior for every caller that doesn't pass isAlive.
const aliveFn=isAlive||isMounted;// Mode derived from geomName: 'shaderball-scene' -> full authored GLB
...
const flat2d=geomName==='buffer2d';// Known before the renderer exists, so the shadow map can be configured
...
let originalGeometry=null,baseGeometry=null,displacedGeometry=null;let displacementSources=null,dispKey=null,dispToken=0;// dispState: 'none' | 'pending' | 'applied' | 'skipped' | 'failed' | 'off'.
let dispState='none',dispSubdivLevel=null,dispTriangles=0;let dispWithinBudget=true;let dispCappedNotice=null,dispDroppedNotice=null,dispEvalNotices=[];let materialNotices=[];let dispFileMap=null,dispRunInFlight=false;let dispSettlePromise=null,dispSettleResolve=null;let applyDispDebounceToken=0;// Reassigned once, below, to the real handle object literal; declared
...
let handle=null;// Scene-mode state, null/empty when sceneMode is null (sphere/cube
...
let sceneGroup=null,sceneOwnedMaterials=[],pmremRT=null;// Depth-peel shell state (see the FORCE_TRANSPARENCY flag's header
...
const envRotationMatrix3=rad=>new THREE.Matrix3().setFromMatrix4(new THREE.Matrix4().makeRotationY(rad));// Attaches the live-rotatable env patch to one neutral glTF PBR
...
const patchNeutralMaterialEnvRotation=material=>{material.onBeforeCompile=shader=>{shader.uniforms.uEnvRotation={value:envRotationMatrix3(envRotationRad)};shader.fragmentShader=shader.fragmentShader.replace('#include <envmap_physical_pars_fragment>',NEUTRAL_ENV_ROTATION_CHUNK);material.userData.envRotationUniform=shader.uniforms.uEnvRotation;};// r128's Material default already derives customProgramCacheKey
// from onBeforeCompile.toString(), which already keys these apart;
// set explicitly anyway as insurance against a future edit.
material.customProgramCacheKey=()=>'neutralEnvRotation';};// Shell-owned skybox mesh, replacing scene.background: r128's
...
let bgMesh=null;// Studio backdrop group (wall/floor lathe + shadow catcher + zero-
// intensity spotlight), null for flat2d/full-scene, where the
// studio mode is never built (see its construction further down).
let studioGroup=null,studioMesh=null,studioCatcher=null,studioLight=null;// Shell-level env (IBL) state, fetched ONCE (not per material
...
let envKeyLight=null;// Cheaper fallback direction for the studio shadow ONLY (no direct
...
try{const dispGeoms=new Set([originalGeometry,baseGeometry,displacedGeometry].filter(Boolean));dispGeoms.delete(geometry);dispGeoms.forEach(g=>{try{g.dispose();}catch(e2){/* already disposed/invalid */}});}catch(e){/* best-effort */}// bgMesh: dispose its own geometry/material and drop it from
// the scene. Do NOT dispose bgMesh.material.map (envBgTexture):
// env textures are shared/cached across every live view.
try{if(bgMesh){scene.remove(bgMesh);bgMesh.geometry.dispose();bgMesh.material.dispose();}}catch(e){/* already disposed/invalid, or scene never got this far */}// studioGroup: drop it, dispose its two per-view MATERIALS and
// the spotlight's own shadow render target. Do NOT dispose the
// two lathe geometries (reused everywhere).
try{if(studioGroup){scene.remove(studioGroup);if(studioMesh)studioMesh.material.dispose();if(studioCatcher)studioCatcher.material.dispose();if(studioLight)studioLight.shadow.dispose();}}catch(e){/* already disposed/invalid, or scene never got this far */}// sceneGroup (scene-mode only): drops the GLB hierarchy and
// disposes its per-view material CLONES (sceneOwnedMaterials).
// Does NOT dispose geometries, shared with other cached views.
try{if(sceneGroup){scene.remove(sceneGroup);sceneOwnedMaterials.forEach(m=>{try{m.dispose();}catch(e){/* already disposed/invalid */}});}}catch(e){/* already disposed/invalid, or scene never got this far */}// pmremRT: this view's OWN render target, safe to dispose.
...
const __srcs=await generatePreviewSourcesWithinBudget({mx,gen,genContext,renderable,label,materialName,isMounted,stageLightCount:PREVIEW_STAGE_LIGHT_COUNT,sceneFeatureOptions:PREVIEW_FEATURE_OPTIONS,allowConstInputs});// Bail if this build was superseded while awaiting above:
// nothing GL-side exists yet, so disposePartial() is a
// safe, idempotent no-op beyond flagging `stopped`.
if(!__srcs){disposePartial();return null;}// introspected: already plain JS, converted inside the
// mxExclusive-locked generatePreviewSourcesUnlocked
// before the lock released. No wasm reads left here.
const{vs,fs,introspected,transparent,geomprops,notices,maxLights}=__srcs;// Pre-warms the driver compile BEFORE the display renderer
// is created; the old after-renderer placement measured
// 0.8-2.5s WebGLRenderer init stalls from queue contention.
prewarmDisplacementSources(__srcs,isMounted,label);const warmResult=await prewarmShaderCompile({vs,fs,isMounted,label});if(warmResult==='bailed'||!isMounted()){disposePartial();return null;}// --- three.js scene (WebGL2) ---
...
peelPipeline=createPeelPipeline(renderer,{getDisplayTransform});const scene=new THREE.Scene();// Instantiates the scene-mode GLB (if any) BEFORE the
...
console.warn('shaderball scene unavailable, falling back to sphere:',geomName);}if(sceneInst){sceneGroup=sceneInst.group;sceneOwnedMaterials=sceneInst.ownedMaterials;// Env-rotation patch: every neutral glTF PBR material
// EXCEPT the backplanes' MeshBasicMaterial clones
// (no envMap). Same duck-typing check as setEnvExposure.
sceneOwnedMaterials.forEach(m=>{// BISECT: the env-rotation chunk is the only hand-injected
// shader in the scene, and it is the last suspect for the
// black neutral materials under an enabled shadow map.
if('envMapIntensity'in m&&!wantsStudio)patchNeutralMaterialEnvRotation(m);});}// fullScene: the full authored preset (shaderball.glb),
// fixed camera, no fallback spin by default; docs/viewer
// opt into orbit/zoom via sceneOrbit. 'simple' is NOT fullScene.
const fullScene=!!(sceneInst&&sceneMode==='full');// Populated only in the fullScene-adoption branch below;
// read again by syncSize on every resize. null in every
// other mode (fixed-45-degree camera untouched).
let fullSceneAuthoredFov=null;let fullSceneAuthoredAspect=null;// Authored GLB camera pose cached at adoption time,
// the scene-orbit config block restores it (OrbitControls
// re-aims at (0,0,0)), and resetCamera() returns to it.
let sceneAuthoredPose=null;// flat2d: ortho frustum whose x extent tracks the canvas
...
gc.getWorldPosition(camera.position);gc.getWorldQuaternion(camera.quaternion);sceneAuthoredPose={position:camera.position.clone(),quaternion:camera.quaternion.clone()};// gc.near/far are already in THREE.PerspectiveCamera's
// units, copy verbatim. gc.fov is captured below
// rather than copied straight, see effectiveFullSceneVFov.
camera.near=gc.near;camera.far=gc.far;// Authored aspect: gc.aspect (this GLB authors
// ~1.7778/16:9); the `|| 1.7778` fallback only matters
// for a hypothetical GLB that omits aspectRatio.
fullSceneAuthoredFov=gc.fov;fullSceneAuthoredAspect=gc.aspect||1.7778;// Aspect from the CANVAS, not the GLB's own, no
// letterbox/pillarbox, same as every other preset.
// effectiveFullSceneVFov widens the fov instead of cropping.
camera.aspect=cw/ch;camera.fov=effectiveFullSceneVFov(fullSceneAuthoredFov,fullSceneAuthoredAspect,camera.aspect);camera.updateProjectionMatrix();}// wheelMode 'scroll': register the gate BEFORE OrbitControls
...
controls=null;if(THREE.OrbitControls&&!flat2d&&(!fullScene||sceneOrbit)){controls=new THREE.OrbitControls(camera,canvas);controls.enableDamping=true;controls.dampingFactor=0.08;controls.enablePan=false;controls.enableZoom=wheelMode!=='none';controls.minDistance=1.4;controls.maxDistance=STUDIO_MAX_ORBIT_DISTANCE;// Camera auto-orbit (off by default): pins the
...
let fullscreenFit=false;// World-space bounding sphere of the whole ball assembly,
// computed ONCE per scene and cached here, see
// getBallBoundingSphere just below.
let ballBoundingSphere=null;// Scene-orbit hard-containment box (sceneGroup bounds
// inset 2%/axis, expanded so the default pose stays
// legal); null in every other mode.
let sceneOrbitClampBox=null;// Reference camera->ball distance for scene-orbit framing,
// captured from the AUTHORED pose so recomputeCameraFov's
// fit-to-ball fov stays constant while the user zooms.
let sceneOrbitFitDist=null;// Radius of the framing target (the ball proper, see the
// config block below), paired with sceneOrbitFitDist for the
// scene-orbit fov fit. null in every other mode.
let sceneOrbitFitRadius=null;// Finds (and caches) the ball assembly's world bounding
...
const withFramingGeometry=fn=>{if(!mesh||!originalGeometry||mesh.geometry===originalGeometry)return fn();const current=mesh.geometry;mesh.geometry=originalGeometry;try{return fn();}finally{mesh.geometry=current;}};const getBallBoundingSphere=()=>{if(ballBoundingSphere)return ballBoundingSphere;if(!sceneGroup)return null;const ballNode=sceneGroup.getObjectByName('shader_ball')||mesh&&mesh.parent||sceneGroup;ballNode.updateMatrixWorld(true);const box=withFramingGeometry(()=>new THREE.Box3().setFromObject(ballNode));ballBoundingSphere=box.getBoundingSphere(new THREE.Sphere());return ballBoundingSphere;};// Single entry point for every fov-affecting event so they
...
let fov=effectiveFullSceneVFov(fullSceneAuthoredFov,fullSceneAuthoredAspect,camera.aspect);// Scene-orbit default framing: fits the ball PROPER
// to the actual viewport aspect (authored ~16:9 fov
// overflows wider canvases). REPLACES the base fov here.
if(sceneOrbitFitDist!=null&&sceneOrbitFitRadius!=null&&sceneOrbitFitDist>sceneOrbitFitRadius){const theta=Math.asin(Math.min(1,sceneOrbitFitRadius/sceneOrbitFitDist));const vForV=2*theta;const vForH=2*Math.atan(Math.tan(theta)/camera.aspect);const SCENE_FIT_MARGIN=1.15;// ball ~1/1.15 of the limiting dimension - tight hero framing
fov=Math.max(vForV,vForH)*180/Math.PI*SCENE_FIT_MARGIN;}if(fullscreenFit){const sphere=getBallBoundingSphere();const dist=sphere?camera.position.distanceTo(sphere.center):0;if(sphere&&dist>sphere.radius){// Angular radius of the ball as seen from the
// camera: asin(r/d), clamped to 1 against fp
// overshoot when dist is barely larger than radius.
const theta=Math.asin(Math.min(1,sphere.radius/dist));// The ball must fit BOTH axes: vertical
// half-fov covers theta directly; horizontal
// half-fov converts back via the same tan/atan.
const vFovForVertical=2*theta;const vFovForHorizontal=2*Math.atan(Math.tan(theta)/camera.aspect);const FIT_MARGIN=1.06;// ~6% breathing room so the ball doesn't touch the frame edge
const fitFovDeg=Math.max(vFovForVertical,vFovForHorizontal)*180/Math.PI*FIT_MARGIN;fov=Math.max(fov,fitFovDeg);// only ever widen -- never crop back below the everyday framing
}}camera.fov=fov;};// flat2d screen-proportional fit (Shadertoy's
...
envBgTexture=makeBackgroundTexture(envRadiance);envHasFile=false;}// Shell-owned skybox mesh (see bgMesh's declaration
...
if(!flat2d){const bgGeometry=new THREE.SphereGeometry(50,64,32);bgGeometry.scale(-1,1,1);bgMesh=new THREE.Mesh(bgGeometry,new THREE.MeshBasicMaterial({map:envBgTexture,depthWrite:false}));bgMesh.renderOrder=-1000;bgMesh.rotation.y=BG_BASE+BG_SIGN*envRotationRad;bgMesh.visible=false;// real visibility set by applyBackdrop() below
scene.add(bgMesh);}}if(sceneInst){// Scene-mode lighting: bakes radianceSrc into a
...
pmremRT=new THREE.PMREMGenerator(renderer).fromEquirectangular(radianceSrc);scene.environment=pmremRT.texture;}}// Last resort (see placeStudioLight): the studio's original
// hardcoded angle, still rotated by envRotationRad. Used
// only when neither envKeyLight nor envSoftKeyDir is available.
const STUDIO_LIGHT_FALLBACK_DIR=new THREE.Vector3(2.5,6,4).normalize();// Single source of truth for the spotlight's placement
// (called here and by setEnvRotation), so the shadow tracks
// envKeyLight, or failing that envSoftKeyDir, like u_lightData does.
const placeStudioLight=()=>{if(!studioLight)return;const toLightDir=(envKeyLight?envKeyLight.direction.clone().negate():envSoftKeyDir?envSoftKeyDir.clone().negate():STUDIO_LIGHT_FALLBACK_DIR.clone()).applyMatrix4(keyLightRotationMatrix(envRotationRad)).normalize();// A near-horizon key light drags the contact shadow far
// past the catcher footprint, so floor the elevation,
// rescaling (x, z) to keep the vector normalized and the azimuth intact.
const minY=Math.sin(STUDIO_LIGHT_MIN_ELEV_RAD);if(toLightDir.y<minY){const horizLen=Math.hypot(toLightDir.x,toLightDir.z);if(horizLen>1e-6){const scale=Math.sqrt(Math.max(0,1-minY*minY))/horizLen;toLightDir.x*=scale;toLightDir.z*=scale;toLightDir.y=minY;}}studioLight.position.copy(toLightDir).multiplyScalar(STUDIO_LIGHT_DISTANCE);};// Procedural studio cyclorama + contact shadow, the third
...
if(wantsStudio){try{const studioGeom=getStudioGeometry();if(studioGeom){studioGroup=new THREE.Group();studioMesh=new THREE.Mesh(getStudioBackdropGeometry()||studioGeom,new THREE.ShaderMaterial({uniforms:{uStop0:{value:new THREE.Vector3()},uStop1:{value:new THREE.Vector3()},uStop2:{value:new THREE.Vector3()},uStop3:{value:new THREE.Vector3()},uHotspotColor:{value:new THREE.Vector3()},uHotspotA:{value:0},uLinearOut:{value:0}},vertexShader:STUDIO_GRADIENT_VERTEX_SHADER,fragmentShader:STUDIO_GRADIENT_FRAGMENT_SHADER(getDisplayTransform()),side:THREE.BackSide,fog:false}));applyStudioVariantUniforms(studioMesh.material,backdropMode==='studio-dark');studioMesh.renderOrder=-900;// BackSide like studioMesh: a FrontSide catcher
// would be culled from inside and show no shadow.
// It keeps the true bowl, so the shadow meets the model where it lands.
studioCatcher=new THREE.Mesh(studioGeom,new THREE.ShadowMaterial({opacity:backdropMode==='studio-dark'?STUDIO_SHADOW_OPACITY_DARK:STUDIO_SHADOW_OPACITY,side:THREE.BackSide}));studioCatcher.receiveShadow=true;studioCatcher.material.depthWrite=false;studioCatcher.renderOrder=-800;// Zero intensity + castShadow: only the simple
// GLB's neutral glTF meshes read lights, so this
// casts a shadow while lighting nothing.
studioLight=new THREE.SpotLight(0xffffff,0);studioLight.target.position.set(0,0,0);studioLight.castShadow=true;studioLight.angle=Math.atan(STUDIO_LIGHT_CONE_R/STUDIO_LIGHT_DISTANCE);studioLight.penumbra=0.5;// Near brackets tightly around the fixed light-
...
studioLight.shadow.camera.near=STUDIO_LIGHT_DISTANCE-4;studioLight.shadow.camera.far=STUDIO_LIGHT_DISTANCE+STUDIO_WALL_R+2;studioLight.shadow.mapSize.set(STUDIO_SHADOW_MAP_SIZE,STUDIO_SHADOW_MAP_SIZE);// VSM honors shadow.radius for a real blur pass;
// PCFSoft ignores it and stair-steps instead.
studioLight.shadow.radius=12;studioLight.shadow.bias=-0.0005;studioLight.shadow.normalBias=0.02;placeStudioLight();studioGroup.add(studioMesh,studioCatcher,studioLight,studioLight.target);scene.add(studioGroup);}}catch(e){// Build failure (e.g. no THREE.LatheGeometry) must
// never take down the whole view, degrade to no
// studio backdrop instead; bgMesh/'none' still work.
studioGroup=null;studioMesh=null;studioCatcher=null;studioLight=null;}}// 'studio' and 'studio-dark' share the same bowl/light/
// catcher, so every mode check below tests this instead of
// a literal 'studio' equality.
const isStudioBackdrop=m=>m==='studio'||m==='studio-dark';// Single source of truth for the four backdrop modes,
...
let studioPolarApplied=false;const applyStudioPolarClamp=()=>{if(!controls)return;if(!studioGroup||!isStudioBackdrop(backdropMode)){// Only ever restore a clamp we set: full-scene mode
// has no studioGroup and owns its own orbit limits.
if(studioPolarApplied){controls.maxPolarAngle=Math.PI;studioPolarApplied=false;}return;}const dist=camera.position.distanceTo(controls.target);const rel=studioGroup.position.y+STUDIO_FLOOR_CLEARANCE-controls.target.y;const limit=dist>1e-3?Math.acos(Math.max(-1,Math.min(1,rel/dist))):STUDIO_MAX_POLAR;controls.maxPolarAngle=Math.min(STUDIO_MAX_POLAR,limit);studioPolarApplied=true;};// Single source of truth for the four backdrop modes,
// applied once below for the initial `backdrop` option,
// and again by the handle's setBackdrop()/setEnvBackground().
...
const sceneBuiltinMaterials=(bgMesh?[bgMesh.material]:[]).concat(studioMesh?[studioMesh.material]:[],studioCatcher?[studioCatcher.material]:[]).concat(sceneOwnedMaterials);const setSceneLinear=on=>{sceneBuiltinMaterials.forEach(m=>{if(m.toneMapped===!on)return;m.toneMapped=!on;m.needsUpdate=true;});// Raw ShaderMaterial ignores toneMapped and RT encoding,
// so the linear pass needs an explicit flag.
if(studioMesh&&studioMesh.material&&studioMesh.material.uniforms&&studioMesh.material.uniforms.uLinearOut){studioMesh.material.uniforms.uLinearOut.value=on?1:0;}};// Selected preview geometry. Scene mode pre-assigns the
...
sceneGroup.updateMatrixWorld(true);}else{geometry=prepGeometry(await buildPreviewGeometry(geomName));// Initial screen-proportional fit for the 2D buffer,
// don't rely on the ResizeObserver's first fire
// ordering against the first rendered frame.
if(flat2d)fitQuadToAspect((canvas.clientWidth||cw)/(canvas.clientHeight||ch));}if(!isMounted()){disposePartial();return null;}// Displacement (P5): kept alive (never disposed) until
// teardown; every geometry below derives from it, never
// from a previous displaced result.
originalGeometry=geometry;// Silhouette-bottom floor placement, factored out so a
// later geometry swap can re-run it too.
const updateStudioFloor=()=>{if(!studioGroup)return;let floorY=-1;try{const box=new THREE.Box3().setFromObject(sceneGroup||mesh);if(isFinite(box.min.y))floorY=box.min.y;}catch(e){/* degenerate/empty box - keep the -1 fallback */}studioGroup.position.y=floorY;};// Current, complete set of displacement-derived notices
// (subdivision cap/drop, plus the latest evaluation run's
// own notices), for getDisplacementState() and the status event.
const currentDispNotices=()=>[dispCappedNotice,dispDroppedNotice].filter(Boolean).concat(dispEvalNotices);// Rebuilt, never appended, so re-runs cannot stack stale copies.
const syncHandleNotices=()=>{if(handle)handle.notices=materialNotices.concat(currentDispNotices());};const dispDispatchStatus=()=>{if(!handle)return;try{window.dispatchEvent(new CustomEvent('mtlx-displacement-status',{detail:{view:handle,state:dispState,notices:currentDispNotices()}}));}catch(e){/* best-effort */}};// Sets `mesh.geometry`/`geometry` to `g`; a genuine
// displaced result is disposed on the NEXT swap, the base/
// original is left alone. Safe pre-`mesh` too (first build).
const swapMeshGeometry=g=>{const prevDisplaced=displacedGeometry;displacedGeometry=g!==originalGeometry&&g!==baseGeometry?g:null;geometry=g;if(mesh){mesh.geometry=g;updateStudioFloor();}if(prevDisplaced&&prevDisplaced!==g){try{prevDisplaced.dispose();}catch(e){/* already disposed/invalid */}}};// Non-indexed corners, Loop-subdivided (creases at authored
// normal breaks) then welded back into an indexed geometry;
// other attributes are dropped (reported by the caller).
const subdivideSourceGeometry=(source,level)=>{const nonIndexed=source.index?source.toNonIndexed():source;const posAttr=nonIndexed.getAttribute('position');const normAttr=nonIndexed.getAttribute('normal');const uvAttr=nonIndexed.getAttribute('uv');const toArr=attr=>attr?attr.array instanceof Float32Array?attr.array:Float32Array.from(attr.array):null;const geomprops=Object.keys(nonIndexed.attributes).filter(name=>name.startsWith('i_geomprop_')).map(name=>({name:name.slice('i_geomprop_'.length),itemSize:nonIndexed.getAttribute(name).itemSize,data:toArr(nonIndexed.getAttribute(name))}));const meshIn={positions:toArr(posAttr),normals:toArr(normAttr),uvs:toArr(uvAttr),geomprops};if(nonIndexed!==source)nonIndexed.dispose();if(!meshIn.positions)return null;const subdivided=MtlxMeshSubdivision.subdivideMesh(meshIn,level,{creaseByNormals:true});if(!subdivided)return null;const welded=MtlxMeshSubdivision.weldMesh(subdivided);const out=new THREE.BufferGeometry();out.setAttribute('position',new THREE.BufferAttribute(welded.positions,3));out.setAttribute('normal',new THREE.BufferAttribute(welded.normals,3));if(welded.uvs)out.setAttribute('uv',new THREE.BufferAttribute(welded.uvs,2));for(const stream of welded.geomprops||[]){out.setAttribute('i_geomprop_'+stream.name,new THREE.BufferAttribute(stream.data,stream.itemSize));}out.setIndex(new THREE.BufferAttribute(welded.indices,1));prepGeometry(out);// prepGeometry, aliasUvGeomprops and computeTangents rebuild these on
// the subdivided mesh, so only genuinely lost attributes are reported.
const rebuilt=new Set(['position','normal','uv','i_position','i_normal','i_texcoord_0','tangent','i_tangent','i_bitangent',...UV_GEOMPROP_ALIASES,...(welded.geomprops||[]).map(stream=>'i_geomprop_'+stream.name)]);const dropped=Object.keys(source.attributes).filter(n=>!rebuilt.has(n));return{geometry:out,dropped};};// Ensures baseGeometry reflects the current preview
// subdivision setting (capped to the triangle budget),
// rebuilding only when the resolved level changed.
const ensureBaseGeometry=()=>{const posAttr=originalGeometry.getAttribute('position');const idx=originalGeometry.getIndex();const baseTriangleCount=Math.max(1,Math.round((idx?idx.count:posAttr?posAttr.count:3)/3));const overrideBudget=window.__mtlxTriangleBudgetOverride;const budget=Number.isFinite(overrideBudget)?overrideBudget:triangleBudget;const{level,capped,triangles,allowed}=pickSubdivisionLevel(baseTriangleCount,getPreviewSubdivisionLevel(),budget);if(dispSubdivLevel===level&&baseGeometry)return{level,capped,triangles,allowed};let built=originalGeometry;let dropped=[];if(level>0){const cacheKey=baseGeomCacheKey(geomName,sceneMode,level);const cached=baseGeomCacheGet(cacheKey);if(cached){built=cached.clone();}else{const result=subdivideSourceGeometry(originalGeometry,level);if(result){dropped=result.dropped;baseGeomCacheSet(cacheKey,result.geometry);built=result.geometry.clone();}}}if(baseGeometry&&baseGeometry!==originalGeometry){try{baseGeometry.dispose();}catch(e){/* already disposed/invalid */}}baseGeometry=built;dispSubdivLevel=level;dispTriangles=triangles;dispWithinBudget=allowed;dispCappedNotice=!allowed?'Displacement skipped: base mesh has '+triangles+' triangles, above the '+budget+' triangle budget':capped?'Subdivision capped at level '+level+' ('+triangles+' triangles) to stay under the budget':null;dispDroppedNotice=dropped.length?'Subdivision dropped extra vertex attributes: '+dropped.join(', '):null;syncHandleNotices();return{level,capped,triangles,allowed};};const bindDisplacementGeomprops=()=>{if(!baseGeometry||!displacementSources||!displacementSources.geomprops)return;bindGeompropAttributes(baseGeometry,displacementSources.geomprops,text=>{if(!dispEvalNotices.includes(text))dispEvalNotices.push(text);});};// Computes displaced positions/normals from `base` and
// clones them into a fresh geometry; null with no position data.
const buildDisplacedGeometry=(base,result)=>{const posAttr=base.getAttribute('position');if(!posAttr)return null;const normAttr=base.getAttribute('normal');const tanAttr=base.getAttribute('i_tangent');const bitanAttr=base.getAttribute('i_bitangent');const idxAttr=base.getIndex();const computed=MtlxMeshDisplacement.computeDisplacedAttributes({positions:posAttr.array,normals:normAttr?normAttr.array:null,tangents:tanAttr?tanAttr.array:null,bitangents:bitanAttr?bitanAttr.array:null,indices:idxAttr?idxAttr.array:null,offsets:result.offsets,mode:result.mode,offsetsTangent:result.offsetsTangent||null,offsetsBitangent:result.offsetsBitangent||null,analyticFrame:result.analyticFrame||null,displacementNormals:getDisplacementNormalsMode()});const out=base.clone();out.setAttribute('position',new THREE.BufferAttribute(computed.positions,3));out.setAttribute('normal',new THREE.BufferAttribute(computed.normals,3));out.deleteAttribute('i_position');out.deleteAttribute('i_normal');out.deleteAttribute('i_tangent');out.deleteAttribute('i_bitangent');out.deleteAttribute('i_texcoord_0');prepGeometry(out);out.computeBoundingBox();out.computeBoundingSphere();return out;};// Lands one evaluateDisplacement() result: a superseded
// token stops without swapping; a null/failed result falls
// all the way back to originalGeometry, never a partial one.
const landDisplacementResult=(token,result,failState)=>{if(token!==dispToken||stopped)return;dispRunInFlight=false;if(dispSettleResolve){dispSettleResolve();dispSettleResolve=null;dispSettlePromise=null;}dispEvalNotices=result&&result.notices||[];syncHandleNotices();if(!result||!result.offsets){dispState=failState||'failed';swapMeshGeometry(originalGeometry);dispDispatchStatus();return;}const built=buildDisplacedGeometry(baseGeometry,result);if(!built){dispState='failed';swapMeshGeometry(originalGeometry);dispDispatchStatus();return;}swapMeshGeometry(built);dispState='applied';dispDispatchStatus();};const cancelDisplacementRun=()=>{dispToken++;dispRunInFlight=false;if(dispSettleResolve){dispSettleResolve();dispSettleResolve=null;dispSettlePromise=null;}};// Evaluates the current displacementSources/baseGeometry
// and lands the result; shared by settings toggles, a
// material debounce and an arriving file map.
const runDisplacement=async()=>{if(stopped||flat2d||!displacementSources||!baseGeometry)return;bindDisplacementGeomprops();const token=++dispToken;if(!dispWithinBudget){landDisplacementResult(token,{offsets:null,notices:[dispCappedNotice]},'skipped');return;}dispState='pending';dispRunInFlight=true;if(!dispSettlePromise)dispSettlePromise=new Promise(res=>{dispSettleResolve=res;});dispDispatchStatus();const posAttr=baseGeometry.getAttribute('position');if(!posAttr||posAttr.count<3){landDisplacementResult(token,{offsets:null,notices:['Displacement skipped: geometry has too few vertices']},'skipped');return;}let result=null;try{result=await evaluateDisplacement({renderer,displacement:displacementSources,geometry:baseGeometry,worldMatrix:mesh?mesh.matrixWorld:new THREE.Matrix4(),fileMap:dispFileMap,textureCache:undefined,isAlive:()=>!stopped&&token===dispToken});}catch(e){result={offsets:null,notices:['Displacement evaluation failed: '+(e&&e.message?e.message:String(e))]};}landDisplacementResult(token,result,'failed');};// First build: subdivide + evaluate before the first apply
// so the first frame shows the final geometry; a filename-
// driven or slow (>4s) program lands later instead.
displacementSources=__srcs.displacement;dispKey=displacementSources?displacementSources.key:null;if(!flat2d&&displacementSources&&getDisplacementEnabled()){ensureBaseGeometry();bindDisplacementGeomprops();geometry=baseGeometry;if(mesh)mesh.geometry=baseGeometry;// See hasDisplacementFileRef's header comment above.
if(!dispWithinBudget){const token=++dispToken;landDisplacementResult(token,{offsets:null,notices:[dispCappedNotice]},'skipped');}else if(!hasDisplacementFileRef(displacementSources)){const token=++dispToken;dispState='pending';dispRunInFlight=true;if(!dispSettlePromise)dispSettlePromise=new Promise(res=>{dispSettleResolve=res;});const evalPromise=evaluateDisplacement({renderer,displacement:displacementSources,geometry:baseGeometry,worldMatrix:mesh?mesh.matrixWorld:new THREE.Matrix4(),fileMap:dispFileMap,textureCache:undefined,isAlive:()=>!stopped&&token===dispToken}).catch(e=>({offsets:null,notices:['Displacement evaluation failed: '+(e&&e.message?e.message:String(e))]}));const FIRST_BUILD_DISPLACEMENT_TIMEOUT_MS=4000;const timedOut=Symbol('mtlx-disp-timeout');const raced=await Promise.race([evalPromise,new Promise(resolve=>setTimeout(()=>resolve(timedOut),FIRST_BUILD_DISPLACEMENT_TIMEOUT_MS))]);if(raced===timedOut){// Keep waiting in the background; the rest of
// this function is synchronous, so `mesh`/`handle`
// both exist well before this resolves.
evalPromise.then(result=>landDisplacementResult(token,result,'failed'));}else{landDisplacementResult(token,raced,'failed');}}// else: filename-driven, wait for onDisplacementFileMap.
}if(fullScene&&sceneOrbit&&controls){// OrbitControls' constructor already ran update()
// against its placeholder (0,0,0) target and re-aimed
// the camera, restore the authored pose first.
if(sceneAuthoredPose){camera.position.copy(sceneAuthoredPose.position);camera.quaternion.copy(sceneAuthoredPose.quaternion);}const sphere=getBallBoundingSphere();// Pivot on the authored view ray at the ball's depth:
// orientation is unchanged by OrbitControls' first
// lookAt (zero roll), and the orbit pivots at the ball.
const fwd=new THREE.Vector3(0,0,-1).applyQuaternion(camera.quaternion);let d=sphere?sphere.center.clone().sub(camera.position).dot(fwd):0;if(!(d>0))d=sphere?camera.position.distanceTo(sphere.center):0.5;controls.target.copy(camera.position).addScaledVector(fwd,d);controls.minDistance=Math.min(d,sphere?sphere.radius*1.5:d*0.5);// Auto-rotate stays OFF regardless of autoRotate: the
// rotate button is hidden here and setAutoRotate no-ops
// for fullScene, a stale `rotating` could start a turntable.
controls.autoRotate=false;// Containment: sceneGroup bounds == the backdrop box.
// Inset 2% per axis, then union the authored camera
// position so the default pose is always legal.
const box=new THREE.Box3().setFromObject(sceneGroup);const size=box.getSize(new THREE.Vector3());box.min.x+=size.x*0.02;box.max.x-=size.x*0.02;box.min.y+=size.y*0.02;box.max.y-=size.y*0.02;box.min.z+=size.z*0.02;box.max.z-=size.z*0.02;box.expandByPoint(camera.position);sceneOrbitClampBox=box;// Zoom-out limit: the ray-box EXIT distance from the
// pivot through the camera, always >= the authored
// distance so the initial framing stays reachable.
const back=camera.position.clone().sub(controls.target).normalize();const exit=box.containsPoint(controls.target)?new THREE.Ray(controls.target.clone(),back).intersectBox(box,new THREE.Vector3()):null;controls.maxDistance=exit?controls.target.distanceTo(exit):d*4;// Captures setup distance + ball radius for the fit-to-
// ball fov. Radius = HALF the largest AABB extent, not
// Box3.getBoundingSphere() (which framed ~1.7x too far).
let fitCenter=null,fitRadius=null;if(mesh){mesh.updateMatrixWorld(true);const bb=withFramingGeometry(()=>new THREE.Box3().setFromObject(mesh));fitCenter=bb.getCenter(new THREE.Vector3());const bs=bb.getSize(new THREE.Vector3());fitRadius=Math.max(bs.x,bs.y,bs.z)/2;}else if(sphere){fitCenter=sphere.center;fitRadius=sphere.radius;}sceneOrbitFitRadius=fitRadius;sceneOrbitFitDist=fitCenter?camera.position.distanceTo(fitCenter):null;recomputeCameraFov();camera.updateProjectionMatrix();// Snapshot for resetCamera(): position0/target0 now
...
// ------------------------------------------------------
const bindMaterialUniforms=srcs=>{const{vs,fs,introspected}=srcs;// Shadow sampling and the occlusion block are gated out of
// preview sources (nothing here can bind either), so their
// no-op seeds are gated the same way.
const featureSkips=srcs.featureSkips||{};// MaterialX-generated shaders expect their own attribute
// names (i_position, i_normal, ...) and u_* transform
// uniforms, so we use RawShaderMaterial and feed both manually.
const newUniforms={u_worldMatrix:{value:new THREE.Matrix4()},u_viewProjectionMatrix:{value:new THREE.Matrix4()},u_worldInverseTransposeMatrix:{value:new THREE.Matrix4()},u_viewPosition:{value:new THREE.Vector3()},// Depth-peel uniforms (see injectPeelDiscard's header
...
u_peelMode:{value:0},u_peelHasPrev:{value:0},u_peelPrevDepth:{value:getDummyTex()},u_opaqueDepth:{value:getDummyTexWhite()},// Harmless when shadow sampling was gated out (no such
// uniform then); with it generated the Viewer must bind
// white moments (fully lit) or its materials render black.
u_shadowMap:{value:getDummyTexWhite()},u_shadowMatrix:{value:shadowOffMatrix()},// encodeDisplay defers to finalMat only while the
// peel pipeline draws a linear intermediate target.
// Ordinary opaque frames must remain display encoded.
u_peelLinear:{value:0},// Injected by encodeDisplay, so MaterialX never
// introspects it and the defaults pass below cannot
// supply it.
u_displayExposure:{value:displayExposureScale()},u_displayTransform:{value:displayTransformId(getDisplayTransform())}};if(!featureSkips.shadowMap)Object.assign(newUniforms,{// No stage caster in the Viewer, so no slot is shadowed;
...
u_shadowAtlas:{value:getDummyTexWhite()},u_shadowMatrices:{value:Array.from({length:SHADOW_FACE_SLOTS},()=>new THREE.Matrix4())},u_shadowTiles:{value:Array.from({length:SHADOW_FACE_SLOTS},()=>new THREE.Vector4(0,0,1,1))},u_shadowDepthPlanes:{value:Array.from({length:SHADOW_FACE_SLOTS},()=>new THREE.Vector4(0,0,0,1))},u_shadowDepthRanges:{value:Array.from({length:SHADOW_FACE_SLOTS},()=>new THREE.Vector2(0,1))},u_shadowSourceRadii:{value:Array.from({length:SHADOW_FACE_SLOTS},()=>new THREE.Vector4())},u_shadowTexelWorldSize:{value:new Array(SHADOW_FACE_SLOTS).fill(0)},u_shadowFaceOrigin:{value:Array.from({length:SHADOW_FACE_SLOTS},()=>new THREE.Vector3())},u_shadowFaceValid:{value:new Array(SHADOW_FACE_SLOTS).fill(0)},u_shadowFaceBasisX:{value:Array.from({length:SHADOW_FACE_SLOTS},()=>new THREE.Vector3(1,0,0))},u_shadowFaceBasisY:{value:Array.from({length:SHADOW_FACE_SLOTS},()=>new THREE.Vector3(0,1,0))},u_shadowFaceBasisZ:{value:Array.from({length:SHADOW_FACE_SLOTS},()=>new THREE.Vector3(0,0,1))},u_shadowSlotFace:{value:new Int32Array(SHADOW_LIGHT_SLOTS_MAX).fill(-1)},u_shadowSlotFaceCount:{value:new Int32Array(SHADOW_LIGHT_SLOTS_MAX).fill(0)},u_shadowDiagnosticVisibilityScale:{value:1}});if(!featureSkips.occlusion)Object.assign(newUniforms,{// Same sampler-unit hazard as the Scene: see
// createMtlxSceneUniforms. No stage volume here, so
// the white 1x1x1 dummy at strength 0 is the value.
u_skyVisMap:{value:getDummyTex3DWhite()},u_skyVisMin:{value:new THREE.Vector3()},u_skyVisSize:{value:new THREE.Vector3(1,1,1)},u_skyVisCell:{value:0},u_skyVisStrength:{value:0},// No baked occlusion volume in the Viewer either;
// same sampler-unit hazard as u_skyVisMap above.
u_aoVolumeMap:{value:getDummyTex3DWhite()},u_aoVolumeMin:{value:new THREE.Vector3()},u_aoVolumeSize:{value:new THREE.Vector3(1,1,1)},u_aoVolumeCell:{value:0},u_aoVolumeStrength:{value:0}});// Baked diffuse bounce is patched in independently of the
// occlusion gate, so it is seeded whatever that gate did.
Object.assign(newUniforms,{// No baked diffuse bounce in the Viewer either; same
// sampler-unit hazard as u_skyVisMap above.
u_skyBounceMap:{value:getDummyTex3DWhite()},u_skyBounceMin:{value:new THREE.Vector3()},u_skyBounceSize:{value:new THREE.Vector3(1,1,1)},u_skyBounceCell:{value:0},u_skyBounceStrength:{value:0},u_bounceScale:{value:0},u_bounceTint:{value:new THREE.Vector3(1,1,1)}});// GLSL ES 3.0 forbids uniform initializers, so the app
// must upload each default, an unset uniform reads as
// 0 in WebGL, which blacked out every unlit/PBR preview.
applyIntrospectedUniformDefaults(newUniforms,introspected);if(DEBUG_SHADERS){console.log('introspected uniforms:',introspected.map(u=>`${u.type} ${u.name}${u.data!=null?' (default uploaded)':''}`));if(!introspected.length){console.warn('Shader introspection found NO uniform blocks, defaults not uploaded; expect black. (Binding API mismatch, report the mxShader/stage method names used by generatePreviewSourcesUnlocked.)');}}// Discover what the generated shader actually declares,
// so we bind by real names rather than assumptions.
const declared=parseUniforms(fs).concat(parseUniforms(vs));const declaredNames=new Set(declared.map(u=>u.name));const has=n=>declaredNames.has(n);// MaterialX gives u_time/u_frame no default value, so the
// introspected-defaults pass above never binds them.
if(has('u_time'))newUniforms.u_time={value:MTLX_CLOCK.time};if(has('u_frame'))newUniforms.u_frame={value:MTLX_CLOCK.frame};// Finds a declared sampler by pattern, ALWAYS anchored
// to /env/i first, without it, a material sampler
// named e.g. "specular" could false-match (a real past bug).
const findSampler=(re,exclude)=>declared.find(u=>/sampler/i.test(u.type)&&/env/i.test(u.name)&&re.test(u.name)&&!(exclude&&exclude.test(u.name)));if(DEBUG_SHADERS){console.group(`MaterialX preview: ${label}`);console.log('kind:',debugKind,'needsLighting:',needsLighting);console.log('declared uniforms:',declared.map(u=>`${u.type} ${u.name}`));console.log('VERTEX SHADER\n',vs);console.log('PIXEL SHADER\n',fs);console.groupEnd();}// Image-based lighting: binds the already-fetched,
// shell-level env textures to whatever sampler names
// THIS shader uses, matched loosely against version drift.
if(needsLighting){// "u_envIrradiance" also matches /radiance/i, so the
// radiance pattern must exclude it explicitly here.
const radSampler=findSampler(/radiance|specular|prefilter/i,/irradiance/i);const irrSampler=findSampler(/irradiance|diffuse/i);if(radSampler)newUniforms[radSampler.name]={value:envRadiance};if(irrSampler)newUniforms[irrSampler.name]={value:envIrradiance};// Captured so the view-handle's setEnvironment()/
// setEnvRotation()/setEnvExposure() methods below can
// live-swap/mutate the right uniforms after creation.
envRadSamplerName=radSampler&&radSampler.name;envIrrSamplerName=irrSampler&&irrSampler.name;// +90° Y is the official viewer's fixed base; the
// user's rotation adds on top, seeded from
// envRotationRad (not 0) so a material swap preserves it.
if(has('u_envMatrix'))newUniforms.u_envMatrix={value:new THREE.Matrix4().makeRotationY(Math.PI/2+envRotationRad)};if(has('u_envRadianceMips'))newUniforms.u_envRadianceMips={value:envMips};if(has('u_envRadianceSamples'))newUniforms.u_envRadianceSamples={value:16};// Seeded from envExposure (not a literal 1.0) so a
// material swap PRESERVES whatever exposure the
// user already dialed in via setEnvExposure().
if(has('u_envLightIntensity')&&!newUniforms.u_envLightIntensity)newUniforms.u_envLightIntensity={value:envExposure};// Generated ESSL declares u_refractionTwoSided (the name
// the official viewer also binds). false matches upstream's
// LightHandler default; true double-squares tinted transmission.
if(has('u_refractionTwoSided'))newUniforms.u_refractionTwoSided={value:false};// Direct lights = rig (fixed) + auto-extracted env
...
const nLights=activeLightCount(lightData,envKeyLight,null,srcs.maxLights);if(has('u_numActiveLightSources'))newUniforms.u_numActiveLightSources={value:nLights};if(has('u_lightData')){const entries=currentLights(lightData,envKeyLight,envRotationRad,null,undefined,null,srcs.maxLights);newUniforms.u_lightData={value:entries};}if(DEBUG_SHADERS){console.log('env bound → radiance:',radSampler&&radSampler.name,'| irradiance:',irrSampler&&irrSampler.name,envHasFile?envPrefilteredIrr?'(radiance + prefiltered irradiance files)':'(radiance file; irradiance SH-synthesized)':'(synthesized)','| direct lights:',nLights,'(rig '+rigCount+' + key '+(envKeyLight?1:0)+')');const envUnbound=declared.filter(u=>/sampler/i.test(u.type)&&/env/i.test(u.name)&&!newUniforms[u.name]);if(envUnbound.length)mtlxWarn('UNBOUND env samplers (likely cause of black):',envUnbound.map(u=>u.name));}}return newUniforms;};// syncMeshMaterialMode, derives the mesh material's
...
// ------------------------------------------------------
const applyMaterialInternal=(srcs,applyLabel)=>{if(geometry&&srcs.geomprops&&srcs.geomprops.length){bindGeompropAttributes(geometry,srcs.geomprops,text=>{if(!srcs.notices)srcs.notices=[];if(!srcs.notices.includes(text))srcs.notices.push(text);});}const newUniforms=bindMaterialUniforms(srcs);// Transparency verdict is srcs.transparent, gated on
...
viewIsTransparent=!!srcs.transparent;const newMaterial=new THREE.RawShaderMaterial({vertexShader:srcs.vs,fragmentShader:srcs.fs,glslVersion:THREE.GLSL3,uniforms:newUniforms,side:THREE.DoubleSide,// Neutral literals: syncMeshMaterialMode() below is the
// real source of truth and overwrites both immediately.
transparent:false,depthWrite:true});// Stash the outgoing material/uniforms so a compile
...
applyMaterialInternal({vs,fs,introspected,transparent,geomprops,notices,maxLights},label);// Contact-shadow casters, only when a studioGroup exists
// to receive them. Full-scene mode has no catcher, so
// `mesh`/sceneGroup meshes there are left untouched.
if(studioGroup){// The whole model casts the contact shadow now: the
...
if(mesh)mesh.castShadow=true;if(sceneGroup){sceneGroup.traverse(obj=>{if(!obj.isMesh||!obj.material)return;const mats=Array.isArray(obj.material)?obj.material:[obj.material];if(mats.some(m=>'envMapIntensity'in m))obj.castShadow=true;});}// Factored out above as updateStudioFloor so a later
// displacement swap can recompute it too.
updateStudioFloor();}// renderFrame, the ONE render entry point for this view,
...
setEnvRotation:rad=>{if(uniforms.u_envMatrix){uniforms.u_envMatrix.value=new THREE.Matrix4().makeRotationY(Math.PI/2+rad);}envRotationRad=rad;// The extracted key light tracks the (clamped) sun's
// position as the env rotates, rig lights don't.
updateKeyLightUniformEntry(uniforms,rigCount,envKeyLight,rad,envExposure);// Studio spotlight follows the SAME rotated direction, so
// the shadow agrees with the highlight; shadow.autoUpdate
// defaults to true, so the shadow map redraws on its own.
placeStudioLight();// Rotates the visible backdrop mesh to match (a real
// geometry rotation, not a texture-offset, see bgMesh's
// declaration above for why offset.x never worked on r128).
if(bgMesh)bgMesh.rotation.y=BG_BASE+BG_SIGN*rad;// Scene-mode neutral parts: mirrors the SAME offset onto
// every patched material's live uEnvRotation uniform, a
// call before first compile is a safe no-op, seeded fresh.
sceneOwnedMaterials.forEach(m=>{const u=m.userData.envRotationUniform;if(u)u.value=envRotationMatrix3(rad);});},// IBL-only exposure multiplier, direct lights are
// unaffected, but IBL is the dominant light source in these
// previews so this reads as a full exposure control.
setEnvExposure:x=>{if(uniforms.u_envLightIntensity)uniforms.u_envLightIntensity.value=x;// Persist onto the shell too: bindMaterialUniforms seeds
// a NEW material's u_envLightIntensity from envExposure,
// so a future swap keeps the user's setting, not resetting to 1.0.
envExposure=x;// Scene-mode's sceneGroup meshes are ordinary glTF PBR
// materials lit via scene.environment/PMREM, their
// envMapIntensity is the equivalent knob. Skip `mesh`.
if(sceneGroup){sceneGroup.traverse(obj=>{if(obj.isMesh&&obj!==mesh&&obj.material&&'envMapIntensity'in obj.material){obj.material.envMapIntensity=x;}});}},// Re-derives the material's blend/depth flags from the stored
...
envKeyLight=env.keyLight||null;envSoftKeyDir=env.softKeyDir||null;updateKeyLightUniformEntry(uniforms,rigCount,envKeyLight,envRotationRad,envExposure);// Same refresh for the shadow: without this the studio light
// would keep aiming along the PREVIOUS env's key light until
// the next rotation change.
placeStudioLight();// bgMesh is null for previews with no env, guard so
// an Import/Reset broadcast (setEnvOverride's LIVE_VIEWS
// loop) can't throw calling this standalone.
if(bgMesh){bgMesh.material.map=envBgTexture;bgMesh.material.needsUpdate=true;}// Scene-mode PMREM regen: a PMREM render target is baked
...
getDisplacementState:()=>({state:dispState,mode:displacementSources?displacementSources.mode:null,level:dispSubdivLevel||0,capped:!!dispCappedNotice,triangles:dispTriangles,notices:currentDispNotices()}),// Resolves once no evaluation is actively in flight (merely
// waiting on a file map does NOT count, that could hang forever).
whenDisplacementSettled:()=>dispRunInFlight?dispSettlePromise||Promise.resolve():Promise.resolve(),// Called by the P3 setters through LIVE_VIEWS on every live
// view; a no-op for flat2d or a material with no displacement.
refreshDisplacement:()=>{if(flat2d||!displacementSources)return;if(!getDisplacementEnabled()){applyDispDebounceToken++;cancelDisplacementRun();if(dispState!=='off'){swapMeshGeometry(originalGeometry);dispState='off';dispDispatchStatus();}return;}const prevLevel=dispSubdivLevel;const wasOff=dispState==='off'||dispState==='none';ensureBaseGeometry();if(wasOff||prevLevel!==dispSubdivLevel)runDisplacement();},// bindDroppedTextures calls this once per drop for every live
...
el.style.height='auto';el.style.maxWidth='none';el.style.maxHeight='none';el.style.margin='0';el.style.zIndex='9990';el.style.backgroundColor='#111827';}catch(e){// Couldn't style el at all, nothing was actually maximized,
