...
const countFragmentSamplers=fs=>{const re=/uniform\s+(?:(?:low|medium|high)p\s+)?(sampler2D|sampler3D|samplerCube)\s+(\w+)\s*(?:\[\s*(\d+)\s*\])?\s*;/g;const names=[];let count=0;let m;while((m=re.exec(fs))!==null){const size=m[3]?parseInt(m[3],10):1;count+=size;names.push(m[3]?m[2]+'['+m[3]+']':m[2]);}return{count,names};};// Estimates fragment uniform vector cost against MAX_FRAGMENT_UNIFORM_VECTORS.
...
const CONST_INPUT_DENY=new Set(['thin_walled','geometry_thin_walled','transmission_weight','transmission_color','transmission_depth','geometry_opacity']);const CONST_INPUT_GLSL_TYPES={float:'float',integer:'int',boolean:'bool'};// Deterministic GLSL literal for one introspected default; floats always
...
const constInputKey=u=>{const name=String(u&&u.name||'');if(!name||name.indexOf('u_')===0)return null;// engine/private uniform
const seg=u.path?String(u.path).split('/').pop():'';if(seg&&CONST_INPUT_NAMES.indexOf(seg)>=0)return seg;for(const n of CONST_INPUT_NAMES){if(name===n||name.endsWith('_'+n))return n;}return null;};// Rewrites `uniform T name;` into `const T name = <literal>;` for the
...
const constifyInputUniforms=(vs,fs,introspected)=>{const constInputs=[];const kept=[];let outVs=vs;let outFs=fs;for(const u of introspected){const glslType=CONST_INPUT_GLSL_TYPES[u.type];const key=glslType?constInputKey(u):null;// `mode` is a generic name: only take it when the generator typed it
// as an enum selector (integer), never a float or boolean input.
if(!key||CONST_INPUT_DENY.has(String(u.name))||key==='mode'&&u.type!=='integer'){kept.push(u);continue;}const literal=u.data==null?null:constInputLiteral(u.type,u.data);if(literal==null){kept.push(u);continue;}const escaped=String(u.name).replace(/[.*+?^${}()|[\]\\]/g,'\\$&');const declRe=new RegExp('(^|\\n)([ \\t]*)uniform[ \\t]+(?:(?:low|medium|high)p[ \\t]+)?'+glslType+'[ \\t]+'+escaped+'[ \\t]*;','g');const hits=(outVs.match(declRe)||[]).length+(outFs.match(declRe)||[]).length;if(hits!==1){kept.push(u);continue;}const replacement='$1$2const '+glslType+' '+u.name+' = '+literal+';';outVs=outVs.replace(declRe,replacement);outFs=outFs.replace(declRe,replacement);constInputs.push({name:u.name,path:u.path||null,value:u.data});}return{vs:outVs,fs:outFs,introspected:kept,constInputs};};// Sweep run before every writeToXmlString call, fixing two attributes
...
if(!isMounted()){cleanup();return'bailed';}}if(window.MTLX_PERF_LOG){console.log('[mtlx-perf] GL compile wait: '+(performance.now()-__waitStart).toFixed(1)+'ms (target: '+label+')');}if(!timedOut)MTLX_WARMED_SOURCES.add(key);cleanup();return'done';};// Background driver pre-warm for an off-screen preview target, builds,
...
const colorspaceDoc=mxSafe(()=>renderable.getDocument(),null)||documentArg;let colorspaceAliasResult=null;let colorspaceTransformResult=null;let materialWorkspaceResult=null;if(colorspaceDoc){colorspaceAliasResult=applyColorspaceAliases(colorspaceDoc);// Must follow the aliases: an authored "srgb_tx" only becomes a
...
if(materialWorkspaceResult)materialWorkspaceResult.restore();if(colorspaceTransformResult)colorspaceTransformResult.restore();if(colorspaceAliasResult)colorspaceAliasResult.restore();}if(window.MTLX_PERF_LOG){console.log('[mtlx-perf] gen.generate: '+(performance.now()-__genPerfStart).toFixed(1)+'ms (target: '+label+')');}// Stage identifiers: some JS builds don't expose the mx.Stage enum
...
const vsAnchorRe=/gl_Position\s*=\s*u_viewProjectionMatrix\s*\*\s*hPositionWorld\s*;/;let vsSpliced=false;if(vsAnchorRe.test(vs)){vs=vs.replace(vsAnchorRe,'gl_Position = vec4(i_dispTexel, 0.0, 1.0);\n    gl_PointSize = 1.0;');vs=vs.replace(/\bvoid\s+main\s*\(/,'in vec2 i_dispTexel;\nvoid main(');vsSpliced=true;}if(!psSpliced||!vsSpliced){mtlxWarn('mtlx-engine: displacement splice anchor missing, '+(!vsSpliced?'vertex: '+vs.slice(0,200):'pixel: '+fs.slice(0,200)));throw new Error('generated shader anchors not found (vertex or pixel); MaterialX codegen changed');}const key=fnv1aHex(vs+String.fromCharCode(0)+fs+String.fromCharCode(0)+JSON.stringify(introspected.map(u=>({name:u.name,type:u.type,data:u.data,samplerModes:u.samplerModes||null}))));return{vs,fs,introspected,geomprops,mode,key,notices};}finally{if(colorspaceTransformResult)colorspaceTransformResult.restore();if(colorspaceAliasResult)colorspaceAliasResult.restore();if(hadHwTransparency){try{genContext.getOptions().hwTransparency=prevHwTransparency;}catch(e){/* option absent */}}try{shader&&shader.delete&&shader.delete();}catch(e){/* already deleted */}}};// Public entry point: serializes generatePreviewSourcesUnlocked against
...
const generatePreviewSourcesWithinBudget=async args=>{const overrideBudget=typeof window!=='undefined'?window.__mtlxSamplerBudgetOverride:undefined;const budget=Number.isFinite(overrideBudget)?overrideBudget:DEFAULT_SAMPLER_BUDGET;const dropped=[];let srcs=null;for(let attempt=0;;attempt++){// The caller's feature gating is the base; budget drops add to it.
const sceneFeatureOptions=Object.assign({},args.sceneFeatureOptions||null);for(const d of dropped)sceneFeatureOptions[d.key]=true;srcs=await generatePreviewSources(Object.assign({},args,{sceneFeatureOptions}));if(!srcs)return null;if(countFragmentSamplers(srcs.fs).count<=budget)break;if(attempt>=SAMPLER_BUDGET_DROP_ORDER.length)break;dropped.push(SAMPLER_BUDGET_DROP_ORDER[attempt]);}if(dropped.length){const count=countFragmentSamplers(srcs.fs).count;const effects=joinWithAnd(dropped.map(d=>d.userLabel));srcs.notices=(srcs.notices||[]).concat(['Texture slots: this material uses more textures than this GPU allows ('+budget+'), so '+effects+(dropped.length>1?' are':' is')+' turned off for it']);srcs.samplerBudget={limit:budget,count,dropped:dropped.map(d=>d.label),droppedLabels:dropped.map(d=>d.userLabel)};}return srcs;};// Scene-view material compiler. This deliberately exposes the preview shader
...
try{compileFilteringDriverNoise(renderer,scene,camera);// Attribute to THIS material's own program, not the first broken
...
const warmResult=await prewarmShaderCompile({vs,fs,isMounted,label});if(warmResult==='bailed'||!isMounted()){disposePartial();return null;}// --- three.js scene (WebGL2) ---
