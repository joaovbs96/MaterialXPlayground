...
const parseUniforms=src=>{const out=[];const re=/uniform\s+(\w+)\s+(u_\w+)\s*(?:\[\s*\w+\s*\])?\s*;/g;let m;while((m=re.exec(src))!==null)out.push({type:m[1],name:m[2]});return out;};// three.js RawShaderMaterial + glslVersion:GLSL3 prepends its own
...
const patchAmbientOcclusion=fs=>{const anchor=/(\/\/ Ambient occlusion\s*\n\s*)occlusion = 1\.0;/;if(!anchor.test(fs))return fs;// Sky visibility needs the world position; without that varying only the
// screen space term is available and the volume lookup is skipped.
const hasWorldPos=/\bin\s+vec3\s+positionWorld\s*;/.test(fs)&&/\bin\s+vec3\s+normalWorld\s*;/.test(fs);const out=fs.replace(anchor,hasWorldPos?'$1occlusion = mx_ssao_occlusion() * mx_sky_visibility();':'$1occlusion = mx_ssao_occlusion();');const skyDecls=!hasWorldPos?[]:[// Baked coarse visibility of the environment (js/usd-scene-skyvis.js).
...
const patchTransmissionThickness=fs=>{const anchor='vdf.throughput = exp(-absorption);';if(fs.indexOf(anchor)===-1)return fs;// Needs the standard HW varyings to locate the fragment along the ray.
const hasVars=/\bin\s+vec3\s+positionWorld\s*;/.test(fs)&&/uniform\s+vec3\s+u_viewPosition\s*;/.test(fs);if(!hasVars)return fs;let out=fs.replace(anchor,'vdf.throughput = exp(-absorption * mx_transmission_path_length());');const decls=['uniform sampler2D u_thicknessMap;','uniform vec2 u_thicknessTexel;',// The renderer's sceneRoot has converted positions to metres. The
...
// ------------------------------------------------------------------
const generatePreviewSourcesUnlocked=({mx,gen,genContext,renderable,label,isMounted=()=>true,document:documentArg=null,sceneRgbt=false,lightTransport=false})=>{// OFFICIAL PARITY: per-material generation options on SHARED
...
fs=patchTransmissionAlpha(fs);let payloadSupported=false;if(sceneRgbt){fs=patchRgbtPayload(fs);payloadSupported=fs.indexOf('/* MX_RGBT_PAYLOAD_SUPPORTED */')!==-1;}let lightTransportSupported=false;if(lightTransport){fs=patchLightTransportPayload(fs,notices,lightTransport);lightTransportSupported=fs.indexOf('MX_LIGHT_TRANSPORT_TERMINAL_RETURN')!==-1;}fs=patchShadowBounds(fs);fs=patchShadowLightScope(fs);fs=patchLightSourceKindStruct(fs);fs=patchAreaLightSourceCosine(fs);fs=patchAmbientOcclusion(fs);fs=patchTransmissionThickness(fs);// Depth-peel machinery: baked into every fragment shader
...
const generatePreviewSources=(...args)=>mxExclusive(()=>generatePreviewSourcesUnlocked(...args));// Scene-view material compiler. This deliberately exposes the preview shader
...
const compileMtlxSceneMaterial=async({mx,gen,genContext,renderable,label='material',isMounted=()=>true,document:documentArg=null,sceneRgbt=false,lightTransport=false})=>{if(!renderable)throw new Error('MaterialX scene material is missing its renderable surface.');const srcs=await generatePreviewSources({mx,gen,genContext,renderable,label,isMounted,document:documentArg,sceneRgbt,lightTransport});if(!srcs)return null;const declared=parseUniforms(srcs.vs).concat(parseUniforms(srcs.fs));return{...srcs,declared,// Program identity excludes uniforms and object transforms. Source
// text is already fully adapted by generatePreviewSources.
programKey:srcs.vs+'\\n/* scene-fs */\\n'+srcs.fs,sceneRgbt,lightTransport:lightTransport===true?1:Number(lightTransport)||0,lightTransportSupported:!!srcs.lightTransportSupported,payloadSupported:!!srcs.payloadSupported,label};};// Create a detached uniform map for one scene object. Every call returns a
...
if(has('u_thicknessMap'))uniforms.u_thicknessMap={value:thicknessMap||getDummyTex()};if(has('u_thicknessTexel'))uniforms.u_thicknessTexel={value:thicknessTexel?thicknessTexel.clone():new THREE.Vector2()};if(has('u_thicknessScale'))uniforms.u_thicknessScale={value:thicknessMap?thicknessScale:0};if(has('u_thicknessTargetValid'))uniforms.u_thicknessTargetValid={value:thicknessMap?1:0};if(has('u_thicknessReferencePath'))uniforms.u_thicknessReferencePath={value:0};// Squares the tint for a closed solid, where the ray crosses the surface
