...
'uniform int u_shadowSlotFace['+SHADOW_LIGHT_SLOTS_MAX+'];','uniform int u_shadowSlotFaceCount['+SHADOW_LIGHT_SLOTS_MAX+'];','#define SHADOW_NORMAL_OFFSET_TEXELS '+SHADOW_NORMAL_OFFSET_TEXELS.toFixed(4),'#define SHADOW_DEPTH_BIAS_TEXELS '+SHADOW_DEPTH_BIAS_TEXELS.toFixed(4),'float mx_shadow_vsm(vec2 moments, float receiverDepth) {','    float p = (receiverDepth <= moments.x) ? 1.0 : 0.0;','    float variance = max(moments.y - moments.x * moments.x, 2e-7);','    float d = receiverDepth - moments.x;','    float lit = max(p, variance / (variance + d * d));','    return smoothstep(0.3, 1.0, lit);','}','float mx_shadow_atlas(int caster, vec3 P, vec3 Ng) {','    if (caster < 0) return 1.0;','    vec4 depthPlane = u_shadowDepthPlanes[caster];','    vec2 depthRange = u_shadowDepthRanges[caster];','    float nearDepth = depthRange.x;','    float depthSpan = max(depthRange.y, 1e-9);','    vec2 projectionScale = u_shadowSourceRadii[caster].zw;','    bool perspective = projectionScale.x > 0.0 || projectionScale.y > 0.0;','    vec2 sourceRadius = u_shadowSourceRadii[caster].xy;',// Normal offset AND depth bias share one texel-world estimate,
...
const dataDirToWorld=(tex,x,y,W,H)=>{const U=(x+0.5)/W;const gamma=2*Math.PI*U-Math.PI;const vRow=tex.flipY?H-1-y:y;const thetaV=Math.PI*(vRow+0.5)/H;const sinV=Math.sin(thetaV),cosV=Math.cos(thetaV);return new THREE.Vector3(sinV*Math.cos(gamma),cosV,sinV*Math.sin(gamma)).negate();};const extractKeyLight=tex=>{try{const img=tex.image;const W=img.width,H=img.height;const stride=img.data.length/(W*H);const isHalf=img.data.constructor===Uint16Array;const rd=i=>isHalf?halfToFloat(img.data[i]):img.data[i];const wr=(i,v)=>{img.data[i]=isHalf?floatToHalf(v):v;};// Pass 1: per-texel luminance + solid-angle weight -> mean + peak.
const lum=new Float32Array(W*H);let sumW=0,sumLW=0,peakL=-1,peakX=0,peakY=0;for(let y=0;y<H;y++){const theta=Math.PI*(y+0.5)/H;const dOmega=Math.sin(theta)*(2*Math.PI/W)*(Math.PI/H);for(let x=0;x<W;x++){const idx=(y*W+x)*stride;const Lraw=0.2126*rd(idx)+0.7152*rd(idx+1)+0.0722*rd(idx+2);const finiteL=Number.isFinite(Lraw);const L=finiteL?Lraw:0;// stray non-finite texel: 0 for sums, never the peak
lum[y*W+x]=L;sumW+=dOmega;sumLW+=L*dOmega;if(finiteL&&L>peakL){peakL=L;peakX=x;peakY=y;}}}const meanL=sumW>0?sumLW/sumW:0;if(!(peakL>=KEYLIGHT_MIN_CONTRAST*Math.max(meanL,1e-6)))return null;// no sun-like source
...
const Lfloor=Math.max(8*meanL,0.02*peakL);let Er=0,Eg=0,Eb=0,cxW=0,cyW=0,cW=0;let annR=0,annG=0,annB=0,annN=0;const clusterIdx=[];for(let y=0;y<H;y++){const theta=Math.PI*(y+0.5)/H;const dOmega=Math.sin(theta)*(2*Math.PI/W)*(Math.PI/H);const sinT=Math.sin(theta),cosT=Math.cos(theta);for(let x=0;x<W;x++){const phi=2*Math.PI*(x+0.5)/W;const dx=sinT*Math.cos(phi),dy=cosT,dz=sinT*Math.sin(phi);const cosAng=dx*pDir[0]+dy*pDir[1]+dz*pDir[2];const ang=Math.acos(Math.min(1,Math.max(-1,cosAng)));const idx=(y*W+x)*stride;const L=lum[y*W+x];if(!Number.isFinite(L))continue;// stray non-finite texel: excluded from cluster and annulus
if(ang<=KEYLIGHT_RADIUS_RAD&&L>=Lfloor){const r=rd(idx),g=rd(idx+1),b=rd(idx+2);Er+=r*dOmega;Eg+=g*dOmega;Eb+=b*dOmega;cxW+=x*(L*dOmega);cyW+=y*(L*dOmega);cW+=L*dOmega;clusterIdx.push(idx);}else if(ang>KEYLIGHT_RADIUS_RAD&&ang<=2*KEYLIGHT_RADIUS_RAD){annR+=rd(idx);annG+=rd(idx+1);annB+=rd(idx+2);annN++;}}}if(!clusterIdx.length||cW<=0)return null;const cx=cxW/cW,cy=cyW/cW;// Direction: data-coord centroid -> world, via the shared helper
// above (also used by extractSoftKeyDir).
const direction=dataDirToWorld(tex,cx,cy,W,H);// Clamp: overwrite the cluster with the annulus's mean color, the
// "split" that removes the sun from radiance/irradiance/backdrop.
const aN=annN||1;const annColor=[annR/aN,annG/aN,annB/aN];for(const idx of clusterIdx){wr(idx,annColor[0]);wr(idx+1,annColor[1]);wr(idx+2,annColor[2]);}const maxE=Math.max(Er,Eg,Eb,1e-8);return{direction,color:[Er/maxE,Eg/maxE,Eb/maxE],intensity:maxE};}catch(e){console.warn('key-light extraction failed:',e);return null;}};// Cheaper direction-only estimate for the studio shadow when
...
if(has('u_envMatrix')){const m=new THREE.Matrix4().makeRotationY(Math.PI/2+envRotationRad);uniforms.u_envMatrix={value:envTilt?m.multiply(envTilt):m};}if(has('u_envRadianceMips'))uniforms.u_envRadianceMips={value:mips};if(has('u_envRadianceSamples'))uniforms.u_envRadianceSamples={value:16};if(has('u_envLightIntensity'))uniforms.u_envLightIntensity={value:envExposure};// White moments read as fully lit, so materials are unaffected until a
