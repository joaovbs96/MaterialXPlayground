...
return out.slice(0,at)+'\n    /* MX_LIGHT_TRANSPORT_EARLY_RETURN MX_LIGHT_TRANSPORT_TAIL_PRUNED */\n    '+om[1]+'='+payload+'; return;\n'+out.slice(mainEnd);};// Gives MaterialX's volume absorption the path length it is missing.
//
...
programKey:srcs.vs+'\\n/* scene-fs */\\n'+srcs.fs,sceneRgbt,lightTransport:lightTransport===true?1:Number(lightTransport)||0,lightTransportSupported:!!srcs.lightTransportSupported,payloadSupported:!!srcs.payloadSupported,label,samplerCount:samplerInfo.count,samplerNames:samplerInfo.names,samplerBudget:{limit:budget,count:samplerInfo.count,dropped:dropped.map(d=>d.label)},samplerOverBudget:overBudget};};// Create a detached uniform map for one scene object. Every call returns a
