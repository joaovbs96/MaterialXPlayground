...
let out=fs.replace(call,'occlusion = 1.0;');// A material's light loop evaluates the same world-space point once per
...
'uniform float u_shadowTexelWorldSize['+SHADOW_CASTER_SLOTS+'];',// Per light slot: which caster shadows it, or -1 for none.
...
'    float lit = mx_shadow_vsm(moments, receiverDepth);','    if (sourceRadius.x <= 0.0 && sourceRadius.y <= 0.0) {','        vec2 e0 = min(sc.xy, vec2(1.0) - sc.xy);','        return mix(1.0, lit, smoothstep(0.0, 0.04, min(e0.x, e0.y)));','    }','    vec2 searchRadius = sourceRadius * projectionScale * 0.5 / max(nearDepth, 1e-9)','        * max(receiverZ - nearDepth, 0.0) / max(receiverZ, 1e-6);','    searchRadius = min(searchRadius, vec2(2.0) * tileTexel);',// Estimate a blocker over a source-size footprint. The search and
...
u_shadowAtlas:{value:shadowAtlas||getDummyTexWhite()},u_shadowMatrices:{value:shadowMatrices&&shadowMatrices.length===SHADOW_CASTER_SLOTS?shadowMatrices:Array.from({length:SHADOW_CASTER_SLOTS},()=>new THREE.Matrix4())},u_shadowTiles:{value:shadowTiles&&shadowTiles.length===SHADOW_CASTER_SLOTS?shadowTiles:Array.from({length:SHADOW_CASTER_SLOTS},()=>new THREE.Vector4(0,0,1,1))},u_shadowDepthPlanes:{value:shadowDepthPlanes&&shadowDepthPlanes.length===SHADOW_CASTER_SLOTS?shadowDepthPlanes:Array.from({length:SHADOW_CASTER_SLOTS},()=>new THREE.Vector4(0,0,0,1))},u_shadowDepthRanges:{value:shadowDepthRanges&&shadowDepthRanges.length===SHADOW_CASTER_SLOTS?shadowDepthRanges:Array.from({length:SHADOW_CASTER_SLOTS},()=>new THREE.Vector2(0,1))},u_shadowSourceRadii:{value:shadowSourceRadii&&shadowSourceRadii.length===SHADOW_CASTER_SLOTS?shadowSourceRadii:Array.from({length:SHADOW_CASTER_SLOTS},()=>new THREE.Vector4())},u_shadowTexelWorldSize:{value:shadowTexelSizes&&shadowTexelSizes.length===SHADOW_CASTER_SLOTS?shadowTexelSizes:new Array(SHADOW_CASTER_SLOTS).fill(0)},u_shadowSlotCaster:{value:shadowSlotCaster&&shadowSlotCaster.length===SHADOW_LIGHT_SLOTS_MAX?shadowSlotCaster:new Int32Array(SHADOW_LIGHT_SLOTS_MAX).fill(-1)},// Baked sky visibility. Seeded unconditionally, NOT through has():
...
u_shadowAtlas:{value:getDummyTexWhite()},u_shadowMatrices:{value:Array.from({length:SHADOW_CASTER_SLOTS},()=>new THREE.Matrix4())},u_shadowTiles:{value:Array.from({length:SHADOW_CASTER_SLOTS},()=>new THREE.Vector4(0,0,1,1))},u_shadowDepthPlanes:{value:Array.from({length:SHADOW_CASTER_SLOTS},()=>new THREE.Vector4(0,0,0,1))},u_shadowDepthRanges:{value:Array.from({length:SHADOW_CASTER_SLOTS},()=>new THREE.Vector2(0,1))},u_shadowSourceRadii:{value:Array.from({length:SHADOW_CASTER_SLOTS},()=>new THREE.Vector4())},u_shadowTexelWorldSize:{value:new Array(SHADOW_CASTER_SLOTS).fill(0)},u_shadowSlotCaster:{value:new Int32Array(SHADOW_LIGHT_SLOTS_MAX).fill(-1)},// Same sampler-unit hazard as the Scene: see
