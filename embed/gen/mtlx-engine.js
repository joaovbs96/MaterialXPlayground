...
let mxRigLightCount=0;const mxEnvPromises=new Map();// Classic-<script> fallback for UMD builds (e.g. 1.39.4) that have no
...
mxRemoveAttr(inp,'value');}if(inp&&wantedType&&mxElType(inp)!==wantedType){mtlxWarn('ensureTypedInput: "'+inputName+'" is "'+mxElType(inp)+'" (wanted "'+wantedType+'"), path='+how);}return inp;};// Light-limit kill switch, default ON. '0' disables it, read per
...
const chooseStageLightTier=count=>{const n=Number(count);if(!Number.isFinite(n)||n<0)return STAGE_LIGHT_SLOTS;const tier=STAGE_LIGHT_TIERS.find(t=>t>=n);return tier===undefined?STAGE_LIGHT_SLOTS:tier;};// Const-inputs kill switch, default ON. '0' disables it. Read per
// generation.
...
// ------------------------------------------------------------------
const generatePreviewSourcesUnlocked=({mx,gen,genContext,renderable,label,isMounted=()=>true,document:documentArg=null,sceneRgbt=false,lightTransport=false,sceneFeatureOptions=null,stageLightCount=null,allowConstInputs=true})=>{// Sampler-budget drops, requested only by compileMtlxSceneMaterial's
...
if(!skipOcclusion)fs=patchAmbientOcclusion(fs,{skipSkyVis,skipAoVolume});fs=patchTransmissionThickness(fs,{dropThicknessMap});// Depth-peel machinery: baked into every fragment shader
...
try{mxShader.delete();}catch(e){/* already deleted */}// What this source does NOT contain, so uniform seeding can skip the
// same features instead of binding samplers no program declares.
const featureSkips={shadowMap:skipShadowMap,occlusion:skipOcclusion,skyVis:skipOcclusion||skipSkyVis,aoVolume:skipOcclusion||skipSkyVis||skipAoVolume};return{vs,fs,introspected,transparent,vertexInputs,geomprops,notices,payloadSupported,lightTransportSupported,maxLights,constInputs,featureSkips};};// Public entry point: serializes generatePreviewSourcesUnlocked against
...
const DEFAULT_UNIFORM_VECTOR_BUDGET=1024;const compileMtlxSceneMaterial=async({mx,gen,genContext,renderable,label='material',isMounted=()=>true,document:documentArg=null,sceneRgbt=false,lightTransport=false,samplerBudget=null,uniformVectorBudget=null,stageLightCount=null,featureOptions=null})=>{if(!renderable)throw new Error('MaterialX scene material is missing its renderable surface.');// Test-only override wins over the caller's live GL limit, so a headless
...
for(const u of introspected){if(u.type!=='filename')continue;const slot=uniforms[u.name];if(!samplerHoldsDefault(slot))continue;slot.value=getFilenameDefaultTexture(introspected,u.name);}};// ------------------------------------------------------------------
...
// ------------------------------------------------------------------
const tryRefreshRenderView=async({view,mx,gen,genContext,renderable,label,isMounted=()=>true})=>{const __t=window.MTLX_PERF_LOG?performance.now():0;let srcs;try{// Same generation options the live view was built with, else the
// byte compare below can never match.
srcs=await generatePreviewSourcesWithinBudget({mx,gen,genContext,renderable,label,isMounted,stageLightCount:PREVIEW_STAGE_LIGHT_COUNT,sceneFeatureOptions:PREVIEW_FEATURE_OPTIONS,allowConstInputs:view?view.allowConstInputs!==false:true});}catch(e){return{refreshed:false,srcs:null};}if(!srcs)return{refreshed:false,srcs:null};// Belt-and-suspenders: compare the transparency verdict explicitly
...
const __srcs=await generatePreviewSourcesWithinBudget({mx,gen,genContext,renderable,label,isMounted,stageLightCount:PREVIEW_STAGE_LIGHT_COUNT,sceneFeatureOptions:PREVIEW_FEATURE_OPTIONS,allowConstInputs});// Bail if this build was superseded while awaiting above:
...
if(stopped||!isMounted())return null;if(!srcs){srcs=await generatePreviewSourcesWithinBudget({mx,gen,genContext,renderable,label,isMounted,stageLightCount:PREVIEW_STAGE_LIGHT_COUNT,sceneFeatureOptions:PREVIEW_FEATURE_OPTIONS,allowConstInputs});}// A thrown generation error is NOT caught here, it
