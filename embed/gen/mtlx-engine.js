...
const skipSkyVis=!!(sceneFeatureOptions&&sceneFeatureOptions.skipSkyVis);const dropThicknessMap=!!(sceneFeatureOptions&&sceneFeatureOptions.dropThicknessMap);const skipTransmittance=!!(sceneFeatureOptions&&sceneFeatureOptions.skipTransmittance);// OFFICIAL PARITY: per-material generation options on SHARED
...
const SAMPLER_BUDGET_DROP_ORDER=[{key:'skipSkyVis',label:'sky visibility (u_skyVisMap)'},{key:'dropThicknessMap',label:'thickness map (u_thicknessMap)'},{key:'skipTransmittance',label:'shadow transmittance (u_shadowTransmittance)'}];// Scene-view material compiler. This deliberately exposes the preview shader
