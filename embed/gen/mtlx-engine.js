...
const hasWorldPos=!skipSkyVis&&/\bin\s+vec3\s+positionWorld\s*;/.test(fs)&&/\bin\s+vec3\s+normalWorld\s*;/.test(fs);const out=fs.replace(anchor,hasWorldPos?'$1occlusion = mx_ssao_occlusion() * mx_sky_visibility();':'$1occlusion = mx_ssao_occlusion();');const skyDecls=!hasWorldPos?[]:[// Baked coarse visibility of the environment (js/usd-scene-skyvis.js).
...
'    vec4 moments = texture(u_skyVisMap, uvw);','    float visibilityMean = moments.r;','    vec3 visibilityDirection = (moments.gba * 255.0 - vec3(128.0)) / 127.0;','    float vis = clamp(visibilityMean + dot(visibilityDirection, skyNormal), 0.0, 1.0);','    return mix(1.0, vis, clamp(u_skyVisStrength, 0.0, 1.0));','}'];const decls=['uniform sampler2D u_ssaoMap;','uniform vec2 u_ssaoTexel;','uniform float u_ssaoStrength;','float mx_ssao_occlusion() {','    float ao = texture(u_ssaoMap, gl_FragCoord.xy * u_ssaoTexel).r;','    return mix(1.0, clamp(ao, 0.0, 1.0), clamp(u_ssaoStrength, 0.0, 1.0));','}'].concat(skyDecls).concat(['']).join('\n');// Must land before the FIRST function definition, not before main():
...
const generatePreviewSources=(...args)=>mxExclusive(()=>generatePreviewSourcesUnlocked(...args));// ANGLE D3D11 reports 16 texture image units; a Scene material bakes eleven
...
u_skyVisMap:{value:getDummyTex3DWhite()},u_skyVisMin:{value:new THREE.Vector3()},u_skyVisSize:{value:new THREE.Vector3(1,1,1)},u_skyVisCell:{value:0},u_skyVisStrength:{value:0}};// GLSL ES 3.0 forbids uniform initializers, so the app
