          ...
          if (HwGen && HwGen.bindLightShader && ldef) {
            ...
            for (const [name, id] of [['ND_point_light', LIGHT_TYPE_POINT], ['ND_spot_light', LIGHT_TYPE_SPOT]]) {
              try {
                ...
                console.warn('light shader ' + name + ' unavailable:', e);
              }
            }
...
const parseVertexInputs = vs => {
  ...
  let m;
  while ((m = re.exec(vs)) !== null) out.push({
    type: m[1],
    name: m[2]
  });
  return out;
...
const patchGeompropVaryings = (vs, fs) => {
  ...
  const patchedVs = vs.replace(/^\s*out\s+(\w+)\s+i_geomprop_(\w+)\s*;/gm, 'out $1 vd_geomprop_$2;').replace(/^(\s*)i_geomprop_(\w+)\s*=\s*i_geomprop_\2\s*;/gm, '$1vd_geomprop_$2 = i_geomprop_$2;');
...
const patchShadowBounds = fs => {
  ...
  const guard = ['if (shadowCoord4.w <= 0.0) return 1.0;', 'if (any(lessThan(shadowCoord, vec3(0.0))) || any(greaterThan(shadowCoord, vec3(1.0)))) return 1.0;',
...
const patchShadowLightScope = fs => {
  ...
  '    float receiverDepth = dot(vec4(offsetP, 1.0), depthPlane);', '    float receiverZ = nearDepth + depthSpan * receiverDepth;',
  ...
  '    float lit = mx_shadow_vsm(moments, receiverDepth);', '    if (sourceRadius.x <= 0.0 && sourceRadius.y <= 0.0) {', '        vec2 e0 = min(sc.xy, vec2(1.0) - sc.xy);', '        return mix(1.0, lit, smoothstep(0.0, 0.04, min(e0.x, e0.y)));', '    }', '    vec2 searchRadius = sourceRadius * projectionScale * 0.5 / max(nearDepth, 1e-9)', '        * max(receiverZ - nearDepth, 0.0) / max(receiverZ, 1e-6);', '    searchRadius = min(searchRadius, vec2(2.0) * tileTexel);',
...
const applyColorspaceTransforms = (doc, maxDepth) => {
  ...
  if (docSpace && docSpace !== 'lin_rec709') {
    return {
      restore: () => {},
      converted,
      unsupported: new Set(['(document works in "' + docSpace + '", not lin_rec709)'])
    };
  }
  let serial = 0;
  const visit = (parent, depth) => {
    ...
    for (const node of nodes) {
      ...
      if (!fileInput || !mxElHasAttr(fileInput, 'colorspace')) {
        visit(node, depth + 1);
        continue;
      }
...
const loadBoundedBitmapTexture = async (blob, maxSize) => {
  ...
  let source;
  try {
    source = await createImageBitmap(blob, opts);
  } catch (error) {
    throw new Error('The source image could not be decoded for bounded scene preview.');
  }
  let image = source;
  if (maxSize > 0 && Math.max(source.width, source.height) > maxSize) {
    ...
    try {
      image = await createImageBitmap(blob, resizeOpts);
    } catch (error) {
  ...
  try {
    ...
    if (buf[0] === 0xff && buf[1] === 0xd8) {
      ...
      while (offset + 9 < buf.length) {
        ...
        if (marker >= 0xc0 && marker <= 0xcf && marker !== 0xc4 && marker !== 0xc8 && marker !== 0xcc) {
          return {
            height: dv.getUint16(offset + 5),
            width: dv.getUint16(offset + 7)
          };
        }
        offset += 2 + segLen;
      }
    ...
    if (isLE || isBE) {
      ...
      for (let i = 0; i < count; i += 1) {
        ...
        if (tag === 256) width = value;else if (tag === 257) height = value;
      }
    ...
    if (cached) {
      ...
      if (ext === 'ktx2') {
        ...
        const startBoundedLoad = () => {
          ...
          return typeof createImageBitmap === 'function' ? loadBoundedBitmapTexture(blob, Number(view.maxTextureSize)) : Promise.reject(new Error('createImageBitmap is unavailable for bounded scene texture preview'));
        };
        let boundedLoad;
...
const prepGeometry = geometry => {
  ...
  const writeFrame = (index, tx, ty, tz, sign, tangentOut, bitangentOut) => {
    ...
    tangentOut[index * 3] = tx;
    tangentOut[index * 3 + 1] = ty;
    tangentOut[index * 3 + 2] = tz;
    bitangentOut[index * 3] = bx;
    bitangentOut[index * 3 + 1] = by;
    bitangentOut[index * 3 + 2] = bz;
  };
  ...
  const canTangent = !!(geometry.getIndex() && position && normal && geometry.getAttribute('uv'));
  ...
  if (!iTangent && !geometry.getIndex() && position.count >= 3) {
    ...
    for (let base = 0; base + 2 < position.count; base += 3) {
      ...
      const du1 = uv.getX(base + 1) - uv.getX(base),
        dv1 = uv.getY(base + 1) - uv.getY(base);
      const du2 = uv.getX(base + 2) - uv.getX(base),
        dv2 = uv.getY(base + 2) - uv.getY(base);
...
const initGlobalGeom = () => {
  let stored = null;
  try {
    stored = localStorage.getItem(GLOBAL_GEOM_KEY);
...
const setGlobalGeom = value => {
  ...
  if (window.self === window.top && value !== 'custom') {
    try {
      localStorage.setItem(GLOBAL_GEOM_KEY, value);
    } catch (e) {/* privacy mode */}
  }
  window.dispatchEvent(new CustomEvent('mtlx-global-geom', {
    detail: {
      value
    }
  }));
...
const broadcastDisplaySettings = () => {
  LIVE_VIEWS.forEach(v => {
    try {
      v.refreshDisplaySettings && v.refreshDisplaySettings();
    } catch (e) {/* view mid-teardown */}
  });
};
...
const initDisplayTransform = () => {
  let stored = null;
  try {
    stored = localStorage.getItem(DISPLAY_TRANSFORM_KEY);
  } catch (e) {/* privacy mode */}
...
const setDisplayTransform = value => {
  ...
  if (window.self === window.top) {
    try {
      localStorage.setItem(DISPLAY_TRANSFORM_KEY, value);
    } catch (e) {/* privacy mode */}
  }
  broadcastDisplaySettings();
  window.dispatchEvent(new CustomEvent('mtlx-display-transform', {
    detail: {
      value
    }
  }));
  ...
  const parts = meshes.map(mesh => {
    ...
    if (keepIndex) {
      ...
      for (let i = 0; i < vcount; i++) {
        ...
        if (normal) {
          normal[i * 3] = normAttr.getX(i);
          normal[i * 3 + 1] = normAttr.getY(i);
          normal[i * 3 + 2] = normAttr.getZ(i);
        }
      ...
      for (let k = 0; k < vcount; k++) {
        ...
        position[k * 3] = posAttr.getX(i);
        position[k * 3 + 1] = posAttr.getY(i);
        position[k * 3 + 2] = posAttr.getZ(i);
        if (normal) {
          normal[k * 3] = normAttr.getX(i);
          normal[k * 3 + 1] = normAttr.getY(i);
          normal[k * 3 + 2] = normAttr.getZ(i);
        }
        if (hasUv) {
          uv[k * 2] = uvAttr.getX(i);
          uv[k * 2 + 1] = uvAttr.getY(i);
        }
      }
      g.setAttribute('position', new THREE.BufferAttribute(position, 3));
      if (normal) g.setAttribute('normal', new THREE.BufferAttribute(normal, 3));
  ...
  (json.materials || []).forEach(mat => {
    ...
    Object.keys(mat.extensions || {}).forEach(key => {
      ...
      Object.keys(ext).forEach(k => {
        if (/Texture$/.test(k)) delete ext[k];
      });
    });
  });
...
const getDracoLoader = () => {
  ...
  if (!dracoLoaderInstance) {
    dracoLoaderInstance = new THREE.DRACOLoader().setDecoderPath(new URL('vendor/three/draco/', document.baseURI).href);
  }
  ...
  try {
    ...
    try {
      ...
      gltfLoader.setKTX2Loader({
        load: (u, onLoad) => onLoad(new THREE.Texture())
      });
      const parsePromise = new Promise((resolve, reject) => {
        try {
          gltfLoader.parse(payload, resourcePath, resolve, reject);
        } catch (e) {
          reject(e);
        }
      });
      ...
      const gltf = declaresDraco ? await Promise.race([parsePromise, new Promise((_, reject) => {
      ...
      if (ext === 'gltf') {
        const hint = hasSidecars || !resourcePath ? 'This .gltf references external data; select its .bin file(s) together with the .gltf.' : 'A file referenced by this .gltf could not be fetched (CORS or missing).';
...
const commitCustomGeom = (root, label, seqId) => {
  ...
  if (seqId !== customGeomLoadSeq) {
    try {
      built.dispose();
    } catch (e) {/* registry copy is never GPU-uploaded */}
    return null;
  }
  ...
  window.dispatchEvent(new CustomEvent('mtlx-custom-geom', {
    detail: {
      epoch: CUSTOM_GEOM.epoch,
      name: CUSTOM_GEOM.name
    }
  }));
...
const loadCustomPreviewGeomFromFile = async input => {
  ...
  const files = input instanceof FileList ? Array.from(input) : Array.isArray(input) ? input : input ? [input] : [];
  ...
  const root = await parseModelRoot(ext, data, primary.name, {
    sidecars
  });
...
const loadCustomPreviewGeomFromUrl = async url => {
  ...
  const opts = ext === 'obj' ? undefined : {
    resourcePath: clean.slice(0, clean.lastIndexOf('/') + 1)
  };
...
'    vec3 worldN = mx_latlong_map_projection_inverse(vec2(uv.x + 0.5, uv.y));', '    float alpha = mx_latlong_lod_to_alpha(uMip);',
...
const ensurePrefilteredEnv = (renderer, env) => {
  ...
  const w = src.image && src.image.width,
    h = src.image && src.image.height;
  if (!w || !h) return env;
  ...
  if (window.MTLX_PERF_LOG) {
    console.log('[mtlx-perf] env prefilter: ' + (performance.now() - t0).toFixed(1) + 'ms (' + levels + ' levels, ' + w + 'x' + h + ')');
  }
  return env;
};
...
const mxOutputTypeAndNode = (node, outputName) => {
  let outs = [];
  try {
    outs = vecToArray(node.getOutputs ? node.getOutputs() : null);
  } catch (e) {
    outs = [];
  }
  if (outs.length > 1 || mxElType(node) === 'multioutput') {
    const out = outputName ? mxSafe(() => node.getOutput(outputName), null) || outs.find(o => mxElName(o) === outputName) : outs[0];
    if (!out) return null;
    return {
      type: mxElType(out),
      producer: mxSafe(() => out.getConnectedNode(), null)
    };
  }
  return {
    type: mxElType(node),
...
const findTypeMismatches = (renderable, mx) => {
  ...
  const walk = (node, depth) => {
    ...
    for (const input of inputs) {
      if (found) return;
      const connected = mxElAttr(input, 'nodename') || mxElAttr(input, 'nodegraph') || mxElAttr(input, 'interfacename');
...
const generatePreviewSourcesUnlocked = ({
  ...
  try {
    genContext.getOptions().premultipliedBsdfAdd = false;
  } catch (e) {/* option absent in older bindings */}
  ...
  try {
    ...
    const wanted = getSpecularEnvMethod() === 'fis' ? methods.SPECULAR_ENVIRONMENT_FIS : methods.SPECULAR_ENVIRONMENT_PREFILTER;
  ...
  if (mismatch) {
    throw new Error(`Material "${label}": input "${mismatch.inputName}" on "${mismatch.nodeName}" ` + `(${mismatch.inputType}) is connected to "${mismatch.sourceName}" (${mismatch.sourceType}); ` + 'MaterialX requires matching types');
  }
  let mxShader;
  ...
  ({
    vs,
    fs
  } = patchGeompropVaryings(vs, fs));
  const vertexInputs = parseVertexInputs(vs);
  const geomprops = vertexInputs.filter(v => v.name.startsWith('i_geomprop_')).map(v => ({
    name: v.name.slice('i_geomprop_'.length),
    type: v.type
  }));
  const notices = [];
...
const compileMtlxSceneMaterial = async ({
  mx,
  gen,
  genContext,
  renderable,
...
const createMtlxSceneUniforms = ({
  ...
  const uniforms = {
    ...
    u_shadowAtlas: {
      value: shadowAtlas || getDummyTexWhite()
    },
    u_shadowMatrices: {
      value: shadowMatrices && shadowMatrices.length === SHADOW_CASTER_SLOTS ? shadowMatrices : Array.from({
        length: SHADOW_CASTER_SLOTS
      }, () => new THREE.Matrix4())
    },
    u_shadowTiles: {
      value: shadowTiles && shadowTiles.length === SHADOW_CASTER_SLOTS ? shadowTiles : Array.from({
        length: SHADOW_CASTER_SLOTS
    ...
    u_shadowTexelWorldSize: {
      value: shadowTexelSizes && shadowTexelSizes.length === SHADOW_CASTER_SLOTS ? shadowTexelSizes : new Array(SHADOW_CASTER_SLOTS).fill(0)
    },
    u_shadowSlotCaster: {
  ...
  if (has('u_time')) uniforms.u_time = {
    value: MTLX_CLOCK.time
  };
  if (has('u_frame')) uniforms.u_frame = {
    value: MTLX_CLOCK.frame
  };
  if (has('u_envRadiance')) uniforms.u_envRadiance = {
    value: radiance
  };
  if (has('u_envIrradiance')) uniforms.u_envIrradiance = {
    value: irradiance
  };
  ...
  if (has('u_envRadianceSamples')) uniforms.u_envRadianceSamples = {
    value: 16
  };
...
const createPeelPipeline = (renderer, {
  ...
  const render = (scene, camera, transparentMeshes, opts = {}) => {
    ...
    try {
      ...
      scene.traverse(o => {
        if (o.isMesh && !meshSet.has(o) && o.visible) {
          o.visible = false;
          hidden.push(o);
        }
      });
...
const createMtlxRenderView = async ({
  ...
  const disposeFetchedEnv = env => {
    ...
    try {
      if (env.radiancePrefiltered) env.radiancePrefiltered.dispose();
    } catch (e) {/* ditto */}
    try {
  ...
  try {
    ...
    onGlLost = () => {
      window.dispatchEvent(new CustomEvent('mtlx-gl-context', {
        detail: {
          canvas,
          state: 'lost'
        }
    ...
    peelPipeline = createPeelPipeline(renderer, {
      getDisplayTransform
    });
    const scene = new THREE.Scene();
    ...
    const bindMaterialUniforms = srcs => {
      ...
      const newUniforms = {
        ...
        u_shadowTexelWorldSize: {
          value: new Array(SHADOW_CASTER_SLOTS).fill(0)
        },
        u_shadowSlotCaster: {
      ...
      if (has('u_time')) newUniforms.u_time = {
        value: MTLX_CLOCK.time
      };
      if (has('u_frame')) newUniforms.u_frame = {
        value: MTLX_CLOCK.frame
      };
    ...
    const handle = {
      ...
      captureFrame: () => {
        ...
        if (!__captureCanvas) {
          ...
          __captureCtx = __captureCanvas.getContext('2d', {
            willReadFrequently: true
          });
        }
        if (__captureCanvas.width !== w || __captureCanvas.height !== h) {
          __captureCanvas.width = w;
          __captureCanvas.height = h;
        }
...
Object.assign(window, {
  ...
  loadCustomPreviewGeomFromFile,
  loadCustomPreviewGeomFromUrl,
  getCustomPreviewGeom,
  clearCustomPreviewGeom,
  getGlobalGeom,
  setGlobalGeom,
  getDisplayTransform,
  setDisplayTransform,
  ...
  loadEnvironmentFromBuffer,
  makeFlatEnvironment,
  setEnvOverride,
  getEnvOverride,
  getKeyLightEnabled,
  setKeyLightEnabled,
  prewarmShaderCompile,
  createMtlxRenderView,
  compileMtlxSceneMaterial,
