...
const pickSubdivisionLevel=(baseTriangles,requestedLevel,budget=PREVIEW_TRIANGLE_BUDGET)=>{let level=0;let triangles=baseTriangles;for(let l=0;l<=requestedLevel;l++){const t=baseTriangles*Math.pow(4,l);if(l===0||t<=budget){level=l;triangles=t;}else{break;}}return{level,capped:level<requestedLevel,triangles};};// LRU (3 entries) of undisplaced subdivided base geometries, shared by
...
const loadExrTexture=async blob=>{if(typeof THREE.EXRLoader==='undefined'){console.warn('mtlx-engine: THREE.EXRLoader unavailable (script blocked/offline); .exr textures keep the node default color.');return null;}try{const buf=await blob.arrayBuffer();const d=new THREE.EXRLoader().setDataType(THREE.FloatType).parse(buf);if(!d||!d.data)return null;const tex=new THREE.DataTexture(d.data,d.width,d.height,d.format,d.type);tex.minFilter=tex.magFilter=THREE.LinearFilter;return tex;}catch(e){console.warn('mtlx-engine: failed to parse dropped .exr texture, keeping the node default color:',e);return null;}};// Parses a dropped .hdr Blob via THREE.RGBELoader's synchronous .parse().
...
const loadTextureForHit=async(hit,blob,view)=>{const ext=(hit.key.split('.').pop()||'').toLowerCase();if(ext==='exr')return loadExrTexture(blob);if(ext==='hdr')return loadHdrTexture(blob);if(ext==='tif'||ext==='tiff')return loadTifTexture(blob,hit.key);if(view&&view.maxTextureSize&&typeof createImageBitmap==='function'){return loadBoundedBitmapTexture(blob,Number(view.maxTextureSize));}return new Promise(resolve=>{const url=URL.createObjectURL(blob);new THREE.TextureLoader().load(url,tex=>{URL.revokeObjectURL(url);resolve(tex);},undefined,()=>{URL.revokeObjectURL(url);resolve(null);});});};// Scene snapshots can contain many UDIM tiles.  When a caller supplies a
...
const plainizeMxUniformData=u=>{if(u.data==null||!VECTOR_MX_TYPES.has(u.type))return u;return Object.assign({},u,{data:mxDataToPlainArray(u.data)});};// Converts a MaterialX default value into a three.js uniform. Returns
...
const configureLoadedTexture=t=>{t.wrapS=t.wrapT=THREE.RepeatWrapping;t.flipY=false;t.anisotropy=8;t.needsUpdate=true;return t;};// ---- Preview geometry ----
...
const bindGeompropAttributes=(geometry,geomprops,notify)=>{if(!geometry||!geomprops||!geomprops.length)return geometry;const uv=geometry.getAttribute('uv');for(const{name,type}of geomprops){const attrName='i_geomprop_'+name;if(geometry.getAttribute(attrName))continue;if(type==='vec2'&&uv){geometry.setAttribute(attrName,uv);continue;}const itemSize=GEOMPROP_ITEM_SIZE[type];if(itemSize){const count=geometry.getAttribute('position')?geometry.getAttribute('position').count:0;geometry.setAttribute(attrName,new THREE.BufferAttribute(new Float32Array(count*itemSize),itemSize));}if(typeof notify==='function'){notify(`geompropvalue "${name}" (${type}) has no geometry stream in this viewer and reads zeros`);}}return geometry;};// Center a geometry at the origin and scale it to bounding radius 1
...
let introspected=[];for(const stageName of[VERTEX_STAGE,PIXEL_STAGE]){let st=null;try{st=mxShader.getStage(stageName);}catch(e){/* stage absent */}if(st)introspected=introspected.concat(collectMxUniforms(st));}introspected=introspected.map(plainizeMxUniformData);// uv_scale/uv_offset are ALWAYS emitted as uniforms (never inline
...
const generateDisplacementSourcesUnlocked=({mx,gen,genContext,renderable,materialName})=>{const notices=[];const source=resolveDisplacementSource({mx,renderable,materialName,notices});if(!source)return null;const mode=detectDisplacementMode(source);let prevHwTransparency;let hadHwTransparency=true;try{prevHwTransparency=genContext.getOptions().hwTransparency;}catch(e){hadHwTransparency=false;}let shader=null;try{try{genContext.getOptions().hwTransparency=false;}catch(e){/* option absent */}try{shader=gen.generate('mtlx_displacement',source,genContext);}catch(genErr){const msg=mxErr(mx,genErr);if(/mix/i.test(msg)&&/implementation/i.test(msg)){throw new Error('Displacement mixing is not supported by the MaterialX shader generator.');}throw new Error('shader generation failed ('+msg.slice(0,160)+')');}const VERTEX_STAGE=mx.Stage&&mx.Stage.VERTEX||'vertex';const PIXEL_STAGE=mx.Stage&&mx.Stage.PIXEL||'pixel';let vs=stripVersion(shader.getSourceCode(VERTEX_STAGE));let fs=stripVersion(shader.getSourceCode(PIXEL_STAGE));let introspected=[];for(const stageName of[VERTEX_STAGE,PIXEL_STAGE]){let st=null;try{st=shader.getStage(stageName);}catch(e){/* stage absent */}if(st)introspected=introspected.concat(collectMxUniforms(st));}introspected=introspected.map(plainizeMxUniformData);// Pixel splice: retarget the discarded final assignment into a
...
const vsAnchorRe=/gl_Position\s*=\s*u_viewProjectionMatrix\s*\*\s*hPositionWorld\s*;/;let vsSpliced=false;if(vsAnchorRe.test(vs)){vs=vs.replace(vsAnchorRe,'gl_Position = vec4(i_dispTexel, 0.0, 1.0);\n    gl_PointSize = 1.0;');vs=vs.replace(/\bvoid\s+main\s*\(/,'in vec2 i_dispTexel;\nvoid main(');vsSpliced=true;}if(!psSpliced||!vsSpliced){mtlxWarn('mtlx-engine: displacement splice anchor missing, '+(!vsSpliced?'vertex: '+vs.slice(0,200):'pixel: '+fs.slice(0,200)));throw new Error('generated shader anchors not found (vertex or pixel); MaterialX codegen changed');}const key=fnv1aHex(vs+String.fromCharCode(0)+fs+String.fromCharCode(0)+JSON.stringify(introspected.map(u=>({name:u.name,type:u.type,data:u.data}))));return{vs,fs,introspected,mode,key,notices};}finally{if(hadHwTransparency){try{genContext.getOptions().hwTransparency=prevHwTransparency;}catch(e){/* option absent */}}try{shader&&shader.delete&&shader.delete();}catch(e){/* already deleted */}}};// Public entry point: serializes generatePreviewSourcesUnlocked against
...
const SAMPLER_BUDGET_DROP_ORDER=[// Parked with screen-space reflections (always skipped for now).
// { key: 'skipSsr', label: 'screen-space reflection (u_opaqueColor)' },
{key:'skipAoVolume',label:'occlusion volume (u_aoVolumeMap)'},{key:'skipSkyVis',label:'sky visibility (u_skyVisMap)'},{key:'dropThicknessMap',label:'thickness map (u_thicknessMap)'},{key:'skipTransmittance',label:'shadow transmittance (u_shadowTransmittance)'},{key:'skipRefraction',label:'refraction colour (u_opaqueColor)'}];// Viewer-path generation with the same sampler budget as the Scene: drops
...
programKey:srcs.vs+'\\n/* scene-fs */\\n'+srcs.fs,sceneRgbt,lightTransport:lightTransport===true||lightTransport===4||lightTransport==='transfer'?4:0,lightTransportSupported:!!srcs.lightTransportSupported,payloadSupported:!!srcs.payloadSupported,label,samplerCount:samplerInfo.count,samplerNames:samplerInfo.names,samplerBudget:{limit:budget,count:samplerInfo.count,dropped:dropped.map(d=>d.label)},samplerOverBudget:overBudget,fragmentUniformVectors:{estimate:uniformInfo.estimate,limit:uniformLimit,largest:uniformInfo.largest},fragmentUniformOverBudget:uniformInfo.estimate>uniformLimit};};// Create a detached uniform map for one scene object. Every call returns a
...
let dispState='none',dispSubdivLevel=null,dispTriangles=0;let dispCappedNotice=null,dispDroppedNotice=null,dispEvalNotices=[];let materialNotices=[];let dispFileMap=null,dispRunInFlight=false;let dispSettlePromise=null,dispSettleResolve=null;let applyDispDebounceToken=0;// Reassigned once, below, to the real handle object literal; declared
...
const getBallBoundingSphere=()=>{if(ballBoundingSphere)return ballBoundingSphere;if(!sceneGroup)return null;const ballNode=sceneGroup.getObjectByName('shader_ball')||mesh&&mesh.parent||sceneGroup;ballNode.updateMatrixWorld(true);const box=new THREE.Box3().setFromObject(ballNode);ballBoundingSphere=box.getBoundingSphere(new THREE.Sphere());return ballBoundingSphere;};// Single entry point for every fov-affecting event so they
...
const swapMeshGeometry=g=>{const prevDisplaced=displacedGeometry;displacedGeometry=g!==originalGeometry&&g!==baseGeometry?g:null;geometry=g;if(mesh){mesh.geometry=g;ballBoundingSphere=null;updateStudioFloor();if(sceneOrbitFitRadius!=null){const sphere=getBallBoundingSphere();if(sphere)sceneOrbitFitRadius=sphere.radius;}}if(prevDisplaced&&prevDisplaced!==g){try{prevDisplaced.dispose();}catch(e){/* already disposed/invalid */}}};// Non-indexed corners, Loop-subdivided (creases at authored
// normal breaks) then welded back into an indexed geometry;
// other attributes are dropped (reported by the caller).
const subdivideSourceGeometry=(source,level)=>{const nonIndexed=source.index?source.toNonIndexed():source;const posAttr=nonIndexed.getAttribute('position');const normAttr=nonIndexed.getAttribute('normal');const uvAttr=nonIndexed.getAttribute('uv');const toArr=attr=>attr?attr.array instanceof Float32Array?attr.array:Float32Array.from(attr.array):null;const meshIn={positions:toArr(posAttr),normals:toArr(normAttr),uvs:toArr(uvAttr)};if(nonIndexed!==source)nonIndexed.dispose();if(!meshIn.positions)return null;const subdivided=MtlxMeshSubdivision.subdivideMesh(meshIn,level,{creaseByNormals:true});if(!subdivided)return null;const welded=MtlxMeshSubdivision.weldMesh(subdivided);const out=new THREE.BufferGeometry();out.setAttribute('position',new THREE.BufferAttribute(welded.positions,3));out.setAttribute('normal',new THREE.BufferAttribute(welded.normals,3));if(welded.uvs)out.setAttribute('uv',new THREE.BufferAttribute(welded.uvs,2));out.setIndex(new THREE.BufferAttribute(welded.indices,1));prepGeometry(out);// prepGeometry, aliasUvGeomprops and computeTangents rebuild these on
// the subdivided mesh, so only genuinely lost attributes are reported.
const rebuilt=new Set(['position','normal','uv','i_position','i_normal','i_texcoord_0','tangent','i_tangent','i_bitangent',...UV_GEOMPROP_ALIASES]);const dropped=Object.keys(source.attributes).filter(n=>!rebuilt.has(n));return{geometry:out,dropped};};// Ensures baseGeometry reflects the current preview
// subdivision setting (capped to the triangle budget),
// rebuilding only when the resolved level changed.
const ensureBaseGeometry=()=>{const posAttr=originalGeometry.getAttribute('position');const idx=originalGeometry.getIndex();const baseTriangleCount=Math.max(1,Math.round((idx?idx.count:posAttr?posAttr.count:3)/3));const overrideBudget=window.__mtlxTriangleBudgetOverride;const budget=Number.isFinite(overrideBudget)?overrideBudget:triangleBudget;const{level,capped,triangles}=pickSubdivisionLevel(baseTriangleCount,getPreviewSubdivisionLevel(),budget);if(dispSubdivLevel===level&&baseGeometry)return{level,capped,triangles};let built=originalGeometry;let dropped=[];if(level>0){const cacheKey=baseGeomCacheKey(geomName,sceneMode,level);const cached=baseGeomCacheGet(cacheKey);if(cached){built=cached.clone();}else{const result=subdivideSourceGeometry(originalGeometry,level);if(result){dropped=result.dropped;baseGeomCacheSet(cacheKey,result.geometry);built=result.geometry.clone();}}}if(baseGeometry&&baseGeometry!==originalGeometry){try{baseGeometry.dispose();}catch(e){/* already disposed/invalid */}}baseGeometry=built;dispSubdivLevel=level;dispTriangles=triangles;dispCappedNotice=capped?'Subdivision capped at level '+level+' ('+triangles+' triangles) to stay under the budget':null;dispDroppedNotice=dropped.length?'Subdivision dropped extra vertex attributes: '+dropped.join(', '):null;syncHandleNotices();return{level,capped,triangles};};// Computes displaced positions/normals from `base` and
...
let fitCenter=null,fitRadius=null;if(mesh){mesh.updateMatrixWorld(true);const bb=new THREE.Box3().setFromObject(mesh);fitCenter=bb.getCenter(new THREE.Vector3());const bs=bb.getSize(new THREE.Vector3());fitRadius=Math.max(bs.x,bs.y,bs.z)/2;}else if(sphere){fitCenter=sphere.center;fitRadius=sphere.radius;}sceneOrbitFitRadius=fitRadius;sceneOrbitFitDist=fitCenter?camera.position.distanceTo(fitCenter):null;recomputeCameraFov();camera.updateProjectionMatrix();// Snapshot for resetCamera(): position0/target0 now
