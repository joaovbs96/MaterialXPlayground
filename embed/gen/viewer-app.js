...
const loadMtlxDocument = async (xmlText, path, version) => {
  ...
  const genContext = renderables.some(r => r.definition) && typeof env.createGenContext === 'function' ? env.createGenContext() : env.genContext;
...
function MaterialViewerApp({
  ...
  React.useEffect(() => {
    ...
    const onCustomGeom = () => {
      if (surfaceHidden()) {
        pendingCustomGeomRef.current = true;
        return;
      }
      applyCustomGeom();
    };
  ...
  React.useEffect(() => {
    const onGlContext = e => {
      ...
      if (d.state === 'lost') {
        ...
        if (surfaceHidden()) pendingGlRestoredRef.current = true;else setGlEpoch(n => n + 1);
      }
    };
  ...
  React.useEffect(() => {
    const flush = () => {
      ...
      requestAnimationFrame(() => {
        ...
        if (pendingCustomGeomRef.current) {
          pendingCustomGeomRef.current = false;
          applyCustomGeom();
        }
